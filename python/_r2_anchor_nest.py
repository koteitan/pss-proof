...
# R2 anchor test (CORRECTED): the per-column fold operates on the FULL host
# Mq @ take m B (NOT the slice).  n0 = jm1 of Mq itself (fixed throughout).
#   host(m) = Mq @ B[:m],  m=0..w   (B = Msq[len(Mq):], Msq = Mq @ B)
#   c1(m)   = transC1(host(m+1)) = Mark(host(m))(transJm1(host(m+1)))
#   accumulator before op m = Mark(host(m), n0)
# anchor: (Mark(host(m),n0), c1(m)) in MarkedB  <=>  scb_decomp exists.
# Reduction via Mark_MarkedB_nest needs:
#   marked(host(m), n0), marked(host(m), j_m), n0 <= j_m, reduced(host(m))
