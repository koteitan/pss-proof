#!/usr/bin/env python3
# r27: is RN(transC1 M)!1 == RN(Trans M)!1 ? And relate to RightAnces(seg M 0 jm1)!1.
import sys, time
...
def run(ml,cap,budget,mn,ms):
    ...
    t0=time.time(); tot=0; eqTM=0; badTM=[]; TMadmpos=0
    for M in pool:
        ...
        tot+=1
        TM=Trans(M); rnTM=RN(TM)
        # transJm1 M > 0 ?
        if jm1>0: TMadmpos+=1
        if len(rnTM)>=2 and rnTM[1]==rn[1]: eqTM+=1
        else: badTM.append((fmt(M), rn[1], rnTM[:4], jm1))
    print(f"tot={tot} jm1>0 count={TMadmpos}")
    print(f"  RN(transC1)!1 == RN(Trans M)!1 : {eqTM}/{tot}  bad={len(badTM)}")
    for b in badTM[:8]: print("    CEX", b)
