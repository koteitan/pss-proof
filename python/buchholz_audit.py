...
def main():
    ...
    desc_fail=[]; desc_nat_fail=[]; mono_fail=[]; ot_fail=[]; ot_nat_fail=[]
    for a in ot:
        ws = doms_witnesses(a)
        for z in ws:
        ...
        if not is_zero(a):
            for n in range(4):
                ...
                if not in_OT(az):
                    ot_nat_fail.append((fmt(a), n, fmt(az)))
    ...
    print(f"[3.2a] a[z]<a fails={len(desc_fail)} numeral-extension fails={len(desc_nat_fail)}  [3.2b] monotone fails={len(mono_fail)}  [3.3] a[z]∈OT fails={len(ot_fail)} numeral-extension fails={len(ot_nat_fail)}")
    for x in desc_fail[:5]: print("   desc FAIL:", x)
    for x in desc_nat_fail[:5]: print("   numeral FAIL:", x)
    for x in ot_fail[:5]: print("   OT   FAIL:", x)
    for x in ot_nat_fail[:5]: print("   OT numeral FAIL:", x)
