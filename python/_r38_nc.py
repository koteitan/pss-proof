...
t0=time.time()
def gen(seedlist, target, maxL):
    seen=set(); dep={}
    for s in seedlist:
        seen.add(tuple(s)); dep[tuple(s)]=0
    frontier=[tuple(s) for s in seedlist]
    while frontier and len(seen)<target:
        nxt=[]
        for M in frontier:
            if Lng(M)>maxL: continue
            for n in (2,3):
                try:
                    it=oper(list(M),n)
                    if Lng(it)>16: continue
                    R=tuple(Red(it))
                except Exception: continue
                if 2<=Lng(R)<=maxL and reg2(list(R)) and R not in seen:
                    seen.add(R); dep[R]=dep[M]+1; nxt.append(R)
                if len(seen)>=target: break
            if len(seen)>=target: break
        frontier=nxt
    return seen,dep
seeds=[[(i,i) for i in range(k)] for k in (3,4,5,6)]
corpus,dep=gen(seeds, 1500, 10)
print(f"corpus={len(corpus)} maxdepth={max(dep.values())} ({round(time.time()-t0,1)}s)",flush=True)
step=[list(M) for M in corpus if reg7(list(M)) and cj(list(M))<Lng(list(M))-1 and reg7(Pred(list(M))) and len(Br(Pred(list(M))))>0]
sdep=max((dep[tuple(M)] for M in step),default=0)
print(f"STEP-hosts={len(step)} max-deriv-depth={sdep} maxLng={max((Lng(M) for M in step),default=0)} deep(>=6):{sum(1 for M in step if dep[tuple(M)]>=6)}",flush=True)
...
print(f"[STEP impl] ok={s} FAIL={sb}",flush=True)
for c in scex: print("  STEP-CEX",c)
print(f"[keystone case] case4={c4} case3={c3} other={oth}",flush=True)
print(f"[VE4 core hN==tau3] ok={rok} bad={rbad}",flush=True)
print(f"[VE3_N extends] ok={v3ok} bad={v3bad}",flush=True)
