#!/usr/bin/env python3
r"""r22-WIRESTEP: probe the two facts that give  j0' <= transJm1 M cleanly:
    (a) adm M j0'          (last-branch joint is admissible)
    (b) j0' <= j0          (already 1339/1339)
    plus TrMax relations:  j0' <= TrMax M,  TrMax M vs tjm1.
Since  Adm M j0 = Max{j'<=j0 : adm M j'},  (a)+(b) give j0' <= Adm M j0 = tjm1.
Also directly re-check  adm M j0'  AND  j0' <= tjm1  on the STEP domain.
"""
import sys, os, time
...
def main():
    tally = {k: [0,0,[]] for k in
             ['adm_j0p','j0p_le_j0','j0p_le_tjm1','admMax_eq_tjm1',
              'j0p_lt_TrMax','TrMax_le_tjm1','j0p_le_TrMax']}
    def rec(k, ok, info=None):
        tally[k][0 if ok else 1] += 1
        if not ok and len(tally[k][2]) < 8 and info is not None:
            tally[k][2].append(info)
    pool = gen_pool(maxlen=9, maxn=3, maxseed=6, cap=2600)
    ...
    for M in hosts:
        ...
        j0 = parent(M, 0, j1); tjm1 = transJm1(M); trm = TrMax(M)
        n += 1
        info = (fmt(M), f'j0p={j0p} j0={j0} tjm1={tjm1} TrMax={trm} j1p={j1p} j1={j1} admj0p={adm(M,j0p)}')
        rec('adm_j0p', adm(M, j0p), info)
        rec('j0p_le_j0', j0p <= j0, info)
        rec('j0p_le_tjm1', j0p <= tjm1, info)
        # Adm M j0 == Max{j'<=j0 : adm}
        admle = max(jp for jp in range(j0+1) if adm(M, jp))
        rec('admMax_eq_tjm1', admle == tjm1, info)
        rec('j0p_lt_TrMax', j0p < trm, info)
        rec('TrMax_le_tjm1', trm <= tjm1, info)
        rec('j0p_le_TrMax', j0p <= trm, info)
    pr(f"hosts_step={n} ({round(time.time()-t0,1)}s)")
    for k in tally:
        ok, bad, cex = tally[k]
        pr(f"   {k:16s} {ok}/{ok+bad}" + ("" if not bad else f"  CEX(first)={cex[:3]}"))
