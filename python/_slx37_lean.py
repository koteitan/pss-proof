...
st={'n':0,'fn_lt_T':0,'fnp_eq_fn':0,'jlp_eq_jl':0,'lngbr_eq':0,
    'nextR_RNp_1_d_Tp':0,'le0_RNp_fnp_Tp':0,'nextR_M_1_jm2_Lm2':0,'strict':0,
    'jm3pos':0,'dgt1':0}
cexA=[]; cexB=[]
def check(M):
    L=R.Lng(M); j1=L-1
    ...
    if len(br)>0:
        ...
        if fn<T: st['fn_lt_T']+=1
            ...
            if len(cexA)<6: cexA.append(('FNEQT',R.fmt(M),'fn',fn,'T',T,'RN',R.fmt(RN),'RNp',R.fmt(RNp)))
    ...
    if R.nextR(RNp,1,d,Tp): st['nextR_RNp_1_d_Tp']+=1
        ...
        if len(cexB)<6: cexB.append(('noNextB',R.fmt(M),'d',d,'Tp',Tp,'RNp',R.fmt(RNp),'par1Tp',R.parent(RNp,1,Tp)))
