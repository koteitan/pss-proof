...
def cores(maxlen,maxval):
    ...
    for L in range(2,maxlen+1):
        for body in itertools.product(range(maxval+1),repeat=2*L):
            ...
            out.append(M)
    return out
# row1 cross with kk==0: p = parent M 1 j1, npJ* = p+1, entry M 1 j1 = npJ*, entry M 1 p = p, p<=TrMax
n=0;fa=0;fb=0;fc=0;fd=0
for M in cores(5,3):
    Jstar=len(Br(M))-1
    blks=[]
    ...
    nps=npJ(M,Jstar)
    if nps!=p+1:fa+=1
    if entry(M,1,j1)!=nps:fb+=1
    if p>TrMax(M):fc+=1
    if entry(M,1,p)!=p:fd+=1
print(f"row1 cross kk==0 n={n}: npJ*!=p+1:{fa} entry1_j1!=npJ*:{fb} p>TrMax:{fc} entry1_p!=p:{fd}")
