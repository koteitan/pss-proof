...
strictc=[0,0]; tncle=[0,0]; eqc=0; tot=0
ex=[]
for N in Ns:
    ...
    for n in (1,2,3):
        ...
        for j0p in range(LM):
            for j1p in range(j0p+1,LM):
                ...
                tN=TrMax(Nred)
                tot+=1
                tncle[1]+=1; tncle[0]+=(1 if tN<=c else 0)
                strictc[1]+=1; strictc[0]+=(1 if tN+1<=c else 0)
                if tN==c:
                    eqc+=1
                    if len(ex)<8: ex.append((fmt(N),n,j0p,j1p,j0red,j1red,c,tN,(j0red+(j1p-j0p)>LN-1)))
print(f"d1pos std N={len(Ns)} (len<={ml} val<={mv} KMAX={km}) cases={tot}", flush=True)
print(f"  tnc  (tN<=c):    {tncle[0]}/{tncle[1]}", flush=True)
print(f"  strict(tN+1<=c): {strictc[0]}/{strictc[1]}   eq(tN=c)={eqc}", flush=True)
for e in ex: print("  tN=c EX:",e, flush=True)
