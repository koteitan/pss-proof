...
- 🚨 §8 停止性
  ...
  - 🚨 §8.1 条件 (I) の下での展開規則
    ...
    - 🚨 補題（条件 (I) か (III) の下での $c_1$ 前後の具体表示）（A20/A21）
      ...
      - 🚨 (4-1)(4-2)：slicepeel
        - ✅ (4-1)〔`m_8_1_c1_around_part4_1`、setup/head/Nred/B1/Adm0/cond/segpos splice〕
        - 🚨🤖 (4-2)（depth-2、tractable、workflow）
  - 🚨 §8.2 強単項性
    ...
    - 🚨 補題（部分表現の単項成分と $\textrm{Pred}$ の関係）(§8.2 keystone)
      ...
      - 🚨 w-id 討伐 → keystone 組立
        ...
        - ✅ baseU〔`m_8_2_baseU`〕
        - 🚨🤖 keystone⟸branchPar 済〔`m_8_2_subexpr_component_Pred_done`〕、残=branchPar(branch親≥TrMax、workflow)
