...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]
  ...
  - 🚨 §7 Buchholzの表記系への翻訳
    ...
    - 🚨 §7.3 翻訳写像
      ...
      - 🚧 命題（$\textrm{Mark}$ の $\textrm{Trans}$ による表示）〔`m_7_4_Mark_Trans_repr`: (M,m)∈Marked, M∈RT_PS, m<Lng-1 ⟹ Mark M m = Trans(seg M m (Lng-1))(後方切片)。**経験検証 違反0=真**(mono/multi, maxlen4)。原典スケルトン(content 2490-2560)= Lng 強帰納で N=Red(seg M m j1) と M の surgery 一致を示す。**1帰納を残すのみまで分解・全green済**(11コミット, agent並列 wt-repr sole-builder):
        ...
        - **残=Step2 markShift discharge**(`Mark(Pred N)(transJm1 M-m)=Mark(Pred M)(transJm1 M)` を IH at Pred M/Pred N で discharge、transJm1=Lng-2 境界サブケース有)+ **Step4 measure_induct 組立**(object-implication 形, 4ケース dispatch, IH で ihPred/markShift 解消) → **repr7 (wt-repr) 稼働中、keystone 完成狙い**。完成すれば §8.1 c1-around スライス等式が unblock〕
    - 🚨 §7.4 許容的親子関係
