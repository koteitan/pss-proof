...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]〔§5〜§8 全命題の sorry 転記完了、以降は証明フェーズ〕
  ...
  - 🚨 §6 ペア数列の基本性質
    ...
    - 🚨🚧 §6.6 簡約性 〔大半 $T_{\textrm{PS}}$ で真。keystone「reduced ⟺ RedCondA∧RedCondB」は**A4非依存・今証明可能**と確定 (877/0)、`RedCondA⟹red_le` で §6.5 を unblock。設計 `docs/reducedness.md`〕
      ...
      - 🚨 命題（$P$ が簡約性を保つこと）
      - 🚨 命題（簡約性が基本列で保たれること）
      - ✅🚧 命題（簡約性と係数の関係）〔**keystone** `reduced⟺RedCondA∧RedCondB`。**🎉 forward monoT-core 完成**: `kst_reduced_imp_condAB_monoT_core`(M∈RT_PS∧monoT∧core ⟹ RedCondA∧RedCondB) **無条件 green**。最難 condA_top(最終列 RedCondA)を **trunk-reachability 論証**(N-construction は循環と厳密排除 docs §17; `wf22_trunk_reaches_last`/`wf22_row1parent_le_joint`/`wf22_r1cross`)で閉鎖。WLOG: multiT(`key_reduced_iff_cond_multi`)/zeroT(`kst_reduced_iff_cond_zeroT`)/shift(空虚) 既green。**残 forward＝mono m10>0(非core)を core へ還元+general M 組立**。**backward(A∧B⇒reduced) 未着手**。設計 §15-17〕
