...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]
  ...
  - 🚨 §6 ペア数列の基本性質
    ...
    - 🚨 §6.8 降順性 〔`descending` 補題はゼロ(新規)。`m_6_4_P_leftend_mono`(P成分の row-0 左端単調) は既証明＝`descending(P M)` の row-0 部はこれで出る〕
      - 🚨 命題（標準形の切片と $\textrm{Br}$ の降順性の関係）〔`p_6_8_standard_slice_Br_descending`(pss_paper 577): `M∈ST_PS, (0,j0')≤(0,j1') ⟹ monoT(seg M j0' j1') ∧ descending(Br(seg ...))`。**本開発最難**。設計 `docs/slice-Br-descending.md`、経験的に真(`python/sk_68_prop1_audit.py`)。**分解確定**: `monoT(seg)`＝`m_6_2_mono_ancestor_slice`で free、`descending(Br)` 行0部＝枝seg(∈T_PS)に`m_6_4_P_leftend_mono`で free、残る難所＝**Br成分の行1 tie-break**(原文 content.md 1450-1615 の`k_0`帰納＋`N_{1,j1}=0/>0`・商余・`FirstNodes`/`TrMax`/`Joints`/`IncrFirst`・Br-under-oper 分解)。helper 済: `descending_P_of_ST`(X∈ST_PS⟹descending(P X))・`entry_FirstNodes_eq_component_gen`(任意行)。**原文訂正 A7**: 1434「M'が標準形」は誤り(切片は非標準形、反例`(0,0)(1,1)(2,0)`)→「Br(M')降順」が正。ST_PSショートカット不可〕
