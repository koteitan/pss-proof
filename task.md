...
- 🚨 §8 停止性
  ...
  - 🚨 §8.1 条件 (I) の下での展開規則
    - 🚨 命題（条件 (I) の下での $\textrm{Trans}$ と基本列の交換関係）
      ...
      - 🚨 交換則(1)本体〔stepT還元=`m_8_1_Trans_fseq_condI_comm_append_reduce`〕
        ...
        - 🚨🤖 stepT j0>0（残=general step(=flatdec同核,content.md3119-3260)+itercond(無条件でない=Adm0のみ済)+dM、wf wb9mtjtbb wt-mark）
      - 🚨 OT所属（→§8.7）
  ...
  - 🚨 §8.2 強単項性
    ...
    - 🚨 補題（強単項性の下での部分表現の単項成分の基本性質）
      ...
      - 🚨🤖 (2)(3)(4) leBT：factA/wid 無条件化済、widH→det→**keystone case-4排除**還元〔`m_8_2_det_of_case4excl`〕(cases1/2/3済)。残=case4(§7.4 Mark geometry/cp-coupling)、wf wb9mtjtbb wt-detx
  ...
  - 🚨 §8.4 条件 (III) か (IV) の下での展開規則
    - 🚨🤖 命題（条件 (III) か (IV) の下での $\textrm{Trans}$ と基本列の交換関係）：inequality(≤)、lhs→flat-decomp還元〔`m_8_45_lhs_of_flat_decomp`〕、残=flatdec(共有marking-nesting recurrence)、condIV=A27、wf wb9mtjtbb wt-mark
    - 🚨 補題（右端の非許容直系先祖の基本性質）
  ...
  - 🚨 §8.5 条件 (V) の下での展開規則
    - 🚨🤖 命題（条件 (V) の下での $\textrm{Trans}$ と基本列の交換関係）：inequality(≤)+engine、lhs→flatdec還元（=§8.4と同一）、残=flatdec(=kind-0 step 同核)、wf wb9mtjtbb wt-mark
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔**surgery-free** keystone Pred再帰、原文の循環M[n]ルート迂回〕
      ...
      - 🚨🤖 wid（=§8.2 widH/L35 と同一、det→case-4排除に還元済〔`m_8_2_det_of_case4excl`〕。残=case4、wf wb9mtjtbb wt-detx）
