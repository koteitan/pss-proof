...
- 🚨 corrections.md の全数監査(残=A29/A33/A34/A37/A38 の再検証。operB 解禁済)
- ✅ §5 定式化
- 🚨 §6 ペア数列の基本性質
  - 🚨 命題（単項性と $\textrm{Red}$ の関係）の $M_{1,0}=0$ の場合(原文は真。我々の nat 減算のため未証明)
- 🚨 §7 Buchholzの表記系への翻訳
  - 🚨 SCOPE GAP: 原文 $T_{PS}$ → 証明 $RT_{PS}$（7命題）
...
- 🚨 §8 停止性（定理は完了。原文カバレッジに残欠）
  ...
  - ✅ §8.7 主結果
  - 🚨 §8.1 条件 (I) の下での展開規則
    - 🚨 命題（条件 (I) の下での $\textrm{Trans}$ と基本列の交換関係）の $RT_{PS}$ 版（原文は真=26200ホスト0失敗。証明済は $ST_{PS}$ 版のみ）
  - 🚨 §8.3 条件 (II) の下での展開規則
