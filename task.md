...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]〔§5〜§8 全命題の sorry 転記完了、以降は証明フェーズ〕
  ...
  - 🚨 §6 ペア数列の基本性質
    ...
    - 🚨 §6.7 標準形
      ...
      - ✅ 命題（標準形の始切片への遺伝性）
    - 🚨 §6.8 降順性
      - 🚨🤖 命題（標準形の切片と $\textrm{Br}$ の降順性の関係）〔本開発最難、設計 `docs/slice-Br-descending.md`（最新=continued 32）。`m_6_8_slice_Br_descending_monoT`(worktree slice-wip-68, 緑チェックポイント 9177499)は base/caseA/B/C + d0pos 全 case 緑＝**slice_P_tiebreak stub 1本に集約**(agent B B3 assembly)。infra: agent A の i1非依存 `oper_gen_block_nth/entry0/entry1`+`oper_gen_nth_prefix`、`nlocal_adj_tie`(緑/真だが非局所tieで不十分)、d0pos Z1-Z7+H1+B1 緑。**ルート決定(continued 32)=記事忠実・直接**: slice_P_tiebreak(=drop-core)を使わず d0pos を記事 regime A/B 直接 case 分析(N スライスへ IH)で閉じ stub 除去。**brle 分割で単一成分(137/149)完全証明済**(agent A `monoT_seg_of_le0`/`descending_Br_of_branch_le0`, slice bdc0a84, slice_P_tiebreak 削除)。**残＝¬brle 多成分のみ**＝記事 regime A+B 全体(delta-shift付、深掃引で 57 regA/92 regB、prefix は Br N' の +k·δ IncrFirst シフト)≈d0zero caseC 移植+funpow-IncrFirst 新機構(~400行)。**¬brle は文脈で非空虚(18/183, agent B の vacuity 主張は偽=shallow gen artifact、continued 35)**。assembly 全ブリック bank 済(slice f0a51e8): LOW機構 `P_funpow_IncrFirst`/`oper_d1pos_LOW_source_eq`/`oper_d1pos_notbrle_LOW_eq` + `descending_map_IncrFirst`/`descending_shift_append`/`oper_d1pos_notbrle_P_split`/`descending_Br_of_branch_le0`。**残＝assembly wiring**(LOW を take J1 (Br Np) に同定→descending_map_IncrFirst+IHk、junction は descending_shift_append)〕
      - ✅ 命題（標準形の単項成分が降順であること）
  - 🚨 §7 Buchholzの表記系への翻訳
    - 🚨 §7.1 Buchholzの表記系
