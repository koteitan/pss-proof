...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]〔§5〜§8 全命題の sorry 転記完了、以降は証明フェーズ〕
  ...
  - 🚨 §6 ペア数列の基本性質
    ...
    - 🚨🚧 §6.6 簡約性 〔大半 $T_{\textrm{PS}}$ で真。keystone「reduced ⟺ RedCondA∧RedCondB」は**A4非依存・今証明可能**と確定 (877/0)、`RedCondA⟹red_le` で §6.5 を unblock。設計 `docs/reducedness.md`〕
      ...
      - 🚨 命題（$P$ が簡約性を保つこと）
      - 🚨 命題（簡約性が基本列で保たれること）
      - 🚨🚧 命題（簡約性と係数の関係）〔**keystone** `reduced⟺RedCondA∧RedCondB`。WLOG(multiT/zeroT)+forward 入れ子帰納 skeleton 完成。Pred-lift brick 7本 green(`kfwd_pred_agree`/`nextR_Pred_imp`/`_rev`/`hasParent_Pred_iff`/`parent_Pred_eq`/`entry_Pred_eq`/`monoT_hasParent_top`)で zeroT/trunk/全 j'<j1 witness/最終列 RedCondB(空虚)は green。**forward 残＝condA_top ただ1つ**。これが (a)+(b) に分解と確定: **(a) IncrFirst-correspondence green**(`incf_pow_entry0/1`/`incf_pow_nextR`/`incf_pow_hasParent`/`incf_pow_parent`)。**(b) last-block locate green**(`kfwd_lastblock_locate`)。**🎯 keystone forward 全 induction を green conditional lemma `kst_reduced_imp_condAB_monoT_core_cond`(condA_top を明示仮定に他全ケース discharge)に配線完了**＋`ncons_diag_prefix_entry`(p≤TrMax M で entry=p)/`ncons_Pred_core`。**残＝condA_top**。in-block 対応完成(`wf16_inblock_parent_corr`: 最終列の in-block parent↔Rs 列 kk の nextR/parent/entry 全対応)。**残2: (A) cross-block joint case**(parent<off=過半数60/98、最終列 parent が joint Joints M!Jstar/npJ witness と一致＋block-start +1 を示す global-nextR-into-block 解析)、**(B) RedCondA(Rs) transfer**(IH を N=diag@Rs に当て RedCondA N→ecrux/incf diag-prefix で RedCondA Rs へ)。**backward 未着手**。設計 §15〕
