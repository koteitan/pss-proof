...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]
  ...
  - 🚨 §7 Buchholzの表記系への翻訳
    ...
    - 🚨 §7.3 翻訳写像
      ...
      - 🚧 命題（$\textrm{Mark}$ の $\textrm{Trans}$ による表示）〔`m_7_4_Mark_Trans_repr`: (M,m)∈Marked, m<Lng-1 ⟹ Mark M m = Trans(seg M m (Lng-1))(後方切片)。**経験検証 違反0=命題は真**(mono/multi 共, maxlen4)。✅ **橋渡し `m_7_4_Mark0_slice_eq_Trans`(adb9c56)**: monoT 祖先後方切片 S で `Mark S 0 = Trans S` を無条件証明(`ra_Mark0_eq_Trans` を Red 経由で非簡約 S へ一般化)。**これにより「fact2 (`Mark M m = Mark(seg M m j1) 0`) は repr と等価」と判明**(旧「fact2=踏み台」は循環だった)。**残=真のキーストーン** `Mark M m = Trans(seg M m j1)` の内部-m scb-transport。原典スケルトン(content 2490-2560): Lng 強帰納、N=Red(seg M m j1)、IH を Pred M(at m)/Pred N(at 0)に適用し `c0^M=Trans(Pred N)=t1^N`・`c1^M=c1^N`・`c2^M=c2^N` を示して surgery 一致 ⟹ Trans N=Mark M m。テンプレ=`ra_Mark0_eq_Trans`(8257-8378, m=0 ケース)。**repr2 agent (wt-repr) 稼働中**〕
    - 🚨 §7.4 許容的親子関係
