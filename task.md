...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]
  ...
  - 🚨 §8 停止性
    - 🚨 §8.1 条件 (I) の下での展開規則
      ...
      - 🚧 補題（条件 (I) か (III) の下での $c_1$ 前後の具体表示）〔`p_8_1_condI_III_c1_around`(content 2933): 5 part の大連言。**補正 A20**(part(1) スライス等式 `Trans(seg M j0 (j1-1))=c1` は singleton 切片 j0=j1-1 で偽→`j0<j1-1 ⟶` でガード、CEX `M=[(0,0),(1,0),(2,0)]`)・**A21**(part(5) 周期性は condIII で偽→`transCondI M ⟶` でガード、CEX `M=[(0,0),(1,1),(2,1)]`)。✅ **green マージ済(032ea8d)**: `m_8_1_c1_around_part1_noeq`(part(1) のスライス等式以外: Trans(Pred M)≠0/condI∨III/c1∈T_B/c1単項)・`m_8_1_c1_around_part2`(part(2): j0'≤j1-2/(Pred M,jm1')∈Marked/(seg M jm1'(j1-1),j0-jm1')∈Marked、`m_6_3_marked_slice`/`Marked_Pred`/`Marked_Pred_Adm`)。**残**: part(1) スライス等式 `Trans(seg M j0 (j1-1))=c1=Mark(Pred M)jm1` は **§7.4 `m_7_4_Mark_Trans_repr`(✅完成 57498ea)を Pred M, m=jm1 で適用すれば従う(unblock 済)** / part(3)(4) も repr 適用 / part(5) は M[n] 基本列周期性(content 2995-3093)別途。次は repr を使って c1-around を閉じる〕
    - 🚨 §8.2 強単項性
