...
- 🚨 §8 停止性
  ...
  - 🚨 §8.1 条件 (I) の下での展開規則
    - 🚨 命題（条件 (I) の下での $\textrm{Trans}$ と基本列の交換関係）
      ...
      - 🚨 交換則(1)本体〔stepT還元=`m_8_1_Trans_fseq_condI_comm_append_reduce`〕
        ...
        - 🚨🤖 stepT j0>0（残=general step=marking-nesting〔正=funpow-context recurrence+bottom bound〕+itercond(Adm0のみ)+dM、wf wepeviek7 wt-mark3）
      - 🚨 OT所属（→§8.7）
  ...
  - 🚨 §8.2 強単項性
    ...
    - 🚨 補題（強単項性の下での部分表現の単項成分の基本性質）
      ...
      - 🚨🤖 (2)(3)(4) leBT：widH→det→case-4 を単一 edge fact「¬nextR M 1 (TrMax)(L-1)」に絞込。残=global branch-row-1帰納「∀f. e1 f≤e1(TrMax)」→det閉路、wf wepeviek7 wt-det3
  ...
  - 🚨 §8.4 条件 (III) か (IV) の下での展開規則
    - 🚨🤖 命題（条件 (III) か (IV) の下での $\textrm{Trans}$ と基本列の交換関係）：inequality(≤)。正=funpow-context recurrence+bottom bound(flatdec/D_v0 exact-leaf は非universal)、condIV=A27、wf wepeviek7 wt-mark3
    - 🚨 補題（右端の非許容直系先祖の基本性質）
  ...
  - 🚨 §8.5 条件 (V) の下での展開規則
    - 🚨🤖 命題（条件 (V) の下での $\textrm{Trans}$ と基本列の交換関係）：inequality(≤)+engine。正=funpow-context recurrence+bottom bound(=§8.1 step/§8.4 同核)、wf wepeviek7 wt-mark3
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔**step を {det,[Buc1]} に還元**: 4-case dispatcher `m_8_7_OT_keystone_step`/`m_8_7_OT_step_prefix`。残=strong-Lng帰納組立+det+[Buc1]〕
      ...
      - 🚨🤖 wid（=§8.2 widH/L35、det case-4→単一 edge fact、残=branch-row-1帰納→det閉路、wf wepeviek7 wt-det3）
