...
- 🚨 定理（標準形ペア数列システムの停止性）[§8.7 主結果]
  ...
  - 🚨 §7 Buchholzの表記系への翻訳
    ...
    - 🚨 §7.3 翻訳写像
      ...
      - ✅🚧 命題（$\textrm{Trans}$ の well-defined 性）〔**totality on RT_PS green、補正 A15 記録済**: `m_7_3_Trans_welldef`/`m_7_3_Mark_welldef`(Trans_Mark_dom, Lng 強帰納)。⚠️原典の一行証明は Red 冪等性(T_PS で偽=A4)を暗黙使用→T_PS 全域は不成立見込み、使用箇所(RT/ST)は RT_PS 域で十分。**値部(不変量 (Trans,Mark)∈T_B^Marked)の側ブリック群 100% green**: scb image 存在(`scbimg_image_BT`、crossing=dsum)/補正版置換 `scb_replace_principal`/`dfree_flat`/Marked-Pred×3/multi-Marked/最終成分機構+`MarkedB_last_component_transfer`/`monoT_hasParent0_last`。残=本体 Lng 帰納のみ(骨格 docs/trans-invariant-draft.thy.txt)。memory `pss-73-trans-wd`〕
