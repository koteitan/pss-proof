...
さらに、原文の証明の穴が 1 件ある（訂正候補ではなく、こちらで補ったもの）。

* **命題（基本列的順序が辞書式的順序を含意すること）** の内側の帰納法は
  「\(\textrm{Lng}(N)\) と \((N_j)_{j=0}^{j_1^N-1}\) を固定した有限集合」の上での
  下降帰納だが、その集合の \(<_{\textrm{PS}}\)-最大元における基底段階が扱われていない。
  形式化では代わりに「長さ \(L\) 以下の \(CT_{\textrm{PS}}\) の元全体」（これも
  同じ係数評価 \(M'_{1,j}\leq M'_{0,j}\leq j\) で有限）の上で下降帰納した。
  この集合なら最大元では相手が存在せず主張が空虚に成り立つので、基底段階が自動的に閉じる。
  それ以外の論法は原文どおりである（`12-lex-implies-exp.lean` 冒頭に詳述）。
