...
`05` の訂正形、`13` 系（順序の等価性）、`14` 系（順序の線形性）は証明済みだが、
`04` と `12` の `sorry` を経由するため `sorryAx` に依存する。
...
| | 必要なもの |
|---|---|
| `04` 基本列の辞書式的縮小性 | 退化枝は証明済み。tiling 枝に `nextrel0`/`nextrel1` の与える \(M_{0,j_0}<M_{0,j_1}\) / \(M_{1,j_0}<M_{1,j_1}\) が要る |
| `06` 基本列の切片の不変性 | 同上（`seg_oper_prefix_agree_68` が近い） |
| `12` 基本列的順序が辞書式的順序を含意すること | 二重帰納＋「同じ始切片をもつ \(CT_{\textrm{PS}}\) の元は有限個」 |
