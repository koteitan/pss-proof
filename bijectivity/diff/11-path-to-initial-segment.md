...
[1] の $`\textrm{Pred}`$ が $`[1]`$ で表されることより、帰納法により $`(M_j)_{j=0}^{j_1'}=\textrm{Pred}^{j_1-j_1'}(M)=M[1]^{j_1-j_1'}\lt_{\textrm{PS}[]}M`$ であるから、$`(M_j)_{j=0}^{j_1'}\lt_{\textrm{PS}[]}M`$ である。□
...
```math
\lnot\ \Bigl(\forall M\in ST_{\textrm{PS}},\ \forall j_1'\in\mathbb{N},\ j_1'\leq\textrm{Lng}(M)-1\ \Rightarrow\ (M_j)_{j=0}^{j_1'}\lt_{\textrm{PS}[]}M\Bigr)
```
...
ここで $`\leq_{\textrm{PS}[]}`$、$`\lt_{\textrm{PS}[]}`$ は原文どおり、
$`M\leq_{\textrm{PS}[]}N`$ を「ある $`a\in\mathbb{N}_+^{\lt\omega}`$ が存在して $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$」、
$`\lt_{\textrm{PS}[]}`$ をさらに $`a\neq()`$ を課したもの、として定式化してある。
仮定は $`M\in ST_{\textrm{PS}}`$ ではなく $`M\in T_{\textrm{PS}}`$（$`M\neq()`$）である。

以下、$`M\!\restriction\!k`$ で $`M`$ の先頭 $`k`$ 項からなる列（$`k\gt\textrm{Lng}(M)`$ なら $`M`$ 自身）を表す。
...
成分については、$`i\lt j+1`$ をとると仮定から $`i\lt\textrm{Lng}(M)`$ であり、
左辺の第 $`i`$ 項は $`(M_{0,i},M_{1,i})`$、右辺の第 $`i`$ 項は $`M_i`$ である。
$`M_{i,j}`$ の定式化は「$`M`$ の第 $`j`$ 項が存在すればその第 $`i`$ 成分、無ければ $`0`$」なので、
$`i\lt\textrm{Lng}(M)`$ の下で $`(M_{0,i},M_{1,i})=M_i`$ となり一致する。

**補助 2（反復 $`\textrm{Pred}`$ による経路）.**
任意の $`k\in\mathbb{N}`$ と $`M\in T_{\textrm{PS}}`$ に対し、$`k\lt\textrm{Lng}(M)`$ ならば
...
- $`k=0`$ のとき。$`M\!\restriction\!\textrm{Lng}(M)=M`$ である。添字列として $`a=()`$ をとれば、
  ...
  $`\leq_{\textrm{PS}[]}`$ の定義を満たす。$`M\in T_{\textrm{PS}}`$ も $`k\lt\textrm{Lng}(M)`$ も使わない。
- $`k+1`$ のとき。仮定 $`k+1\lt\textrm{Lng}(M)`$ から $`\textrm{Lng}(M)\gt 1`$。この下で
  1. $`\textrm{Lng}(\textrm{Pred}(M))=\textrm{Lng}(M)-1`$（$`\textrm{Pred}`$ は $`\textrm{Lng}(M)\gt 1`$ のとき末尾 1 項を落とす）。
  2. $`\textrm{Pred}(M)\in T_{\textrm{PS}}`$。実際 $`\textrm{Pred}(M)=()`$ とすると
     $`\textrm{Lng}(\textrm{Pred}(M))=0`$ だが、1 と $`k+1\lt\textrm{Lng}(M)`$ より
...
**(b) 訂正形の証明.**
$`M\in T_{\textrm{PS}}`$ から $`\textrm{Lng}(M)\gt 0`$（$`M`$ が空でないので場合分けで直ちに出る）。
...
補助 2 を $`k=\textrm{Lng}(M)-(j_1'+1)`$ で使う。適用条件 $`k\lt\textrm{Lng}(M)`$ は
$`\textrm{Lng}(M)\gt 0`$ から従う。得られる式の切片の長さは
$`\textrm{Lng}(M)-\bigl(\textrm{Lng}(M)-(j_1'+1)\bigr)=j_1'+1`$（$`j_1'+1\leq\textrm{Lng}(M)`$ による）なので、
そのまま目標と一致する。この $`k`$ が原文の $`j_1-j_1'`$ にあたる。

**(a) 逐語形の反証.**
まず補助として、$`\textrm{Lng}(M)\gt 1`$ ならば $`M\lt_{\textrm{PS}[]}M`$ は偽であることを示す。
$`M\lt_{\textrm{PS}[]}M`$ とすると、$`\textrm{Lng}(N)\gt 1`$ を補った形の命題（辞書式的順序が
基本列的順序を含意すること）、すなわち「$`\textrm{Lng}(N)\gt 1`$ かつ $`M\lt_{\textrm{PS}[]}N`$ ならば
$`M\lt_{\textrm{PS}}N`$」を $`N=M`$ に適用して $`M\lt_{\textrm{PS}}M`$ が出るが、
...
- $`\textrm{Lng}(M)=2\gt 1`$: 有限計算で確かめられる。

よって逐語形を $`M`$、$`j_1'=1`$ に適用すると $`M\lt_{\textrm{PS}[]}M`$ が出て、上の補助に矛盾する。

## 原文通りに書けなかった理由

- **[⚠️Y]** 逐語形の結論 $`\lt_{\textrm{PS}[]}`$ は偽であり、$`\leq_{\textrm{PS}[]}`$ が正しい

  仮定は $`j_1'\leq j_1`$ なので $`j_1'=j_1`$ が許される。このとき
  $`(M_j)_{j=0}^{j_1'}=M`$ なので、原文の結論は $`M\lt_{\textrm{PS}[]}M`$ を主張することになる。
  $`\lt_{\textrm{PS}[]}`$ は「$`a\neq()`$ なる $`a\in\mathbb{N}_+^{\lt\omega}`$ が存在して
  $`M=M[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$」であり、$`\textrm{Lng}(M)\gt 1`$ のときは
  ...
  訂正は $`\lt_{\textrm{PS}[]}`$ を $`\leq_{\textrm{PS}[]}`$ に置き換えるだけで、証明の筋は変わらない
  （帰納法の基底が「空の添字列」になる）。下流でこの補題が狭義の $`\lt_{\textrm{PS}[]}`$ で
  使われる箇所は、$`\leq_{\textrm{PS}[]}`$ の形と「$`\lt_{\textrm{PS}[]}`$ と $`\leq_{\textrm{PS}[]}`$ の合成は
  $`\lt_{\textrm{PS}[]}`$」で置き換えられる。狭義にしたい場合は仮定を $`j_1'\lt j_1`$ とすればよい。

- **[R]** 原文の仮定 $`M\in ST_{\textrm{PS}}`$ は不要で、$`M\in T_{\textrm{PS}}`$ で足りる

  証明で使うのは $`\textrm{Pred}(M)=M[1]`$（$`M\in T_{\textrm{PS}}`$ と $`\textrm{Lng}(M)\gt 1`$ のみを要する）と
...
- **[W]** 原文の「帰納法により」が隠している段が Lean では明示される
  ...
  原文は $`(M_j)_{j=0}^{j_1'}=\textrm{Pred}^{j_1-j_1'}(M)=M[1]^{j_1-j_1'}\lt_{\textrm{PS}[]}M`$ を
  ...
  第三に、各段の側条件 $`\textrm{Lng}(M)\gt 1`$ と $`\textrm{Pred}(M)\in T_{\textrm{PS}}`$ の確認。
  原文はこれらに触れないが、$`\textrm{Pred}(M)=M[1]`$ の適用に $`\textrm{Lng}(M)\gt 1`$ が要り、
  帰納法の仮定の適用に $`\textrm{Pred}(M)\neq()`$ と $`k\lt\textrm{Lng}(\textrm{Pred}(M))`$ が要る。
  なお Lean は $`\textrm{Pred}^{j_1-j_1'}(M)`$ という項自体を作らず、
  $`M[1]^{j_1-j_1'}\lt_{\textrm{PS}[]}M`$ を長さ $`j_1-j_1'`$ の添字列 $`(1,\dots,1)`$ で一括に作る代わりに、
...
| $`\textrm{Lng}(M)\gt 1\Rightarrow\lnot(M\lt_{\textrm{PS}[]}M)`$ | `ltExpPS_irrefl` | `lean/Bijectivity/11-path-to-initial-segment.lean` |
| 命題（辞書式的順序が基本列的順序を含意すること）の訂正形 | `ltExpPS_ltPS_of_lng` | `lean/Bijectivity/05-exp-implies-lex.lean` |
| $`\lt_{\textrm{PS}}`$ の非反射性 | `ltPS_irrefl` | `lean/Bijectivity/02-lex-linear.lean` |
