...
　$`\textrm{dom}(0)=0\neq\omega`$かつ任意の$`u\in\mathbb{N}`$に対して$`\textrm{dom}(D_u0)=\Omega_u\neq\omega`$であるから$`\textrm{Lng}(P(M)_{J_1})\gt 1`$である。  
...
　[1]の$`\textrm{Trans}`$の$`(\textrm{IncrFirst},\textrm{Red})`$不変$`P`$同変性(2)、また任意の$`t_0,t_1\in T_{\textrm{B}}^{\lt\omega}`$に対して明らかに$`\Sigma_{\textrm{B}}(t_0+t_1)=\Sigma_{\textrm{B}}t_0+\Sigma_{\textrm{B}}t_1`$であり、任意の$`n\in\mathbb{N}_+`$に対して$`P(M)_{J_1}`$及び$`P(M)_{J_1}[n]`$は零項ではないから、$`P(M)_{J_1}^+=\textrm{Trans}(P(M)_{J_1})`$かつ$`\textrm{Trans}(M)=\Sigma_{\textrm{B}}(P(M)_J^+)_{J=0}^{J_1}`$かつ任意の$`n\in\mathbb{N}_+`$に対して
...
```math
\text{（単項の場合）}\quad\forall M\in ST_{\textrm{PS}},\ M\text{ が単項},\ \textrm{Lng}(M)\gt 1,\ \textrm{dom}(\textrm{Trans}(M))=\omega\ \Rightarrow\ \exists n\in\mathbb{N}_+,\ \textrm{Trans}(M)[m]\leq_{\textrm{B}}\textrm{Trans}(M[n])
```
...
```math
\text{（条件 (II) の交換関係の逆向き）}\quad\forall M\in ST_{\textrm{PS}},\ M\text{ が単項},\ j_1\gt 1,\ \text{条件 (II)}\ \Rightarrow\ \exists n\in\mathbb{N}_+,\ \textrm{Trans}(M[n])=\textrm{Trans}(M)[m]
```
...
**A0. $`j_1\gt 0`$。** 原文の「$`\textrm{dom}(0)=0\neq\omega`$ かつ $`\textrm{dom}(D_u0)=\Omega_u\neq\omega`$ であるから $`j_1\gt 0`$」に
あたる補題は、$`M`$ が簡約で $`\textrm{dom}(\textrm{Trans}(M))=\omega`$ ならば $`\textrm{Lng}(M)\gt 1`$、という形で立てられている。
...
A0 より $`\textrm{Lng}(M)\gt 1`$。$`M`$ が単項なら B をそのまま適用して終わる。以下 $`M`$ は単項でないとする。
$`\textrm{Lng}(M)\gt 1`$ より $`M`$ は零項でもないので、零項・単項・複項の三分から $`M`$ は複項である。
...
4. $`\textrm{dom}(\textrm{Trans}(P(M)_{J_1}))=\omega`$ と $`\textrm{Lng}(P(M)_{J_1})\gt 1`$。前者は A2 の第 2 枝と 3 に
   $`\textrm{dom}(t+u)=\textrm{dom}(u)`$ を使う。後者は A0 の補題を $`P(M)_{J_1}`$ に適用したもので、
   原文の「$`\textrm{dom}(0)=0\neq\omega`$ …であるから $`\textrm{Lng}(P(M)_{J_1})\gt 1`$」にあたる。
...
$`\textrm{Lng}(P(M)_{J_1})\gt 1`$ の下で使う。得られるのは 2 つの等式
...
（$`D_vb\lt_{\textrm{B}}D_00`$ は $`v\lt 0`$ か「$`v=0`$ かつ $`b\lt_{\textrm{B}}0`$」を要するがどちらも不可能）なので、
$`q`$ の全項が $`D_00`$ である。したがって principal 列の辞書式順序で $`q`$ と $`(D_00)\frown q`$ を比べると、
$`q`$ が空なら空列 $`\lt`$ 非空列、$`q=(D_00)\frown q'`$ なら先頭が等しく残りが帰納法の仮定で小さいので、
...
$`M\in ST_{\textrm{PS}}`$、$`M`$ は単項、$`\textrm{Lng}(M)\gt 1`$、$`\textrm{dom}(\textrm{Trans}(M))=\omega`$ とする。
...
「条件 (II) か条件 (IV)」（$`M`$ が簡約・単項・$`\textrm{Lng}(M)\gt 1`$ の下で、
...
$`j_1=1`$ の段 $`0`$ の親は $`0`$、そこから $`M_{0,1}\gt M_{0,0}\geq0`$ なので $`(M_{0,1},M_{1,1})\neq(0,0)`$。
...
```math
\textrm{Trans}(((u,u))^{k+1})=\begin{cases}(D_u0)\times k&(u=0)\cr  (D_u0)\times(k+1)&(u\gt 0)\end{cases}
```

なので、$`u=0`$ のときは $`n=m+2`$、$`u\gt 0`$ のときは $`n=m+1`$ を取れば等号で
...
$`j_1\gt 1`$ のとき。[1] の条件 (I) の下での $`\textrm{Trans}`$ と基本列の交換関係 (1)
（$`M`$ が簡約・単項、$`j_1\gt 1`$、条件 (I)、$`n\geq1`$ で $`\textrm{Trans}(M[n])=\textrm{Trans}(M)[n-1]`$）に
$`n=m+1`$ を入れて等号を得る。

**B2. 条件 (II)（$`M_{1,j_1}=0`$ かつ $`j_0`$ が $`M`$ 許容でない）。**

まず $`j_1\gt 1`$ である。$`j_1=1`$ なら段 $`0`$ の親は $`j_0=0`$ だが、$`0`$ はつねに $`M`$ 許容
...
**B3. 条件 (III)/(IV)（$`0\lt M_{1,j_1}\leq M_{1,j_0}`$）。**

段 $`1`$ で $`j_1`$ が親を持つかで分ける。持たないときは、[1] の §8.4 の親無し枝の評価
（$`M`$ 簡約・単項、$`j_1\gt 0`$、$`M_{1,j_1}\gt 0`$、段 $`1`$ の親無しならば
$`\textrm{dom}(\textrm{Trans}(M))=T_{M_{1,j_1}-1}`$）より $`\textrm{dom}(\textrm{Trans}(M))\neq\omega`$ となり仮定に反する。

持つときは $`j_1\gt 1`$ である。$`j_1=1`$ とすると段 $`1`$ の親も $`0`$ しかなく、親子関係から
$`M_{1,0}\lt M_{1,1}`$ が出るが、条件 (III)/(IV) は $`M_{1,1}\leq M_{1,j_0}=M_{1,0}`$ を言うので矛盾する。
これで [1] の条件 (III) か (IV) の下での交換関係 (3)（$`n\geq1`$ で
$`\textrm{Trans}(M)[n-1]\lt_{\textrm{B}}\textrm{Trans}(M[n+1])`$）が使え、$`n=m+1`$ として

```math
\textrm{Trans}(M)[m]\lt_{\textrm{B}}\textrm{Trans}(M[m+2])
```

を得る。取る添字は $`m+2`$。

**B4. 条件 (V)（$`M_{1,j_0}+1=M_{1,j_1}`$ かつ $`j_0+1\lt j_1`$）。**

条件そのものから $`j_1\gt 1`$ である。

$`j_0`$ が $`M`$ 許容のとき。[1] の条件 (V) の下での交換関係 (3) の許容枝の第 4 結論
（$`n\geq1`$ で $`\textrm{Trans}(M)[n-1]\lt_{\textrm{B}}\textrm{Trans}(M[n+1])`$）に $`n=m+1`$ を入れて、添字 $`m+2`$ を取る。
...
[1] の条件 (V) の下での $`t_2`$ の非零性より $`t_2\neq0`$、したがって $`0\lt_{\textrm{B}}t_2`$。
塔 $`W_c`$ は種 $`c`$ について狭義単調（$`k`$ についての帰納法。$`k=0`$ は $`D_e`$ の中身の比較、
$`k+1`$ は $`+`$ の右単調性）なので $`W_0(j+1)\lt_{\textrm{B}}W_{t_2}(j+1)`$、$`+`$ の右単調性で
$`\textrm{bodyO}(j+1)\lt_{\textrm{B}}\textrm{bodyM}(j+1)`$。同じ $`s_1,b_1`$ の中で principal 芯だけを
取り替えた 2 項の比較（部分表現の不等式の延長性）により

```math
\textrm{Trans}(M)[j+1]\lt_{\textrm{B}}\textrm{Trans}(M[j+2])\qquad(j\geq0).
```
...
```math
\textrm{Trans}(M)[0]\leq_{\textrm{B}}\textrm{Trans}(M)[1]\lt_{\textrm{B}}\textrm{Trans}(M[2])
```
...
$`M_{1,1}\gt 0`$ なので $`\textrm{idx}_1(M,1)=1`$ であり、段 $`1`$ で $`1`$ が親を持つ
...
（訂正 A23 後の読み）である。$`b=D_{u+1}0`$ は $`u+1\gt 0`$ なので $`b[y]=y`$、したがって
...
$`j_1\gt 1`$ のとき。[1] の条件 (VI) の下での交換関係 (2)
...
$`a\in OT_{\textrm{B}}`$ かつ $`\textrm{dom}(a)=\omega`$ ならば $`a[m]\lt_{\textrm{B}}a[m+1]`$、したがって
...
- **[⚠️Y]** 複項の場合の末尾の等式連鎖は偽で、$`\leq_{\textrm{B}}`$ かつ添字は $`m`$ ではない（訂正 Y-10）

  原文の最後の連鎖は
  $`\Sigma_{\textrm{B}}(P(M)_J^+)_{J=0}^{J_1-1}+(\textrm{Trans}(P(M)_{J_1})[m]) =\Sigma_{\textrm{B}}(P(M)_J^+)_{J=0}^{J_1-1}+\textrm{Trans}(P(M)_{J_1}[m])`$
  ...
  であって、$`n=m`$ とは限らず等号でもない。実際、条件 (III)/(IV)/(V) では狭義の $`\lt_{\textrm{B}}`$ しか出ない。
...
- **[W]** 2 列（$`j_1=1`$）で $`M_0\neq(0,0)`$ の場合が原文のどちらの枝にも入っていない
  ...
  一方 [1] の 5 つの交換関係はいずれも $`j_1\gt 1`$ を仮定する
  （条件 (V) は $`j_0+1\lt j_1`$ から $`j_1\gt 1`$ が自動、他の 4 つは仮定として置かれている）。
...
- **[W]** [1] の交換関係を引くための側条件を原文が確かめていない
  ...
  場合分けの網羅にも一歩要る。
  第一に条件 (I)、(II)、(III)/(IV)、(VI) の交換関係はいずれも $`j_1\gt 1`$ を要求する。
  ...
  それは $`j_1\gt 0`$ から出る。Lean は A1 でこの一歩を明示している。

- **[W]** 条件 (V) の非許容枝では $`m=0`$ が [1] の交換関係で覆えない（訂正 W-34）
  ...
  $`\textrm{Trans}(M)[0]\leq_{\textrm{B}}\textrm{Trans}(M)[1]\lt_{\textrm{B}}\textrm{Trans}(M[2])`$ とする（B4）。
...
| $`a[m]\lt_{\textrm{B}}a[m+1]`$ | `operB_numBT_step` | `lean/Bijectivity/16c-operB-mono.lean` |
...
| $`\lt_{\textrm{B}}`$、$`\leq_{\textrm{B}}`$ | `PSS.lessBT`、`PSS.leBT` | 同上 |
...
| A0（$`\textrm{dom}=\omega`$ ならば $`\textrm{Lng}\gt 1`$） | `Bijectivity.one_lt_lng_of_domIsOmega` | `lean/Bijectivity/15-successor-fseq.lean` |
...
| D の中身（$`a[m]\lt_{\textrm{B}}a[m+1]`$ と広義版） | `PSS.y4_N_mono`、`PSS.y4_N_mono_le` | `lean/OTB-well-founded-syntactic/OTB-well-founded-syntactic-cofinality.lean` |
