...
$T_{\textrm{B}}$ の項は主項（$D_va$ の形のもの）の有限列として表す。空列が $0$、長さ 1 の列 $(D_va)$ が $D_va$、和 $t+u$ は列の連結である。順序 $<_{\textrm{B}}$ は主項列の辞書式順序で、主項どうしは $D_ua<_{\textrm{B}}D_vb\iff u<v\lor(u=v\land a<_{\textrm{B}}b)$、列としては空列が最小である。$P_{\textrm{B}}(t)$ は $t$ の主項成分の列である。
...
**補題D**（$D_00$ の下は $0$ だけ）: $t<_{\textrm{B}}D_00$ ならば $t=0$。$t$ の主項列が空なら $t=0$。空でなく先頭が $D_wc$ だとすると、列の辞書式比較より $D_wc<_{\textrm{B}}D_00$ か、$D_wc=D_00$ かつ残りの列 $<_{\textrm{B}}()$ である。前者は $w<0$ か（$w=0$ かつ $c<_{\textrm{B}}0$）だが、$w<0$ は偽、$c<_{\textrm{B}}0$ も（空列が最小なので）偽。後者の「残りの列 $<_{\textrm{B}}$ 空列」も偽。よって矛盾。
...
**補題E**（単項で 2 列以上の場合）: $M\in RT_{\textrm{PS}}$、$M$ が単項、$\textrm{Lng}(M)>1$ ならば $\textrm{dom}(\textrm{Trans}(M))\neq1$。
...
- $\textrm{Trans}(M)\neq0$ とする。[1] の $\textrm{Trans}$ が単項性を保つこと（簡約単項列の非零な $\textrm{Trans}$ は単一の主項）より $\textrm{Trans}(M)=D_vb$ と書ける。$\textrm{dom}(\textrm{Trans}(M))=\lbrace 0\rbrace$ を仮定すると補題A から $v=0$、$b=0$、すなわち $\textrm{Trans}(M)=D_00$ である。ここで $\textrm{Trans}(\textrm{Pred}(M))$ で場合分けする。
  ...
  - $\textrm{Trans}(\textrm{Pred}(M))\neq0$ のとき。[1] の $\textrm{Pred}$ による $\textrm{Trans}$ の狭義降下（簡約で $\textrm{Lng}(M)>1$ なら $\textrm{Trans}(\textrm{Pred}(M))<_{\textrm{B}}\textrm{Trans}(M)$）より $\textrm{Trans}(\textrm{Pred}(M))<_{\textrm{B}}D_00$、補題D より $\textrm{Trans}(\textrm{Pred}(M))=0$ となり矛盾。

**補題F**（簡約な零項）: $N\in RT_{\textrm{PS}}$ が零項ならば $N=((0,0))$。$\textrm{Red}(N)=((0,0))$ と $\textrm{Red}(N)=N$ から出る。

**補題G**（非複項の $P$）: $N\in T_{\textrm{PS}}$ が複項でないならば $P(N)=(N)$。[1] の「複項 $\iff\textrm{Lng}(P(N))>1$」より $\textrm{Lng}(P(N))\leq1$、$P(N)\neq()$ と合わせて $\textrm{Lng}(P(N))=1$、$P(N)=(X)$ と書ける。$P$ の成分の連結が $N$ に戻ることから $X=N$。

**主補題H**（前半の本体）: $M\in RT_{\textrm{PS}}$ かつ $\textrm{dom}(\textrm{Trans}(M))=1$ ならば $1<\textrm{Lng}(M)$ かつ $M_{0,\textrm{Lng}(M)-1}=M_{1,\textrm{Lng}(M)-1}=0$。

1. $M\in T_{\textrm{PS}}$ より $\textrm{Lng}(M)\geq1$。補題C より $\textrm{Lng}(M)=1$ は仮定に反するので $1<\textrm{Lng}(M)$。
2. $\textrm{Lng}(M)\neq1$ なので $M$ は零項でない。補題E より $M$ は単項でない。よって $M$ は複項である。
3. [1] の複項列の $P$ の末尾成分（$M$ 複項、$\textrm{Lng}(M)>1$）より、$P(M)$ の末尾成分は $N=(M_j)_{j=\textrm{Pcut}(M)}^{\textrm{Lng}(M)-1}$、末尾を除いた部分は $P(A)$（$A=(M_j)_{j=0}^{\textrm{Pcut}(M)-1}$）である。空でない列は「末尾を除いた部分」と「末尾」に分かれるから $P(M)=P(A)\oplus(N)$ であり、$M=A\oplus N$ である。
...
6. $N\neq((0,0))$ の場合（矛盾を導く）。
   ...
   - $N$ は零項でない非複項なので単項である。$\textrm{Lng}(N)>1$ なら補題E に、$\textrm{Lng}(N)=1$ なら補題C に矛盾する。

#### 後半

**補題I**（最終列が $(0,0)$ なら基本列は $\textrm{Pred}$）: $\textrm{Lng}(M)>1$ かつ $M_{0,j_1}=M_{1,j_1}=0$（$j_1=\textrm{Lng}(M)-1$）ならば、任意の $n$ に対し $M[n]=\textrm{Pred}(M)$。$M[n]$ の定義は $j_1=0$ / $M_{j_1}=(0,0)$ / 親が無い / それ以外の 4 分岐で、いまは $j_1\neq0$ かつ $M_{j_1}=(0,0)$ なので第 2 分岐に落ち、値は $n$ に依らず $\textrm{Pred}(M)$ である。

**主補題J**（後半の本体）: $M\in RT_{\textrm{PS}}$、$\textrm{Lng}(M)>1$、$M_{0,j_1}=M_{1,j_1}=0$ ならば $\textrm{Trans}(M[n])+D_00=\textrm{Trans}(M)$。
...
3. [1] の $P$ の加法性を $j_0=j_1$ で適用する。適用条件は $0<j_1$、$j_1\leq\textrm{Lng}(M)-1$、および「$\forall j<j_1$ に対し $M_{0,j_1}\leq M_{0,j}$」の 3 つで、前 2 つは $\textrm{Lng}(M)>1$ から、最後は $M_{0,j_1}=0$ から従う。結論は
...
$\textrm{dom}(\textrm{Trans}(M))=1$ から主補題H で $1<\textrm{Lng}(M)$ と $M_{j_1}=(0,0)$ を得、主補題J で第 2 の選言肢 $\textrm{Trans}(M[n])+D_00=\textrm{Trans}(M)$ を得る。第 1 の選言肢は選ばれない。

なお同じファイルには「$M\in RT_{\textrm{PS}}$ かつ $\textrm{dom}(\textrm{Trans}(M))=\omega$ ならば $1<\textrm{Lng}(M)$」も証明されているが、これは原文のこの命題ではなく後続の命題（基本列の関係・基本列の収束）で使うためのものである。証明は補題C と同じ 1 列の場合分けで、$M=((0,0))$ なら $\textrm{dom}(0)=\varnothing$、$M=(p)$、$p_1\neq0$ なら $\textrm{dom}(D_{p_1}0)=T_{p_1-1}$ で、どちらも $\mathbb{N}$ でない、というものである。
...
- **[W]** 原文は $P$ の加法性の適用条件を確認していない
  ...
  [1] の $P$ の加法性は分割点 $j_0$ について $0<j_0$、$j_0\leq\textrm{Lng}(M)-1$、$\forall j<j_0\ (M_{0,j_0}\leq M_{0,j})$ の 3 条件を要求する。原文は「[1] の $P$ の加法性より $P(M)=P(\textrm{Pred}(M))\oplus_{T_{\textrm{PS}}}(((0,0)))$ である」と書くだけで、$j_0$ が何かも条件の確認も書かない。Lean は $j_0=\textrm{Lng}(M)-1$ を取り、前 2 条件を $\textrm{Lng}(M)>1$ から、第 3 条件を $M_{0,j_0}=0$ から確認したうえで加法性を適用し、得られた 2 つの切片をそれぞれ $\textrm{Pred}(M)$ と $((0,0))$ に書き換える。
...
| $T_{\textrm{B}}$ の項、$0$、$D_va$、$<_{\textrm{B}}$、$P_{\textrm{B}}$ | `PSS.BT`, `PSS.BP`, `PSS.BZero`, `PSS.Dprin`, `PSS.lessBT`, `PSS.lessBP`, `PSS.lessBPList`, `PSS.PB` | `lean/Buchholz-1986/Buchholz-1986-2.1.lean` |
...
| $\textrm{dom}=\omega$ ならば $1<\textrm{Lng}(M)$ | `one_lt_lng_of_domIsOmega` | 同上 |
...
| [1] $P$ の各成分の非複項性、複項 $\iff\textrm{Lng}(P(M))>1$ | `P_components_nonmulti`, `P_components_multi_iff` | `lean/6/6.2-P-components-nonmulti.lean` |
