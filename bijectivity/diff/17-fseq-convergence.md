...
* $`\{(a,b)\mid a,b\in OT_{\textrm{B}}\land a\lt_{\textrm{B}}b\}`$ は整礎（[Buc1] 2.2 にあたる
  事実を、リポジトリ本体が仮定ゼロで証明している）
* $`\lt_{\textrm{B}}`$ は $`OT_{\textrm{B}}`$ 上で三分律と推移律を満たす

が成り立つので、$`(OT_{\textrm{B}},\lt_{\textrm{B}})`$ は整列順序である。そこで $`o`$ を
その順序型への順序同型、すなわち $`t\in OT_{\textrm{B}}`$ に
$`\{u\in OT_{\textrm{B}}\mid u\lt_{\textrm{B}}t\}`$ の順序型を対応させる写像として定める。
$`OT_{\textrm{B}}`$ の外では $`o`$ の値を $`0`$ と置く（例外は $`D_0D_\omega0`$ の一点で、
そこだけ $`\{t\in OT_{\textrm{B}}\mid t\lt_{\textrm{B}}D_0D_\omega0\}`$ の順序型を値とする。
...
$`s\lt_{\textrm{B}}t`$ ならば $`o(s)\lt o(t)`$。順序型への同型が順序を保つことそのもの。

**(o2) 初期切片への全射性（[Buc1] Lemma 2.2(c) にあたる）.** $`t_0\in OT_{\textrm{B}}`$、
$`\alpha\lt o(t_0)`$ ならば、ある $`t\in OT_{\textrm{B}}`$ が存在して $`t\lt_{\textrm{B}}t_0`$ かつ
$`o(t)=\alpha`$。$`\alpha\lt o(t_0)\lt`$（$`(OT_{\textrm{B}},\lt_{\textrm{B}})`$ の順序型）なので
$`\alpha`$ は順序型未満、よって $`o(t)=\alpha`$ なる $`t\in OT_{\textrm{B}}`$ が取れ、
$`o(t)=\alpha\lt o(t_0)`$ と (o1) の逆向き（同型なので同値）から $`t\lt_{\textrm{B}}t_0`$。

**(o3) 初期切片の上限表示.** $`t\in OT_{\textrm{B}}`$ ならば
```math
o(t)=\sup\{\,o(u)+1\ \mid\ u\in OT_{\textrm{B}},\ u\lt_{\textrm{B}}t\,\}.
```
「$`\leq`$」: 右辺 $`\lt o(t)`$ と仮定すると、(o2) を $`\alpha=`$ 右辺 に使って
$`w\in OT_{\textrm{B}}`$、$`w\lt_{\textrm{B}}t`$、$`o(w)=`$ 右辺 が取れる。しかし上限の定義から
$`o(w)+1\leq`$ 右辺 $`=o(w)`$ で矛盾。
「$`\geq`$」: 各 $`u\in OT_{\textrm{B}}`$、$`u\lt_{\textrm{B}}t`$ に (o1) で $`o(u)\lt o(t)`$、
...
$`a\lt_{\textrm{B}}b`$ または $`a=b`$ の定義なので、前者は (o1)、後者は等号。
...
「$`\leq`$」: 各 $`m`$ に [Buc1] Lemma 3.2(a) の数項形 $`t[m]\lt_{\textrm{B}}t`$
（適用に $`t\in OT_{\textrm{B}}`$ と $`t\neq0`$ が要る）と (o1) を使って
$`o(t[m])\lt o(t)`$、したがって $`\sup_m o(t[m])\leq o(t)`$。

「$`\geq`$」: (o3) で $`o(t)`$ を $`\sup\{o(u)+1\mid u\in OT_{\textrm{B}},u\lt_{\textrm{B}}t\}`$ に
書き換える。上限の各項を評価すればよいので、$`u\in OT_{\textrm{B}}`$、$`u\lt_{\textrm{B}}t`$ を
...
3. 基本列の添字単調性 $`t[m]\lt_{\textrm{B}}t[m+1]`$（$`t\in OT_{\textrm{B}}`$ と
   $`\textrm{dom}(t)=\omega`$ を要する）と (o1) より $`o(t[m])\lt o(t[m+1])`$。
4. $`o(t[m+1])\leq\sup_n o(t[n])`$。
5. 2, 3, 4 をつないで $`o(u)\lt\sup_n o(t[n])`$、順序数なので
   $`o(u)+1\leq\sup_n o(t[n])`$。

上限を取って $`o(t)\leq\sup_n o(t[n])`$。両向きから等号。

この節で $`t[m]\lt_{\textrm{B}}t[m+1]`$ を挟むのは、共終性が $`\leq_{\textrm{B}}`$ しか
...
2. $`\textrm{Lng}(M)\gt 1`$。$`\textrm{Lng}(M)=1`$ の簡約形 $`M=((a,b))`$ を仮定して矛盾を出す。
...
```math
\textrm{Trans}(M[n])\lt_{\textrm{B}}\textrm{Trans}(M)
```
（適用に $`M\in ST_{\textrm{PS}}`$、$`n\geq1`$、$`\textrm{Lng}(M)\gt 1`$ が要る）を使い、
4 と 3 の下で (o1) を当てて $`o(\textrm{Trans}(M[n]))\lt o(\textrm{Trans}(M))`$。
...
- **[W]** 原文が [4] の引用で済ませる $`o`$ の性質を、こちらは定理として証明している
  ...
  下流（項目 21 以降）も同じ $`o`$ で一貫している。構成した $`\psi_0\psi_\omega0`$ が
  Buchholz の $`\psi_0(\psi_\omega(0))`$ と一致することを述べていない点は、
  それが主張の内容に効く項目 21 のほうで別に挙げてある。
...
- **[S]** [5] の Theorem 1.4(a) と Lemma 1.6 を引用せず、当たる主張を証明している
  ...
  $`o(t)=\sup_{u\lt_{\textrm{B}}t}(o(u)+1)`$ の右辺の後続を越えるために
  $`t[m]\lt_{\textrm{B}}t[m+1]`$ を 1 段挟む必要がある。原文は「非有界」の一語で通しており、
  この段には触れていない。

- **[W]** 原文が触れない側条件 $`\textrm{Lng}(M)\gt 1`$ を仮定から導いている

  [1] の基本列の降下性 $`\textrm{Trans}(M[n])\lt_{\textrm{B}}\textrm{Trans}(M)`$ は
  $`\textrm{Lng}(M)\gt 1`$ を要する。実際 $`\textrm{Lng}(M)=1`$ なら $`M[n]=M`$ で降下しない。
  原文は「[1] の基本列の降下性より」と書くだけでこの条件に触れないが、この形式化では
  $`\textrm{dom}(\textrm{Trans}(M))=\omega`$ から $`\textrm{Lng}(M)\gt 1`$ を導く段
...
- **[W]** $`o`$ の単調性が $`OT_{\textrm{B}}`$ 上でしか成り立たないので、$`OT_{\textrm{B}}`$ 所属を
  ...
  $`\lt_{\textrm{B}}`$ は Buchholz 項全体では整礎でない（$`D_10`$、$`D_0D_10`$、$`D_0D_0D_10`$、… が
...
| $`a\lt_{\textrm{B}}b`$ | `PSS.lessBT` | `lean/Buchholz-1986/Buchholz-1986-2.1.lean` |
| $`a\leq_{\textrm{B}}b`$ | `PSS.leBT` | 同上 |
| $`\lt_{\textrm{B}}`$ の三分律・推移律 | `PSS.lessBT_linear_trichotomy`, `PSS.lessBT_linear_trans` | `lean/Buchholz-1986/Buchholz-1986-2.1-order.lean` |
| $`OT_{\textrm{B}}`$ | `PSS.OT_B` | `lean/Buchholz-1986/Buchholz-1986-2.2.lean` |
| $`OT_{\textrm{B}}`$ 上の $`\lt_{\textrm{B}}`$ の整礎性 | `PSS.OT_B_wellFounded` | `lean/OTB-well-founded-syntactic/OTB-well-founded-syntactic-main.lean` |
...
| $`(OT_{\textrm{B}},\lt_{\textrm{B}})`$ | `Bijectivity.OTBsub`, `Bijectivity.rOTB` | 同上 |
...
| $`\lt_{\textrm{B}}`$ が項全体では整礎でない証拠 | `Bijectivity.descChain_lt`, `Bijectivity.descChain_not_OT` | `lean/Bijectivity/Cited.lean` |
| [Buc1] Lemma 3.2(a) の数項形 $`t[m]\lt_{\textrm{B}}t`$ | `PSS.buchholz_fseq_lt` | `lean/Buchholz-1986/Buchholz-1986-3.2-descent.lean` |
| [Buc1] Lemma 3.3 の数項形 $`t[m]\in OT_{\textrm{B}}`$ | `PSS.buchholz_fseq_closed` | `lean/Buchholz-1986/Buchholz-1986-3.3.lean` |
| 添字単調性 $`t[m]\lt_{\textrm{B}}t[m+1]`$ | `Bijectivity.operB_numBT_step`（`PSS.y4_N_mono`） | `lean/Bijectivity/16c-operB-mono.lean` |
...
| $`\textrm{dom}(\textrm{Trans}(M))=\omega\Rightarrow\textrm{Lng}(M)\gt 1`$ | `Bijectivity.one_lt_lng_of_domIsOmega` | `lean/Bijectivity/15-successor-fseq.lean` |
