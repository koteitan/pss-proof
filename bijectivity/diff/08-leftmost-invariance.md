...
&nbsp;&nbsp;$`\leq_{\textrm{PS}[]}`$の定義よりある$`a\in\mathbb{N}_+^{\lt\omega}`$が存在して$`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$である。  
&nbsp;&nbsp;任意の非負整数$`i\lt\textrm{Lng}(a)`$に対して$`Q_0=N`$、$`Q_{i+1}=Q_i[a_i]`$とする。  
&nbsp;&nbsp;$`(Q_0)_0=N_0`$である。  
&nbsp;&nbsp;任意の非負整数$`k\lt\textrm{Lng}(a)`$を取り、$`(Q_k)_0=N_0`$であると仮定する。  
&nbsp;&nbsp;&nbsp;&nbsp;$`\textrm{Lng}(Q_k)=1`$ならば$`(Q_{k+1})_0=(Q_k)_0=N_0`$である。  
&nbsp;&nbsp;&nbsp;&nbsp;$`\textrm{Lng}(Q_k)\gt 1`$ならば展開と$`\textrm{Pred}`$の関係より$`(Q_{k+1})_0=\textrm{Pred}(Q_k)_0=N_0`$である。  
...
ペア数列は対 $`(\mathbb{N}\times\mathbb{N})`$ の有限列として表されている。最左列 $`M_0`$ は既定値つきの先頭要素 $`\textrm{head}(M)`$、すなわち $`\textrm{Lng}(M)\gt 0`$ のとき $`M_0`$、$`M=()`$ のとき $`(0,0)`$ となるもの、で表す。$`T_{\textrm{PS}}`$ の元に対してはこれは原文の $`M_0`$ そのものである。示されている命題は
...
```math
M\leq_{\textrm{PS}[]}N\ :\Longleftrightarrow\ \exists a\in\mathbb{N}_+^{\lt\omega},\ M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]
```
...
で与えられている（$`\mathbb{N}_+^{\lt\omega}`$ への所属は「$`a`$ の各成分が $`1`$ 以上」という条件として持たれる）。
...
- $`a=(n)\oplus a'`$ のとき。定義より $`\textrm{expand}(N,(n)\oplus a')=\textrm{expand}(N[n],a')`$ である。$`a'`$ の各成分は $`(n)\oplus a'`$ の成分でもあるから $`1`$ 以上であり、帰納法の仮定を $`N`$ ではなく $`N[n]`$ に対して適用して
  ...
  - $`1\lt\textrm{Lng}(N)`$ のとき。$`n\in(n)\oplus a'`$ から $`n\geq1`$ を得て、下の (2) の一歩補題をそのまま適用する。
...
**(2) 一歩補題（展開は最左列を保つ）。** $`1\lt\textrm{Lng}(M)`$ かつ $`n\geq1`$ ならば $`\textrm{head}(M[n])=\textrm{head}(M)`$。原文が「展開と $`\textrm{Pred}`$ の関係より $`(Q_{k+1})_0=\textrm{Pred}(Q_k)_0=N_0`$」と一行で書いた箇所にあたる。$`j_1=\textrm{Lng}(M)-1`$ と置くと $`1\lt\textrm{Lng}(M)`$ より $`j_1\gt 0`$ であり、次の 4 つを繋ぐ。

- 命題（展開と $`\textrm{Pred}`$ の関係）：$`1\lt\textrm{Lng}(M)`$ かつ $`n\geq1`$ のとき $`\textrm{Lng}(M[n])\geq j_1`$ かつ $`(M[n]_j)_{j=0}^{j_1-1}=\textrm{Pred}(M)`$。ここで使うのは後半の切片の等式である。

- 長さ $`k\gt 0`$ の先頭切片は先頭要素を変えない：$`j_1\gt 0`$ より $`\textrm{head}\big((M[n]_j)_{j=0}^{j_1-1}\big)=\textrm{head}(M[n])`$。

- $`\textrm{Pred}`$ の定義は $`\textrm{Lng}(M)\leq1`$ のとき恒等、そうでないとき末尾 1 項の除去なので、$`1\lt\textrm{Lng}(M)`$ から $`\textrm{Pred}(M)`$ は $`M`$ の末尾 1 項を落とした列である。
...
- **[R]** $`M,N\in T_{\textrm{PS}}`$ の仮定が要らない
  ...
  原文は $`M,N\in T_{\textrm{PS}}`$（空列でない）を仮定し、それに合わせて場合分けを $`\textrm{Lng}(Q_k)=1`$ と $`\textrm{Lng}(Q_k)\gt 1`$ の 2 つで尽くしている。Lean 側は所属の仮定を置かず、空列を含む任意のペア数列に対して述べている。場合分けは $`\textrm{Lng}(N)\leq1`$ と $`1\lt\textrm{Lng}(N)`$ で、前者には原文が排除していた $`\textrm{Lng}(N)=0`$ も入るが、$`j_1=\textrm{Lng}(N)-1`$ が自然数の切り捨て減算で $`0`$ になるため $`\textrm{operator}[]`$ の第 1 分岐が働いて $`N[n]=N`$ となり、原文の $`\textrm{Lng}(Q_k)=1`$ の場合とまったく同じ議論で閉じる。最左列は既定値 $`(0,0)`$ つきの先頭要素で表しており、$`T_{\textrm{PS}}`$ の元に対しては原文の $`M_0`$ と一致するので、原文の主張はこの命題の特別な場合として得られる。
...
- **[W]** 原文の「$`(Q_{k+1})_0=\textrm{Pred}(Q_k)_0=N_0`$」は 3 段階を隠している
  ...
  原文は $`\textrm{Lng}(Q_k)\gt 1`$ の場合を「展開と $`\textrm{Pred}`$ の関係より」の一言で済ませるが、その命題が与えるのは $`Q_k[a_k]`$ の先頭 $`\textrm{Lng}(Q_k)-1`$ 項が $`\textrm{Pred}(Q_k)`$ に等しいという切片の等式であって、先頭要素の等式ではない。Lean 側は、(i) 切片の長さ $`\textrm{Lng}(Q_k)-1`$ が正であること（$`\textrm{Lng}(Q_k)\gt 1`$ から従う）を使って「正の長さの先頭切片は先頭要素を変えない」を挟み、(ii) $`\textrm{Lng}(Q_k)\gt 1`$ のとき $`\textrm{Pred}(Q_k)`$ が末尾 1 項の除去であることを $`\textrm{Pred}`$ の定義から取り出し、(iii) 長さ $`2`$ 以上の列では末尾 1 項の除去が先頭要素を変えないこと（原文の 2 つ目の等号 $`\textrm{Pred}(Q_k)_0=(Q_k)_0`$ にあたり、原文は根拠を挙げていない）を別補題として置いている。いずれも列に関する初等的な補題で、規模は小さい。
