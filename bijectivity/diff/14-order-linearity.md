...
$`\leq_{\textrm{PS}[]}`$ の定義より、ある $`a\in\mathbb{N}_+^{\lt\omega}`$ が存在して $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$ である。
...
系 (辞書式的順序の線形性): $`\lt_{\textrm{PS}}`$ は狭義全順序、$`\leq_{\textrm{PS}}`$ は全順序である。
...
と定める（原文の $`a\in\mathbb{N}_+^{\lt\omega}`$ と $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$ に対応する）。$`\leq_{\textrm{PS}}`$ は原文どおり $`M=N\lor M\lt_{\textrm{PS}}N`$ の略記である。
...
- 非反射性: $`\lnot\,(M\lt_{\textrm{PS}}M)`$
- $`\lt_{\textrm{PS}}`$ の推移性: $`M\lt_{\textrm{PS}}N`$ かつ $`N\lt_{\textrm{PS}}O`$ ならば $`M\lt_{\textrm{PS}}O`$
- 三分律: $`M\lt_{\textrm{PS}}N\ \lor\ M=N\ \lor\ N\lt_{\textrm{PS}}M`$
...
$`CT_{\textrm{PS}}`$ の仮定を捨て、全ペア列上の $`\leq_{\textrm{PS}}`$ の反対称性を仮定 $`M\leq_{\textrm{PS}}N`$、$`N\leq_{\textrm{PS}}M`$ に適用する。中身は、$`M=N`$ の枝は結論そのもの、$`M\lt_{\textrm{PS}}N`$ かつ $`N\lt_{\textrm{PS}}M`$ の枝は $`\lt_{\textrm{PS}}`$ の推移性で $`M\lt_{\textrm{PS}}M`$ を得て非反射性に反する、というものである。

#### 3. $`\leq_{\textrm{PS}}`$ の推移性

$`CT_{\textrm{PS}}`$ の仮定を捨て、全ペア列上の $`\leq_{\textrm{PS}}`$ の推移性を適用する。中身は 2 つの仮定を $`=`$ と $`\lt_{\textrm{PS}}`$ に分解し、等号の枝はもう一方の仮定をそのまま返し、$`\lt_{\textrm{PS}}`$ が 2 つ揃った枝で $`\lt_{\textrm{PS}}`$ の推移性を使うというものである。
...
| $`\lt_{\textrm{PS}}`$ の非反射性・推移性・三分律 | `ltPS_irrefl`, `ltPS_trans`, `ltPS_trichotomy` | 同上 |
