...
任意の $`M,N\in CT_{\textrm{PS}}`$ に対して、$`M\lt_{\textrm{PS}}N`$ ならば $`\textrm{Trans}(M)\lt_{\textrm{B}}\textrm{Trans}(N)`$ である。

### 証明

基本列的順序が辞書式的順序を含意することより $`M\lt_{\textrm{PS}[]}N`$ である。

$`\lt_{\textrm{PS}[]}`$ の定義よりある $`a\in\mathbb{N}_+^{\lt\omega}\setminus\lbrace()\rbrace`$ が存在して $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$ である。

任意の $`((0,0))`$ は $`\lt_{\textrm{PS}}`$ に対して $`T_{\textrm{PS}}`$ の最小の要素であるから、$`N\neq(0,0)`$ である。

上より $`\textrm{Lng}(N)\gt 1`$ である。

任意の非負整数 $`i\lt\textrm{Lng}(a)`$ に対して $`Q_0=N`$、$`Q_{i+1}=Q_i[a_i]`$ とする。

[1]の基本列の降下性より $`\textrm{Trans}(Q_1)=\textrm{Trans}(N[a_0])\lt_{\textrm{B}}\textrm{Trans}(N)`$ である。

任意の非負整数 $`1\leq i\lt\textrm{Lng}(a)`$ をとり、$`\textrm{Trans}(Q_i)\lt_{\textrm{B}}\textrm{Trans}(N)`$ とすると、[1]の基本列の降下性及び[4]のLemma 2.1より $`\textrm{Trans}(Q_{i+1})\leq_{\textrm{B}}\textrm{Trans}(Q_i)\lt_{\textrm{B}}\textrm{Trans}(N)`$ である。

帰納法により任意の非負整数 $`1\leq i\lt\textrm{Lng}(a)`$ に対して $`\textrm{Trans}(Q_i)\lt_{\textrm{B}}\textrm{Trans}(N)`$ である。

よって $`\textrm{Trans}(M)=\textrm{Trans}(Q_{\textrm{Lng}(a)})\lt_{\textrm{B}}\textrm{Trans}(N)`$ である。□
...
```math
\forall M,N\in CT_{\textrm{PS}},\quad M\lt_{\textrm{PS}}N\ \Longrightarrow\ \textrm{Trans}(M)\lt_{\textrm{B}}\textrm{Trans}(N)
```

命題そのものは原文どおりである。$`CT_{\textrm{PS}}`$ は原文の定義
$`CT_{\textrm{PS}}=\lbrace M\mid M\in ST_{\textrm{PS}}\land M_0=(0,0)\rbrace`$ をそのまま、
$`\lt_{\textrm{PS}}`$ も原文の再帰的定義をそのまま形式化したものを使う。
$`\leq_{\textrm{B}}`$ は $`t\leq_{\textrm{B}}u\iff(t\lt_{\textrm{B}}u\lor t=u)`$ である。

原文がこの証明で外部から引く 2 つの事実は、この形式化ではどちらも引用ではなく
証明済みの定理として使う。すなわち [1] の基本列の降下性

```math
M\in ST_{\textrm{PS}},\ n\geq1,\ \textrm{Lng}(M)\gt 1\ \Longrightarrow\ \textrm{Trans}(M[n])\lt_{\textrm{B}}\textrm{Trans}(M)
```

は未証明の仮定を一切含まない形で証明されており、[4] の Lemma 2.1 のうちここで使う
$`\lt_{\textrm{B}}`$ の推移律も証明されている。
...
$`\leq_{\textrm{B}}`$ は $`t\leq_{\textrm{B}}u\iff(t\lt_{\textrm{B}}u\lor t=u)`$ と定義されているので、
次の 3 つが場合分けだけで出る。原文が「[4]のLemma 2.1より」と一言で済ませている部分にあたる。

1. 反射律: 任意の $`t`$ に対し $`t\leq_{\textrm{B}}t`$。定義の第 2 項 $`t=t`$ による。
2. $`\leq_{\textrm{B}}`$ と $`\lt_{\textrm{B}}`$ の合成: $`s\leq_{\textrm{B}}t`$ かつ $`t\lt_{\textrm{B}}u`$ ならば $`s\lt_{\textrm{B}}u`$。
   $`s\leq_{\textrm{B}}t`$ を定義で開き、$`s\lt_{\textrm{B}}t`$ の場合は [4] の Lemma 2.1（$`\lt_{\textrm{B}}`$ の推移律）を
   $`s,t,u`$ に適用、$`s=t`$ の場合は仮定 $`t\lt_{\textrm{B}}u`$ がそのまま結論。
3. $`\leq_{\textrm{B}}`$ の推移律: $`s\leq_{\textrm{B}}t`$ かつ $`t\leq_{\textrm{B}}u`$ ならば $`s\leq_{\textrm{B}}u`$。
   両方を定義で開き、$`s\lt_{\textrm{B}}t\land t\lt_{\textrm{B}}u`$ なら推移律で $`s\lt_{\textrm{B}}u`$、
   $`s\lt_{\textrm{B}}t\land t=u`$ なら $`s\lt_{\textrm{B}}u`$、$`s=t`$ なら $`t\leq_{\textrm{B}}u`$ がそのまま結論。
...
成分がすべて $`1`$ 以上の任意の添字列 $`a\in\mathbb{N}_+^{\lt\omega}`$ と任意の $`Q\in ST_{\textrm{PS}}`$ に対し
...
- $`a=(n)\oplus a'`$ のとき。まず成分条件から $`n\geq1`$。
  1. 一歩ぶんの評価 $`\textrm{Trans}(Q[n])\leq_{\textrm{B}}\textrm{Trans}(Q)`$ を、$`\textrm{Lng}(Q)`$ で場合分けして作る。
     - $`\textrm{Lng}(Q)\gt 1`$ のとき。[1] の基本列の降下性を $`Q`$ と $`n`$ に適用する。
       適用条件は $`Q\in ST_{\textrm{PS}}`$（仮定）、$`n\geq1`$、$`\textrm{Lng}(Q)\gt 1`$ の 3 つで、
       結論は $`\textrm{Trans}(Q[n])\lt_{\textrm{B}}\textrm{Trans}(Q)`$。これから $`\leq_{\textrm{B}}`$ の定義の第 1 項として
     ...
     - $`\textrm{Lng}(Q)\leq1`$ のとき。補助 B より $`Q[n]=Q`$ なので、目標は
       ...
       ここでは降下性は使えない（使えば $`\textrm{Lng}(Q)\gt 1`$ が要る）。
...
**補助 D（$`\textrm{Lng}(N)\gt 1`$）.**
$`M,N\in CT_{\textrm{PS}}`$ かつ $`M\lt_{\textrm{PS}}N`$ ならば $`\textrm{Lng}(N)\gt 1`$。

$`\lt_{\textrm{PS}}`$ を「最初の相違位置」で分解する補題（原文の命題（基本列的順序が
辞書式的順序を含意すること）の証明が使う $`f`$ による場合分けを、$`\lt_{\textrm{PS}}`$ の
定義の言い換えとして切り出したもの）を $`M\lt_{\textrm{PS}}N`$ に適用すると、次のいずれかが成り立つ。

```math
\textrm{Lng}(M)\lt\textrm{Lng}(N)\ \land\ M=(N_j)_{j=0}^{\textrm{Lng}(M)-1}
```

```math
\exists k,\ k\lt\textrm{Lng}(M)\ \land\ k\lt\textrm{Lng}(N)\ \land\ (M_j)_{j=0}^{k-1}=(N_j)_{j=0}^{k-1}\ \land\ M_k\lt_{\textrm{lex}}N_k
```

ここで $`M_k=(M_{0,k},M_{1,k})`$、$`p\lt_{\textrm{lex}}q`$ は $`p_0\lt q_0\lor(p_0=q_0\land p_1\lt q_1)`$ である。

- 前者のとき。$`M\in CT_{\textrm{PS}}`$ より $`M\in ST_{\textrm{PS}}`$、標準形は空列でない（$`M\in T_{\textrm{PS}}`$）ので
  $`\textrm{Lng}(M)\geq1`$、よって $`\textrm{Lng}(N)\gt\textrm{Lng}(M)\geq1`$。
- 後者のとき。背理法で $`\textrm{Lng}(N)\leq1`$ とすると $`k\lt\textrm{Lng}(N)\leq1`$ から $`k=0`$。
  $`N\in CT_{\textrm{PS}}`$ の第 2 条件 $`N_0=(0,0)`$（Lean では $`N`$ が空列でないことから
  $`N_{0,0}=0`$ かつ $`N_{1,0}=0`$ として取り出す）より $`N_k=N_0=(0,0)`$ なので、
  $`M_0\lt_{\textrm{lex}}(0,0)`$ となる。しかし $`p\lt_{\textrm{lex}}(0,0)`$ は $`p_0\lt 0`$ または
  ($`p_0=0`$ かつ $`p_1\lt 0`$) であり、自然数では両方とも偽。矛盾。

**主張の証明.**

1. 原文と同じく、命題（基本列的順序が辞書式的順序を含意すること）を
   $`M,N\in CT_{\textrm{PS}}`$ と $`M\lt_{\textrm{PS}}N`$ に適用して $`M\lt_{\textrm{PS}[]}N`$ を得る。
   $`\lt_{\textrm{PS}[]}`$ の定義（ある添字列 $`a`$ が存在して $`a\neq()`$ かつ $`a`$ の各成分が $`1`$ 以上かつ
   $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$）を開いて、その $`a`$ を取る。
2. 補助 D より $`\textrm{Lng}(N)\gt 1`$。ここが原文の
   「$`((0,0))`$ が最小元だから $`N\neq((0,0))`$、上より $`\textrm{Lng}(N)\gt 1`$」に対応する箇所である。
...
6. 1 で得た $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$ と反復展開の定義
   ...
   目標は $`\textrm{Trans}((N[n])[a'_0]\cdots[a'_{\textrm{Lng}(a')-1}])\lt_{\textrm{B}}\textrm{Trans}(N)`$ になる。
   5 と 4 を補助 A の「$`\leq_{\textrm{B}}`$ と $`\lt_{\textrm{B}}`$ の合成」でつないで閉じる。

## 原文通りに書けなかった理由

- **[R]** 原文の「$`((0,0))`$ は $`\lt_{\textrm{PS}}`$ に対して $`T_{\textrm{PS}}`$ の最小の要素」は使っていない

  原文は $`\textrm{Lng}(N)\gt 1`$ を、最小元性から $`N\neq((0,0))`$ を出し、$`N\in CT_{\textrm{PS}}`$ の
  $`N_0=(0,0)`$ と合わせて長さが $`1`$ ではないと結論する、という 2 段で得ている。
  Lean はこの最小元性の補題を持っておらず、補助 D のとおり $`\lt_{\textrm{PS}}`$ を最初の相違位置で
  分解した形から直接 $`\textrm{Lng}(N)\gt 1`$ を出す。使うのは $`M`$ が空列でないことと
  $`N_0=(0,0)`$ の 2 つだけで、$`T_{\textrm{PS}}`$ 全体での最小元性（$`M\lt_{\textrm{PS}}((0,0))`$ が
  どんな $`M`$ でも偽であること、および $`((0,0))\leq_{\textrm{PS}}M`$）は現れない。
  結論も仮定も原文と同じなので、下流には何も影響しない。

- **[W]** 原文は各段の $`\textrm{Lng}(Q_i)\gt 1`$ を確かめていない（長さ $`1`$ では基本列が恒等になる）

  [1] の基本列の降下性は $`\textrm{Lng}(M)\gt 1`$ を前提にもつ。原文は $`i=0`$ の段については
  $`\textrm{Lng}(N)\gt 1`$ を用意しているが、$`i\geq1`$ の段の $`Q_i`$ については何も言わずに
  ...
  $`\lt_{\textrm{PS}[]}`$ の witness $`a`$ にはこのような冗長な段が入りうるので、
  原文の $`Q_i`$ の列も長さ $`1`$ の段を含みうる。このとき降下性は適用できず、
  成り立つのは等号のほうである（原文の結論が $`\lt_{\textrm{B}}`$ ではなく $`\leq_{\textrm{B}}`$ になっているのは
  この場合を含むためだと読めるが、原文はその理由を書いていない）。
  Lean は補助 C の各段で $`\textrm{Lng}(Q)\gt 1`$ かどうかで場合分けし、真なら降下性、
...
- **[W]** 原文の帰納法の結論の範囲が、最後の行で使う $`i=\textrm{Lng}(a)`$ を含んでいない
  ...
  原文は「帰納法により任意の非負整数 $`1\leq i\lt\textrm{Lng}(a)`$ に対して
  $`\textrm{Trans}(Q_i)\lt_{\textrm{B}}\textrm{Trans}(N)`$」と書いたうえで、次の行で
  $`\textrm{Trans}(M)=\textrm{Trans}(Q_{\textrm{Lng}(a)})\lt_{\textrm{B}}\textrm{Trans}(N)`$ を結論している。
  $`i=\textrm{Lng}(a)`$ は範囲 $`1\leq i\lt\textrm{Lng}(a)`$ の外である。帰納の段の主張
  （$`1\leq i\lt\textrm{Lng}(a)`$ で $`Q_i`$ から $`Q_{i+1}`$ へ）自体は $`i+1=\textrm{Lng}(a)`$ まで届くので、
...
- **[S]** 帰納法の形が違う（原文は $`i`$ の前進帰納、Lean は添字列の構造帰納）
  ...
  原文の帰納法は $`N`$ を固定し、不変量 $`\textrm{Trans}(Q_i)\lt_{\textrm{B}}\textrm{Trans}(N)`$ を
  $`i`$ について保つ形で、各段で「一歩ぶんの $`\leq_{\textrm{B}}`$」と「既に得ている $`\lt_{\textrm{B}}\textrm{Trans}(N)`$」を
...
| $`t\lt_{\textrm{B}}u`$ | `PSS.lessBT` | `lean/Buchholz-1986/Buchholz-1986-2.1.lean` |
...
| 補助 A-2（$`\leq_{\textrm{B}}`$ と $`\lt_{\textrm{B}}`$ の合成） | `leBT_lessBT_trans` | 同上 |
...
| 最初の相違位置による $`\lt_{\textrm{PS}}`$ の分解 | `ltPS_dest_idx` | `lean/Bijectivity/12a-lex-toolkit.lean` |
| $`M_k=(M_{0,k},M_{1,k})`$ | `pairAt` | 同上 |
| $`p\lt_{\textrm{lex}}q`$（ペアの比較） | `pairLt` | 同上 |
| $`M\in CT_{\textrm{PS}}\Rightarrow M_{0,0}=M_{1,0}=0`$ | `ctps_entry_zero` | `lean/Bijectivity/12b-ctps-finite.lean` |
| 補助 D（$`\textrm{Lng}(N)\gt 1`$） | `one_lt_lng_of_ltPS` | `lean/Bijectivity/18-trans-preserves-order.lean` |
