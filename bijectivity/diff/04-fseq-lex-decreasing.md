...
任意の $`M\in T_{\textrm{PS}}`$ と $`n\in\mathbb{N}_+`$ に対して、$`\textrm{Lng}(M)\gt 1`$ ならば $`M[n]\lt_{\textrm{PS}}M`$ である。
...
　$`M[n]=\textrm{Pred}(M)`$ ならば明らかに $`M[n]\lt_{\textrm{PS}}M`$ である。

　よって $`M_{j_1}\neq(0,0)`$ かつある非負整数 $`j_0`$ が存在して $`(i_1,j_0)\lt^{\textrm{Next}}_M(i_1,j_1)`$ であるとする。

　　[1] の $`\textrm{Pred}`$ が $`[1]`$ で表されることより $`n=1`$ ならば $`M[n]=\textrm{Pred}(M)`$ であり、$`M[n]=\textrm{Pred}(M)`$ の場合は考えなくていいから $`n\gt 1`$ とする。
...
　　よって $`M[n]\lt_{\textrm{PS}}M`$ は $`\bigoplus_{\mathbb{N}^2}(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ と同値である。
...
　　　$`\lt^{\textrm{Next}}`$ の定義より、$`M_{0,j_0}\lt  M_{0,j_1}`$ である。

　　　よって $`\bigoplus_{\mathbb{N}^2}(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ である。

　　$`i_1=1`$ とする。

　　　$`(M_{0,j_0}+\delta_0,M_{1,j_0}+\delta_1)=(M_{0,j_1},M_{1,j_0})`$ である。

　　　$`\lt^{\textrm{Next}}`$ の定義より、$`M_{1,j_0}\lt  M_{1,j_1}`$ である。

　　　よって $`\bigoplus_{\mathbb{N}^2}(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ である。

　　よっていずれの場合でも $`\bigoplus_{\mathbb{N}^2}(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ である。

　　従って $`M[n]\lt_{\textrm{PS}}M`$ である。

　よっていずれの場合でも $`M[n]\lt_{\textrm{PS}}M`$ である。□

## Lean

### Lean での命題

任意のペア数列 $`M`$ と $`n\geq1`$ に対して、$`\textrm{Lng}(M)\gt 1`$ ならば $`M[n]\lt_{\textrm{PS}}M`$ である。

原文の $`M\in T_{\textrm{PS}}`$ は仮定に置いていない。$`M`$ の型がペア数列（対の有限列）そのもので、
$`\textrm{Lng}(M)\gt 1`$ から $`M\neq()`$、すなわち $`M\in T_{\textrm{PS}}`$ が従うからである。

これに加えて、下流（辞書式的順序が基本列的順序を含意すること）で使う弱形

任意のペア数列 $`M`$ と $`n\geq1`$ に対して $`M[n]\leq_{\textrm{PS}}M`$ である

も系として置いてある。$`\textrm{Lng}(M)\gt 1`$ なら本命題から、$`\textrm{Lng}(M)\leq1`$ なら
$`j_1=0`$ で $`M[n]=M`$ だから等号で成り立つ。

### Lean での証明

**記号**（[1] の $`\textrm{operator}[]`$ の定義そのもの）。$`j_1=\textrm{Lng}(M)-1`$ とし、
$`i_1`$ を $`M_{1,j_1}\gt 0`$ のとき $`1`$、そうでないとき $`0`$ とする。$`j_0`$ を段 $`i_1`$ における $`j_1`$ の親、
...
(a) 真の接頭辞は $`\lt_{\textrm{PS}}`$ で小さい。すなわち $`k\lt\textrm{Lng}(M)`$ ならば
$`(M_j)_{j=0}^{k-1}\lt_{\textrm{PS}}M`$。証明は $`M`$ の長さに関する再帰である。$`M`$ が空列なら
$`k\lt 0`$ で矛盾。$`M=p\oplus_{\mathbb{N}^2}M'`$ で $`k=0`$ なら左辺は空列で、$`\lt_{\textrm{PS}}`$ の
空列規約（空列は非空列より小さい）から従う。$`k=k'+1`$ なら両辺の先頭が同じ $`p`$ なので
$`\lt_{\textrm{PS}}`$ の定義の第 3 の場合に落ち、$`(M'_j)_{j=0}^{k'-1}\lt_{\textrm{PS}}M'`$ を再帰で得る。

(b) 共通接頭辞は消去できる。すなわち任意の $`A,B,C`$ に対して
$`(A\oplus_{\mathbb{N}^2}B)\lt_{\textrm{PS}}(A\oplus_{\mathbb{N}^2}C)`$ は
$`B\lt_{\textrm{PS}}C`$ と同値。証明は $`A`$ の長さに関する再帰である。$`A`$ が空列なら両辺は同一。
$`A=p\oplus_{\mathbb{N}^2}A'`$ なら先頭が一致するので $`\lt_{\textrm{PS}}`$ の第 1・第 2 の場合が消え、
第 3 の場合だけが残って再帰する。

(c) $`(0,j_0)\lt^{\textrm{Next}}_M(0,j_1)`$ ならば $`M_{0,j_0}\lt M_{0,j_1}`$。上段の
$`\lt^{\textrm{Next}}`$ の定義の連言成分そのものを取り出す。

(d) $`(1,j_0)\lt^{\textrm{Next}}_M(1,j_1)`$ ならば $`M_{1,j_0}\lt M_{1,j_1}`$ であり、かつ
$`(0,j_0)\leq_M(0,j_1)`$ である。どちらも下段の $`\lt^{\textrm{Next}}`$ の定義の連言成分である。

(e) $`(0,j_0)\leq_M(0,j_1)`$ ならば $`M_{0,j_0}\leq M_{0,j_1}`$。$`\leq_M`$（上段）は上段の
$`\lt^{\textrm{Next}}`$ の反射推移閉包で、この形式化では鎖の長さの上界 $`\textrm{Lng}(M)`$ を
燃料とする再帰で定義されている。その燃料に関する帰納法で示す。燃料 $`0`$ のときは
$`j_0=j_1`$ なので等号。燃料 $`f+1`$ のときは $`j_0=j_1`$ であるか、ある $`j\lt j_1`$ が存在して
$`(0,j)\lt^{\textrm{Next}}_M(0,j_1)`$ かつ $`(0,j_0)\leq_M(0,j)`$（燃料 $`f`$）であるかのいずれかで、
後者では帰納法の仮定 $`M_{0,j_0}\leq M_{0,j}`$ と (c) の $`M_{0,j}\lt M_{0,j_1}`$ を推移律でつなぐ。

**本体**

(0) $`\textrm{Lng}(M)\gt 1`$ より $`M\neq()`$、および $`j_1=\textrm{Lng}(M)-1\neq0`$
（原文の「条件より $`j_1\gt 0`$ である」）。以下、$`M_{j_1}=(0,0)`$ か否か、段 $`i_1`$ に一意な親が
あるか否かで場合を分ける。これは $`M[n]`$ の定義の分岐そのものである。

(1) $`M_{j_1}=(0,0)`$ のとき。定義の第 2 分岐から $`M[n]=\textrm{Pred}(M)`$。
$`\textrm{Lng}(M)\gt 1`$ のとき $`\textrm{Pred}(M)=(M_j)_{j=0}^{j_1-1}`$ であり、
$`j_1\lt\textrm{Lng}(M)`$ だから (a) より $`M[n]\lt_{\textrm{PS}}M`$。
（原文の「$`M[n]=\textrm{Pred}(M)`$ ならば明らかに $`M[n]\lt_{\textrm{PS}}M`$ である」に当たる。）
...
$`\textrm{Lng}(M)\gt 1`$ のとき $`\textrm{Pred}(M)=M[1]`$）より $`M[1]=\textrm{Pred}(M)`$ なので (1) と同じ。
原文が「$`n=1`$ ならば $`M[n]=\textrm{Pred}(M)`$ であり … $`n\gt 1`$ とする」と述べる一歩に当たる。
...
・(b) を共通接頭辞 $`(M_j)_{j=0}^{j_1-1}`$ に適用して、$`M[n]\lt_{\textrm{PS}}M`$ は

```math
B_1\oplus_{\mathbb{N}^2}\bigoplus_{k=2}^{m+1}B_k<_{\textrm{PS}}(M_{j_1})
```

と同値である。原文の「よって $`M[n]\lt_{\textrm{PS}}M`$ は
$`\bigoplus_{\mathbb{N}^2}(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ と同値である」に当たる。

・$`j_0\lt j_1`$ より $`B_1`$ は空列でなく、その先頭は $`(M_{0,j_0}+\delta_0,M_{1,j_0}+\delta_1)`$ である。
右辺は長さ $`1`$ の列 $`(M_{j_1})`$ なので、$`\lt_{\textrm{PS}}`$ の定義の第 1 の場合
（第 0 成分が狭義に小さい）か第 2 の場合（第 0 成分が等しく第 1 成分が狭義に小さい）の
どちらかを示せばよい。

・一意な親であることから $`(i_1,j_0)\lt^{\textrm{Next}}_M(i_1,j_1)`$。

・$`i_1=0`$ のとき。$`0\lt i_1`$ も $`1\lt i_1`$ も偽だから $`\delta_0=\delta_1=0`$ で、先頭は
$`(M_{0,j_0},M_{1,j_0})`$ である。(c) より $`M_{0,j_0}\lt M_{0,j_1}`$ なので、$`\lt_{\textrm{PS}}`$ の
第 1 の場合で結論を得る。

・$`i_1\neq0`$ のとき。$`i_1`$ の定義（$`M_{1,j_1}\gt 0`$ なら $`1`$、そうでなければ $`0`$）から $`i_1=1`$。
...
第 0 成分は等しい。第 1 成分は (d) の $`M_{1,j_0}\lt M_{1,j_1}`$ と $`\delta_1=0`$ から
$`M_{1,j_0}+\delta_1\lt M_{1,j_1}`$。よって $`\lt_{\textrm{PS}}`$ の第 2 の場合で結論を得る。

以上いずれの場合でも $`M[n]\lt_{\textrm{PS}}M`$ である。

**系の証明**。$`\textrm{Lng}(M)\gt 1`$ なら本命題から $`M[n]\lt_{\textrm{PS}}M`$。
$`\textrm{Lng}(M)\leq1`$ なら $`j_1=\textrm{Lng}(M)-1=0`$ で定義の第 1 分岐から $`M[n]=M`$ なので
$`M[n]\leq_{\textrm{PS}}M`$ の左（等号）で成り立つ。

## 原文通りに書けなかった理由

- **[W]** 「$`M[n]=\textrm{Pred}(M)`$ ならば明らかに」は、真の接頭辞が $`\lt_{\textrm{PS}}`$ で
  小さいというリスト補題を隠している

  原文はこの分岐を一行で片付けるが、$`\textrm{Pred}(M)\lt_{\textrm{PS}}M`$ は
  $`\lt_{\textrm{PS}}`$ の定義から直ちには出ない。$`\textrm{Lng}(M)\gt 1`$ のとき
  $`\textrm{Pred}(M)=(M_j)_{j=0}^{j_1-1}`$ であることを使ったうえで、準備補題 (a)
  「$`k\lt\textrm{Lng}(M)`$ ならば $`(M_j)_{j=0}^{k-1}\lt_{\textrm{PS}}M`$」を列の長さに関する
  再帰で証明している。再帰の底は空列と非空列の比較（$`\lt_{\textrm{PS}}`$ の再帰的定義が
  最後に到達する空列の規約）で、再帰段は先頭が一致するので定義の第 3 の場合に落ちる。

- **[S]** 原文の場合分け「ある $`j_0`$ が存在して $`(i_1,j_0)\lt^{\textrm{Next}}_M(i_1,j_1)`$」を、
  ...
  $`j_0\lt j_0'`$ がともに段 $`i`$ における $`j_1`$ の親だとすると、$`j_0`$ の側の条件
  （$`j_0\lt j\lt j_1`$ なる $`j`$ すべてについて $`M_{i,j_1}\leq M_{i,j}`$。下段では
  さらに $`(0,j)\leq_M(0,j_1)`$ を満たす $`j`$ に限る）を $`j=j_0'`$ に適用して
  $`M_{i,j_1}\leq M_{i,j_0'}`$ を得るが、$`j_0'`$ の側の条件は $`M_{i,j_0'}\lt M_{i,j_1}`$ で
...
- **[W]** 「同値である」は、共通接頭辞の消去と $`M`$ 側の分解を隠している
  ...
  「よって $`M[n]\lt_{\textrm{PS}}M`$ は $`\bigoplus(B_i)_{i=1}^{n-1}\lt_{\textrm{PS}}(M_{j_1})`$ と
  同値である」と書くが、この一行には (i) $`M`$ 自身を
  $`M=(M_j)_{j=0}^{j_1-1}\oplus_{\mathbb{N}^2}(M_{j_1})`$ と分解すること、(ii) 共通接頭辞
  $`(M_j)_{j=0}^{j_1-1}`$ を $`\lt_{\textrm{PS}}`$ の比較から落とせること、の 2 つが要る。
...
- **[W]** $`i_1=1`$ の場合の等式 $`(M_{0,j_0}+\delta_0,M_{1,j_0}+\delta_1)=(M_{0,j_1},M_{1,j_0})`$ には
  ...
  等式を書いている。Lean では下段の $`\lt^{\textrm{Next}}`$ の定義に含まれる
...
- **[S]** Lean は長さの条件を外した弱形 $`M[n]\leq_{\textrm{PS}}M`$ を系として追加している
  ...
  原文の命題は $`\textrm{Lng}(M)\gt 1`$ を仮定する。下流の「辞書式的順序が基本列的順序を
...
| $`(i,j_0)\lt^{\textrm{Next}}_M(i,j_1)`$ | `PSS.nextR`（`PSS.nextrel0`／`PSS.nextrel1`） | 同上 |
...
| 親は $`j_0\lt j_1`$ を満たす | `PSS.parent_lt_of_hasParent` | `lean/6/6.6-condAB-coeff.lean` |
| 親は $`(i_1,j_0)\lt^{\textrm{Next}}_M(i_1,j_1)`$ を満たす | `PSS.hasParent_next_fseq` | `lean/6/6.2-P-fseq.lean` |
| (c) 上段の $`\lt^{\textrm{Next}}`$ は第 0 成分を狭義に増やす | `nextrel0_entry0_lt` | `lean/Bijectivity/04-fseq-lex-decreasing.lean` |
| (d) 下段の $`\lt^{\textrm{Next}}`$ は第 1 成分を狭義に増やす／$`\leq_M`$ を含む | `nextrel1_entry1_lt`／`nextrel1_le0` | 同上 |
