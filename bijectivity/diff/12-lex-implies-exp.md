...
　標準形の始切片への経路より$`M\lt_{\textrm{PS}[]}N`$である。  
$`j_1^M\gt 0`$ならば、任意の$`M',N'\in CT_{\textrm{PS}}`$に対して、$`\textrm{Lng}(M')\lt\textrm{Lng}(M)`$かつ$`M'\lt_{\textrm{PS}}N'`$ならば$`M'\lt_{\textrm{PS}[]}N'`$であると仮定する。  
　$`f=\min\lbrace \min(j_1^M,j_1^N)+1\rbrace \cup\lbrace j\mid j\in\mathbb{N}\land j\leq\min(j_1^M,j_1^N)\land M_j\neq N_j\rbrace`$とする。  
　$`f=j_1^M+1`$ならば、$`M=(N_j)_{j=0}^{j_1^M}`$であるから標準形の始切片への経路より$`M\lt_{\textrm{PS}[]}N`$である。  
　$`f\leq j_1^M`$とする。  
　　$`M\lt_{\textrm{PS}}N`$であるから$`f\leq j_1^N`$である。  
　　$`f=j_1^N`$とする。  
...
　　　任意の$`M',N'\in CT_{\textrm{PS}}`$に対して、$`\textrm{Lng}(M')=\textrm{Lng}(N)`$かつ$`N\leq_{\textrm{PS}}M'\lt_{\textrm{PS}}N'`$ならば$`M'\lt_{\textrm{PS}[]}N'`$であると仮定する。  
...
　　　　$`\leq_{\textrm{PS}[]}`$の定義よりある$`a\in\mathbb{N}_+^{\lt\omega}`$が存在して$`M=((j,j))_{j=0}^v[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$である。  
　　　　辞書式的順序が基本列的順序を含意することより$`N\leq_{\textrm{PS}}((j,j))_{j=0}^v`$である。  
　　　　$`M=((j,j))_{j=0}^v[a_0]\cdots[a_{\textrm{Lng}(a)-1}]\leq_{\textrm{PS}}N\leq_{\textrm{PS}}((j,j))_{j=0}^v`$よりある$`g\in\mathbb{N}`$が存在して$`g\leq\textrm{Lng}(a)`$かつ$`((j,j))_{j=0}^v[a_0]\cdots[a_g]\lt_{\textrm{PS}}N`$であり、そのような$`g`$で最小のものを$`g_0`$とする。  
　　　　$`N'=((j,j))_{j=0}^v[a_0]\cdots[a_{g_0-1}]`$とする。  
　　　　辞書式的順序の線形性及び$`g_0`$の定義より$`N\leq_{\textrm{PS}}N'`$である。  
　　　　$`N\lt_{\textrm{PS}}N'`$とする。  
　　　　　辞書式的順序が基本列的順序を含意することより$`M\lt_{\textrm{PS}}N'[a_{g_0}]\lt_{\textrm{PS}}N`$である。  
　　　　　$`f`$の定義及び仮定より$`(M_j)_{j=0}^{j_1^N-1}=(N_j)_{j=0}^{j_1^N-1}`$であるから、上より$`\textrm{Lng}(N'[a_{g_0}])\gt j_1^N`$かつ$`(M_j)_{j=0}^{j_1^N-1}=(N'[a_{g_0}]_j)_{j=0}^{j_1^N-1}`$である。  
　　　　　上より$`(N'[a_{g_0}]_j)_{j=j_1^N}^{\textrm{Lng}(N'[a_{g_0}])-1}\lt_{\textrm{PS}}(N_j)_{j=j_1^N}^{j_1^N}=(N_{j_1^N})`$であり、よって$`(N'[a_{g_0}]_{j_1^N})\lt_{\textrm{PS}}(N_{j_1^N})`$であるから、$`(N'[a_{g_0}]_j)_{j=0}^{j_1^N}\lt_{\textrm{PS}}N`$である。  
　　　　　任意の$`n\in\mathbb{N}_+`$を取る。  
　　　　　　$`\textrm{Lng}(N'[n])\leq j_1^N`$ならば、基本列の切片の不変性より$`N'[n]=(N'[a_{g_0}]_j)_{j=0}^{\textrm{Lng}(N'[n])-1}\lt_{\textrm{PS}}(N'[a_{g_0}]_j)_{j=0}^{j_1^N}\lt_{\textrm{PS}}N`$である。  
　　　　　　$`\textrm{Lng}(N'[n])\gt j_1^N`$ならば、基本列の切片の不変性より$`(N'[n]_j)_{j=0}^{j_1^N}=(N'[a_{g_0}]_j)_{j=0}^{j_1^N}`$であり、$`(N'[a_{g_0}]_j)_{j=0}^{j_1^N}\lt_{\textrm{PS}}N`$であるから、$`\lt_{\textrm{PS}}`$の定義より$`N'[n]\lt_{\textrm{PS}}N`$である。  
　　　　　従って任意の$`n\in\mathbb{N}_+`$に対して$`N'[n]\lt_{\textrm{PS}}N`$である。  
　　　　　辞書式的順序が基本列的順序を含意することより任意の$`n\in\mathbb{N}_+`$に対して$`N\lt_{\textrm{PS}[]}N'[n]`$ではない。  
　　　　　従って$`N\lt_{\textrm{PS}[]}N'`$ではない。  
　　　　　仮定より$`N\lt_{\textrm{PS}[]}N'`$であるが、これは上と矛盾する。  
　　　　従って$`N=N'`$である。  
　　　　よって$`N[a_{g_0}]\cdots[a_{\textrm{Lng}(a)-1}]=M`$であるから、$`M\lt_{\textrm{PS}[]}N`$である。  
　　　　仮定より任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`N\lt_{\textrm{PS}}N'`$ならば$`N\lt_{\textrm{PS}[]}N'`$である。  
　　　　基本列的順序が推移性より、任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`N\leq_{\textrm{PS}}N'`$ならば$`M\lt_{\textrm{PS}[]}N'`$である。  
　　　　ある$`M'\in CT_{\textrm{PS}}`$が存在して、$`\textrm{Lng}(M')=\textrm{Lng}(N)`$かつ$`(M'_j)_{j=0}^{j_1^N-1}=(N_j)_{j=0}^{j_1^N-1}`$かつ$`M'\lt_{\textrm{PS}}N`$であるならば、そのようなもののうち$`\lt_{\textrm{PS}}`$に対する最大元をとる。  
　　　　$`M=M'`$とした場合により、任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`N\leq_{\textrm{PS}}N'`$ならば$`M'\lt_{\textrm{PS}[]}N'`$である。  
　　　　任意の$`O\in CT_{\textrm{PS}}`$をとり、$`M'\lt_{\textrm{PS}}O\lt_{\textrm{PS}}N`$とする。  
　　　　　$`\lt_{\textrm{PS}}`$の定義より$`M'\leq_{\textrm{PS}}(O_j)_{j=0}^{\min(\textrm{Lng}(O)-1,j_1^N)}\lt_{\textrm{PS}}N`$である。  
...
　　　　よって任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`M'\lt_{\textrm{PS}}N'`$ならば$`M'\lt_{\textrm{PS}[]}N'`$である。  
　　　$`\textrm{Lng}(N)`$及び$`(N_j)_{j=0}^{j_1^N-1}`$を固定したときの帰納法により、任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`N\leq_{\textrm{PS}}N'`$ならば$`M\lt_{\textrm{PS}[]}N'`$である。  
　　よって$`f=j_1^N`$ならば、任意の$`N'\in CT_{\textrm{PS}}`$に対して、$`N\leq_{\textrm{PS}}N'`$ならば$`M\lt_{\textrm{PS}[]}N'`$である。  
　　$`f\leq j_1^N`$であり、$`f`$の定義より$`N`$を$`(N_j)_{j=0}^f`$に置き換えても$`f`$が不変であり、$`(N_j)_{j=0}^f\leq_{\textrm{PS}}N`$であるから上より$`M\lt_{\textrm{PS}[]}N`$である。  
　　よっていずれの場合でも$`M\lt_{\textrm{PS}[]}N`$である。  
　よっていずれの場合でも$`M\lt_{\textrm{PS}[]}N`$である。  
帰納法により任意の$`M,N\in CT_{\textrm{PS}}`$に対して、$`M\lt_{\textrm{PS}}N`$ならば$`M\lt_{\textrm{PS}[]}N`$である。□
...
- $`\lt_{\textrm{PS}}`$ は原文の再帰的定義。再帰の末尾に現れる空列については、$`()\lt_{\textrm{PS}}N`$ を
  $`N\neq()`$ と同値、$`M\lt_{\textrm{PS}}()`$ を偽、と辞書式順序の規約で補う。
- $`M\leq_{\textrm{PS}[]}N`$ は「ある $`a\in\mathbb{N}_+^{\lt\omega}`$ が存在して
  $`M=N[a_0]\cdots[a_{\textrm{Lng}(a)-1}]`$」、$`M\lt_{\textrm{PS}[]}N`$ はさらに $`a\neq()`$。
...
$`\lt_{\textrm{PS}}`$ を逆向きにした関係についての整礎帰納法（$`S_L`$ の中での下降帰納法）を一つだけ回す。
示す主張は

$`\Phi_L(X)`$: 任意の $`N\in CT_{\textrm{PS}}`$ に対して、$`\textrm{Lng}(N)\leq L`$ かつ $`X\lt_{\textrm{PS}}N`$ ならば $`X\lt_{\textrm{PS}[]}N`$
...
**(1-d) 真の始切片は狭義に下.** $`X,N\in CT_{\textrm{PS}}`$、$`\textrm{Lng}(X)\lt\textrm{Lng}(N)`$、
$`X=(N_j)_{j=0}^{\textrm{Lng}(X)-1}`$ ならば $`X\lt_{\textrm{PS}[]}N`$。
$`\textrm{Lng}(X)\gt 0`$ より $`\textrm{Lng}(X)-1\lt\textrm{Lng}(N)`$ なので (1-c) が使えて
$`X=(N_j)_{j=0}^{\textrm{Lng}(X)-1}\leq_{\textrm{PS}[]}N`$、すなわち $`a`$ が取れる。
$`a=()`$ とすると $`X=N`$ となり $`\textrm{Lng}(X)\lt\textrm{Lng}(N)`$ に反するので $`a\neq()`$。

**(2) $`\lt_{\textrm{PS}}`$ の分解（原文の $`f`$）**

**(2-a) 分解.** $`M\lt_{\textrm{PS}}N`$ ならば次のいずれかが成り立つ。

- (i) $`\textrm{Lng}(M)\lt\textrm{Lng}(N)`$ かつ $`M=(N_j)_{j=0}^{\textrm{Lng}(M)-1}`$。
- (ii) ある $`f`$ が存在して、$`f\lt\textrm{Lng}(M)`$、$`f\lt\textrm{Lng}(N)`$、$`(M_j)_{j=0}^{f-1}=(N_j)_{j=0}^{f-1}`$、
  かつ $`M_{0,f}\lt N_{0,f}`$ または（$`M_{0,f}=N_{0,f}`$ かつ $`M_{1,f}\lt N_{1,f}`$）。

証明は $`\lt_{\textrm{PS}}`$ の再帰的定義に沿った両列同時の帰納である。どちらかが空列の場合は
定義から直ちに (i)。先頭が異なる場合（$`M_{0,0}\lt N_{0,0}`$ または $`M_{0,0}=N_{0,0}\land M_{1,0}\lt N_{1,0}`$）は
...
(ii) は $`f\lt\textrm{Lng}(M)`$ と $`f\lt\textrm{Lng}(N)`$ を同時に与えるので、原文の
「$`M\lt_{\textrm{PS}}N`$ であるから $`f\leq j_1^N`$」の一手は分解に含まれている。

**(2-b) 逆向き.** $`k\lt\textrm{Lng}(M)`$、$`k\lt\textrm{Lng}(N)`$、$`(M_j)_{j=0}^{k-1}=(N_j)_{j=0}^{k-1}`$、
かつ $`M_k`$ が $`N_k`$ より対の辞書式で小さいならば $`M\lt_{\textrm{PS}}N`$。
...
**(2-c) 真の始切片は小さい.** $`k\lt\textrm{Lng}(M)`$ ならば $`(M_j)_{j=0}^{k-1}\lt_{\textrm{PS}}M`$。

**(2-d) 切片が小さければ本体も小さい.** $`\textrm{Lng}(N)\leq k`$ かつ $`(M_j)_{j=0}^{k-1}\lt_{\textrm{PS}}N`$
ならば $`M\lt_{\textrm{PS}}N`$。(2-a) で $`(M_j)_{j=0}^{k-1}\lt_{\textrm{PS}}N`$ を分解する。(i) なら
$`\min(k,\textrm{Lng}(M))=\textrm{Lng}((M_j)_{j=0}^{k-1})\lt\textrm{Lng}(N)\leq k`$ だから
$`\textrm{Lng}(M)\lt k`$、すなわち切片は $`M`$ 自身なので仮定がそのまま結論である。(ii) なら相違位置 $`d`$ は
$`d\lt\textrm{Lng}((M_j)_{j=0}^{k-1})\leq k`$ なので $`M_d=((M_j)_{j=0}^{k-1})_d`$ であり、
$`(M_j)_{j=0}^{d-1}=(N_j)_{j=0}^{d-1}`$ と合わせて (2-b) を $`M`$ と $`N`$ に使える。

なお (2) の道具立てには、対の辞書式比較の非対称性と、接頭辞関係から $`\lt_{\textrm{PS}}`$ を出す補題も
置かれているが、以降では使っていない。

**(3) 有限性（原文の係数評価と個数評価）**

**(3-a) 係数評価.** $`M\in CT_{\textrm{PS}}`$、$`j\lt\textrm{Lng}(M)`$ ならば $`M_{1,j}\leq M_{0,j}\leq j`$。
...
**(3-d) 下降帰納.** $`\lt_{\textrm{PS}}`$ は 辞書式的順序の線形性 より非反射的かつ推移的だから、
有限集合 $`S_L`$ の上でその逆関係は整礎である。よって $`S_L`$ の元 $`X`$ について、
$`S_L`$ の中で $`X`$ より $`\lt_{\textrm{PS}}`$ で真に上の元すべてで主張が成り立つと仮定してよい。

**(4) 相手の長さの帰着（原文の $`N`$ を $`(N_j)_{j=0}^f`$ に置き換える段）**

$`X\in CT_{\textrm{PS}}`$ とし、$`\textrm{Lng}(N)\leq\textrm{Lng}(X)`$ なる $`N\in CT_{\textrm{PS}}`$ について
$`X\lt_{\textrm{PS}}N\Rightarrow X\lt_{\textrm{PS}[]}N`$ が分かっているとする。このとき任意の
$`N\in CT_{\textrm{PS}}`$ について同じ含意が成り立つ。

$`X\lt_{\textrm{PS}}N`$ を (2-a) で分解する。(i) なら (1-d) で $`X\lt_{\textrm{PS}[]}N`$。(ii) の $`f`$ に対して
$`\tilde N=(N_j)_{j=0}^{f}`$ と置くと、

- (1-b) より $`\tilde N\in CT_{\textrm{PS}}`$、
- $`\textrm{Lng}(\tilde N)=f+1\leq\textrm{Lng}(X)`$（$`f\lt\textrm{Lng}(X)`$ より）、
- $`(X_j)_{j=0}^{f-1}=(N_j)_{j=0}^{f-1}=(\tilde N_j)_{j=0}^{f-1}`$ かつ $`\tilde N_f=N_f`$ なので (2-b) より
  $`X\lt_{\textrm{PS}}\tilde N`$。

仮定より $`X\lt_{\textrm{PS}[]}\tilde N`$、(1-c) より $`\tilde N\leq_{\textrm{PS}[]}N`$、(1-a) を合わせて
$`X\lt_{\textrm{PS}[]}N`$。原文はここを「$`(N_j)_{j=0}^f\leq_{\textrm{PS}}N`$ であるから上より」と
$`\leq_{\textrm{PS}}`$ で繋いでいるが、ここでは $`\tilde N\leq_{\textrm{PS}[]}N`$ で繋いでいる。

**(5) 主要部（原文の $`f=j_1^N`$ の場合）**

**主張.** $`M,N\in CT_{\textrm{PS}}`$、$`M\lt_{\textrm{PS}}N`$、$`\textrm{Lng}(N)\leq\textrm{Lng}(M)`$、
$`j_1=\textrm{Lng}(N)-1`$ と置いて $`(M_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$ とする。さらに

$`\Phi(N)`$: 任意の $`N'\in CT_{\textrm{PS}}`$ に対して $`N\lt_{\textrm{PS}}N'`$ ならば $`N\lt_{\textrm{PS}[]}N'`$

を仮定する。このとき $`M\lt_{\textrm{PS}[]}N`$。
...
**(5-a)** 標準形は空列でないので $`\textrm{Lng}(N)=j_1+1\gt 0`$、$`\textrm{Lng}(N)\leq\textrm{Lng}(M)`$ より
$`j_1\lt\textrm{Lng}(M)`$。
...
**(5-c)** $`\leq_{\textrm{PS}[]}`$ の定義より $`a\in\mathbb{N}_+^{\lt\omega}`$ が取れて
...
$`M\lt_{\textrm{PS}}N`$ と合わせて $`N\lt_{\textrm{PS}}N`$ となり非反射性に反する。

**(5-f)** 条件「$`((j,j))_{j=0}^{v}[a_0]\cdots[a_g]\lt_{\textrm{PS}}N`$」は $`g=\textrm{Lng}(a)-1`$ で
成り立つ（そのとき左辺は $`M`$ で、$`M\lt_{\textrm{PS}}N`$）。この条件を満たす最小の $`g`$ を $`g_0`$ と置く。
最小性から $`g_0\leq\textrm{Lng}(a)-1`$、すなわち $`g_0\lt\textrm{Lng}(a)`$ なので $`a_{g_0}`$ が存在する。

**(5-g)** $`N'=((j,j))_{j=0}^{v}[a_0]\cdots[a_{g_0-1}]`$、$`m=a_{g_0}\geq1`$ と置く。添字列の連結より
$`((j,j))_{j=0}^{v}[a_0]\cdots[a_{g_0}]=N'[m]`$ であり、$`g_0`$ の取り方から $`N'[m]\lt_{\textrm{PS}}N`$。

**(5-h)** $`N'\in CT_{\textrm{PS}}`$。$`N'=((j,j))_{j=0}^{v}[a_0]\cdots[a_{g_0-1}]`$ すなわち
$`N'\leq_{\textrm{PS}[]}((j,j))_{j=0}^{v}`$ なので、可算な標準形の起源 の逆向きによる。

**(5-i)** $`N\leq_{\textrm{PS}}N'`$。$`g_0=0`$ なら $`N'=((j,j))_{j=0}^{v}`$ で (5-d) そのもの。$`g_0\gt 0`$ なら
$`g_0`$ の最小性より $`\lnot(N'\lt_{\textrm{PS}}N)`$ であり、辞書式的順序の線形性（三分律）より
$`N\lt_{\textrm{PS}}N'`$ か $`N=N'`$。
...
**(5-k) 挟み撃ち.** 一般に、$`M\leq_{\textrm{PS}}K\lt_{\textrm{PS}}N`$、$`j_1\lt\textrm{Lng}(M)`$、
$`j_1\leq\textrm{Lng}(N)`$、$`(M_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$ ならば
$`(K_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$ かつ $`j_1\lt\textrm{Lng}(K)`$。
$`M=K`$ なら自明。$`M\lt_{\textrm{PS}}K`$ のときは (2-a) で分解する。

- (i)（$`M`$ が $`K`$ の真の始切片）なら $`\textrm{Lng}(M)\lt\textrm{Lng}(K)`$ で $`j_1\lt\textrm{Lng}(M)`$ だから
  $`j_1\lt\textrm{Lng}(K)`$、また $`(K_j)_{j=0}^{j_1-1}=(M_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$。
- (ii) で相違位置 $`d\lt j_1`$ のとき: $`M`$ と $`N`$ は $`j_1`$ 未満で一致するから $`M_d=N_d`$、また
  $`(N_j)_{j=0}^{d-1}=(M_j)_{j=0}^{d-1}=(K_j)_{j=0}^{d-1}`$。すると (2-b) より $`N\lt_{\textrm{PS}}K`$ となり、
  $`K\lt_{\textrm{PS}}N`$ と推移性・非反射性で矛盾。
- (ii) で $`d\geq j_1`$ のとき: $`(K_j)_{j=0}^{j_1-1}=(M_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$ で、
  $`j_1\leq d\lt\textrm{Lng}(K)`$。

これを $`K=N'[m]`$ に適用する（(5-j) と (5-g)、(5-a)）と、$`\textrm{Lng}(N'[m])\gt j_1`$ かつ
$`(N'[m]_j)_{j=0}^{j_1-1}=(N_j)_{j=0}^{j_1-1}`$ を得る。原文の
「$`\textrm{Lng}(N'[a_{g_0}])\gt j_1^N`$ かつ $`(M_j)_{j=0}^{j_1^N-1}=(N'[a_{g_0}]_j)_{j=0}^{j_1^N-1}`$」に対応する。

**(5-l)** $`(N'[m]_j)_{j=0}^{j_1}\lt_{\textrm{PS}}N`$。$`N'[m]\lt_{\textrm{PS}}N`$ を (2-a) で分解する。
$`\textrm{Lng}(N)=j_1+1\leq\textrm{Lng}(N'[m])`$ なので (i) は起きない。(ii) の相違位置 $`d`$ は
$`d\lt\textrm{Lng}(N)=j_1+1`$ であり、$`d\lt j_1`$ なら (5-k) の一致から $`N'[m]_d=N_d`$ となって
「$`N'[m]_d`$ が $`N_d`$ より対の辞書式に小さい」に反するので $`d=j_1`$。よって $`(N'[m]_j)_{j=0}^{j_1}`$ と $`N`$ は
$`j_1`$ 未満で一致し $`j_1`$ で対の辞書式に小さいから、(2-b) より結論を得る。

**(5-m)** 任意の $`n\in\mathbb{N}_+`$ に対して $`N'[n]\lt_{\textrm{PS}}N`$。$`\textrm{Lng}(N'[n])`$ で場合分けする。

- $`\textrm{Lng}(N'[n])\leq j_1`$ のとき。まず $`n\leq m`$ である。実際 $`m\lt n`$ とすると、基本列の切片の不変性の
  基になる「$`m\leq n`$ なら $`N'[n]`$ は $`N'[m]`$ を接頭辞にもつ」より
  $`\textrm{Lng}(N'[n])\geq\textrm{Lng}(N'[m])\gt j_1`$ となり矛盾する。$`n\leq m`$ なら同じ接頭辞性より
  $`N'[n]=(N'[m]_j)_{j=0}^{\textrm{Lng}(N'[n])-1}`$ で、$`\textrm{Lng}(N'[n])\leq j_1\lt j_1+1`$ だからこれは
  $`(N'[m]_j)_{j=0}^{j_1}`$ の真の始切片である。(2-c) より
  $`N'[n]\lt_{\textrm{PS}}(N'[m]_j)_{j=0}^{j_1}`$、(5-l) と推移性で $`N'[n]\lt_{\textrm{PS}}N`$。
- $`\textrm{Lng}(N'[n])\gt j_1`$ のとき。$`n,m\geq1`$ で $`j_1+1`$ が両方の長さ以下だから、上の接頭辞性を
  $`\min(n,m)`$ と $`\max(n,m)`$ に使って $`(N'[n]_j)_{j=0}^{j_1}=(N'[m]_j)_{j=0}^{j_1}`$（基本列の切片の不変性）。
  (5-l) より右辺は $`\lt_{\textrm{PS}}N`$ で、$`\textrm{Lng}(N)=j_1+1`$ だから (2-d) より $`N'[n]\lt_{\textrm{PS}}N`$。

**(5-n)** $`N=N'`$。(5-i) より $`N\lt_{\textrm{PS}}N'`$ か $`N=N'`$ である。$`N\lt_{\textrm{PS}}N'`$ とすると、
(5-h) と $`\Phi(N)`$ より $`N\lt_{\textrm{PS}[]}N'`$、すなわち $`c\neq()`$、$`c\in\mathbb{N}_+^{\lt\omega}`$ が取れて
$`N=N'[c_0]\cdots[c_{\textrm{Lng}(c)-1}]`$。$`c_0\geq1`$ であり $`N`$ は $`N'[c_0]`$ の反復展開だから
$`N\leq_{\textrm{PS}}N'[c_0]`$（$`\textrm{Lng}(c)=1`$ のときは等号）。一方 (5-m) より
$`N'[c_0]\lt_{\textrm{PS}}N`$。合わせて $`N\lt_{\textrm{PS}}N`$ となり非反射性に反する。

**(5-o)** $`M=N'[a_{g_0}]\cdots[a_{\textrm{Lng}(a)-1}]=N[a_{g_0}]\cdots[a_{\textrm{Lng}(a)-1}]`$ であり、
$`(a_j)_{j=g_0}^{\textrm{Lng}(a)-1}`$ は (5-f) の $`g_0\lt\textrm{Lng}(a)`$ より空でなく、その各項は $`1`$ 以上。
よって $`M\lt_{\textrm{PS}[]}N`$。

**(6) 帰納法本体と結論**

$`L`$ を固定し、$`S_L`$ 上の下降帰納 (3-d) で $`\Phi_L`$ を示す。$`X\in S_L`$ を取り、$`S_L`$ の中で $`X`$ より
$`\lt_{\textrm{PS}}`$ で真に上の元すべてで $`\Phi_L`$ が成り立つと仮定する。$`N\in CT_{\textrm{PS}}`$、
$`\textrm{Lng}(N)\leq L`$、$`X\lt_{\textrm{PS}}N`$ とし、(2-a) で分解する。

- (i) なら (1-d) より $`X\lt_{\textrm{PS}[]}N`$。
- (ii) の $`f`$ に対して $`\tilde N=(N_j)_{j=0}^{f}`$ と置く。(1-b) より $`\tilde N\in CT_{\textrm{PS}}`$、
  ...
  $`X\lt_{\textrm{PS}}\tilde N`$。すなわち $`\tilde N`$ は $`S_L`$ の中で $`X`$ より真に上にあるから、
  帰納法の仮定が $`\tilde N`$ に使えて $`\Phi_L(\tilde N)`$ を得る。相手の長さ制限は (4) で外れて、
  $`\Phi(\tilde N)`$（任意の $`N_3\in CT_{\textrm{PS}}`$ に対して $`\tilde N\lt_{\textrm{PS}}N_3`$ ならば
  $`\tilde N\lt_{\textrm{PS}[]}N_3`$）が出る。ここで
  $`\textrm{Lng}(\tilde N)-1=f`$ かつ $`(X_j)_{j=0}^{f-1}=(\tilde N_j)_{j=0}^{f-1}`$ かつ
  $`\textrm{Lng}(\tilde N)\leq\textrm{Lng}(X)`$ だから、(5) の仮定がすべて揃い $`X\lt_{\textrm{PS}[]}\tilde N`$。
  (1-c) の $`\tilde N\leq_{\textrm{PS}[]}N`$ と (1-a) を合わせて $`X\lt_{\textrm{PS}[]}N`$。

最後に、任意の $`M,N\in CT_{\textrm{PS}}`$ と $`M\lt_{\textrm{PS}}N`$ に対して、$`L=\textrm{Lng}(M)`$ と置き、
(4) により相手を $`\textrm{Lng}(N)\leq L`$ の場合に帰着してから $`\Phi_L(M)`$ を使えば $`M\lt_{\textrm{PS}[]}N`$
を得る。$`\square`$
...
- **[R]** 外側の $`\textrm{Lng}(M)`$ に関する帰納法が丸ごと要らない
  ...
  原文は $`j_1^M=0`$ を基底段階、$`j_1^M\gt 0`$ を帰納段として $`\textrm{Lng}(M)`$ についての強帰納法を
  組んでいる（「$`j_1^M\gt 0`$ ならば、任意の $`M',N'\in CT_{\textrm{PS}}`$ に対して
  $`\textrm{Lng}(M')\lt\textrm{Lng}(M)`$ かつ $`M'\lt_{\textrm{PS}}N'`$ ならば $`M'\lt_{\textrm{PS}[]}N'`$ であると
...
- **[W]** 標準形の始切片への経路 を狭義で使う 2 箇所で、狭義性を作り直す一手が要る
  ...
  原文は $`M=(N_j)_{j=0}^{j_1^M}`$ から 標準形の始切片への経路 で直ちに $`M\lt_{\textrm{PS}[]}N`$ を
  得ている。しかしこの補題の結論は $`\leq_{\textrm{PS}[]}`$ であって $`\lt_{\textrm{PS}[]}`$ ではない
  （$`j_1'=j_1`$ のとき原文の逐語形は $`M\lt_{\textrm{PS}[]}M`$ を主張してしまう）。原文自身、後の
  $`((j,j))_{j=0}^{v^M}\leq_{\textrm{PS}[]}((j,j))_{j=0}^{v}`$ では $`\leq`$ で使っている。
  そこで (1-d) では、$`\leq_{\textrm{PS}[]}`$ から取った $`a`$ について「$`a=()`$ なら $`X=N`$ となり
  $`\textrm{Lng}(X)\lt\textrm{Lng}(N)`$ に反する」という一手を足して狭義性を回復している。

- **[⚠️Y]** 「高々 $`(j_1^N)^2`$ 個である」は偽
...
- **[🚨Z]** 内側の下降帰納に基底段階が無い
  ...
  の上での $`\lt_{\textrm{PS}}`$ に関する下降帰納である（仮定は「$`N\leq_{\textrm{PS}}M'`$ なる $`M'`$ について
  結論が既知」、段は「$`S`$ の中で $`N`$ の直下にある最大元 $`M'`$ について結論を導く」）。下降帰納には
  $`S`$ の $`\lt_{\textrm{PS}}`$-最大元 $`X`$ での基底段階が要るが、$`X`$ より上の元は $`S`$ に無いので帰納法の
  仮定が使えず、しかも結論「任意の $`N'\in CT_{\textrm{PS}}`$ に対して $`X\lt_{\textrm{PS}}N'`$ ならば
  $`X\lt_{\textrm{PS}[]}N'`$」は $`N'`$ が $`S`$ の外を動くので自明ではない。原文はこの段階を扱っていない。
  ...
  $`S_L`$ の最大元では $`X\lt_{\textrm{PS}}N`$ なる $`N`$ が台の中に無いので主張が空虚に成り立ち、
...
- **[W]** $`g_0`$ の取り方の境界（$`a=()`$ と $`g_0=0`$）が確かめられていない
  ...
  「$`a=()`$ なら $`M=((j,j))_{j=0}^{v}`$ かつ $`N\leq_{\textrm{PS}}M\lt_{\textrm{PS}}N`$ で矛盾」を補った。
  また原文は「辞書式的順序の線形性及び $`g_0`$ の定義より $`N\leq_{\textrm{PS}}N'`$」とするが、$`g_0=0`$ の
  ときは $`g_0`$ の最小性が何も言わないので、この推論は $`g_0\gt 0`$ でしか通らない。(5-i) で
...
- **[⚠️Y]** $`M\lt_{\textrm{PS}}N'[a_{g_0}]`$ は $`\leq_{\textrm{PS}}`$ が正しい
  ...
  1 手で初めて $`N`$ を下回る場合）。$`\lt_{\textrm{PS}}`$ を $`\leq_{\textrm{PS}}`$ に直せばよく、下流で
  この不等式を使うのは (5-k) の挟み撃ちだけで、そこは $`\leq_{\textrm{PS}}`$ で足りる。
  (5-j) では反復展開が $`\leq_{\textrm{PS}}`$ を下げることから $`M\leq_{\textrm{PS}}N'[a_{g_0}]`$ を得ている。

- **[W]** 「上より $`\textrm{Lng}(N'[a_{g_0}])\gt j_1^N`$ かつ …」の一行は場合分けを隠している

  原文はこの一行を「上より」で済ませているが、$`M\leq_{\textrm{PS}}N'[a_{g_0}]\lt_{\textrm{PS}}N`$ と
  ...
  $`N\lt_{\textrm{PS}}N'[a_{g_0}]\lt_{\textrm{PS}}N`$ の矛盾で潰している。
...
- **[W]** $`N'[n]`$ が $`N'[a_{g_0}]`$ の始切片であるためには $`n\leq a_{g_0}`$ が要る
  ...
  $`n\leq a_{g_0}`$ のときにしか言えない。(5-m) では、$`a_{g_0}\lt n`$ なら接頭辞性から
  $`\textrm{Lng}(N'[n])\geq\textrm{Lng}(N'[a_{g_0}])\gt j_1^N`$ となって場合の仮定に反する、として
  $`n\leq a_{g_0}`$ を先に出している。

- **[W]** 「任意の $`n`$ について $`N\lt_{\textrm{PS}[]}N'[n]`$ でない、従って $`N\lt_{\textrm{PS}[]}N'`$ でない」は
  展開の長さが $`1`$ の場合を落としている

  $`N\lt_{\textrm{PS}[]}N'`$ とは $`c\neq()`$ が取れて $`N=N'[c_0]\cdots[c_{\textrm{Lng}(c)-1}]`$ ということだが、
  $`\textrm{Lng}(c)=1`$ のときは $`N=N'[c_0]`$ であって $`N\lt_{\textrm{PS}[]}N'[c_0]`$ ではないので、原文の
  推論はこの場合を覆わない。(5-n) では $`\leq_{\textrm{PS}}`$ に落として
  「$`N\leq_{\textrm{PS}}N'[c_0]`$ かつ $`N'[c_0]\lt_{\textrm{PS}}N`$ より $`N\lt_{\textrm{PS}}N`$」と
  非反射性で矛盾を出しており、$`\textrm{Lng}(c)=1`$ も含めて閉じている。

- **[R]** 結論の $`\leq_{\textrm{PS}}`$ 単調形への強化と、$`S`$ の直下の最大元へ進む段が要らない

  原文は $`M\lt_{\textrm{PS}[]}N`$ を得たあと、内側の帰納法の仮定と 基本列的順序が推移性 で
  「任意の $`N'\in CT_{\textrm{PS}}`$ に対して $`N\leq_{\textrm{PS}}N'`$ ならば $`M\lt_{\textrm{PS}[]}N'`$」と
  結論を強め、さらに $`S`$ の中で $`N`$ の直下にある最大元 $`M'`$ を取って、$`M'\lt_{\textrm{PS}}O\lt_{\textrm{PS}}N`$
...
| (2-a) $`\lt_{\textrm{PS}}`$ の分解 | `ltPS_dest_idx`（`ltPS_dest` 経由） | `lean/Bijectivity/12a-lex-toolkit.lean` |
...
| $`\leq_{\textrm{PS}}`$ と $`\lt_{\textrm{PS}}`$ の合成 | `lePS_ltPS_trans` | 同上 |
| (5-k) 挟み撃ち | `sandwich_agree` | 同上 |
| (5-l) $`(N'[m]_j)_{j=0}^{j_1}\lt_{\textrm{PS}}N`$ | `take_last_ltPS` | 同上 |
