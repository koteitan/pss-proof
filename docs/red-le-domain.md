...
$\textrm{Br}$ の降順性の関係」(line 1422) の前提と同型。実証（`python/red_anchor2.py`）:

- 標準形ソースの先祖係留切片: 5系すべて成立（2694 切片、失敗0）。
- 簡約+単項ソースの先祖係留切片: 5系すべて成立（103 切片、失敗0）。

## 4. use-site 監査（§7 が当てる $N$ は係留切片か）

`corrections.md`/§7 の各使用箇所で5系が適用される $N$ を確認（`python/` で検証）:
