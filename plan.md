...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
      ...
      - 🚨 `wf RPrel`(principal lessBP 整礎)
        ...
        - 🚨 unbounded-depth chain排除→level jump 1本化〔r51 で構造確定〕
          ...
          - 🚨 wfs_level_jump→collapse_core再尖鋭化〔r52 で構造刷新〕
            ...
            - 🚨🤖 pbody-acc本体(非可述、route=distinguished sets 4-8R)〔wfc_pbody_acc=OT+dfree principal の body が RTrel-acc。staged 形 wfc_jump_step n でも可。distinguished sets(Buchholz-Schütte)を HOL 内包で構成するのが本命〕[r0]
  - 🚨 §8.1 条件 (I) の下での展開規則
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（基本列の降下性）〔`fseq_descend`〕
      ...
      - 🚨 残=census 4点(⛔ OTint/OTpred/OTmulti/TVall)〔🚩**循環警告**: r31 orx_ は otx slots を {resid,multiD} から discharge するが multiD は今や otx slots から接地(apx_multiD_grounded)→**orx 経由の接地は循環で不可**。独立 closure の自然な源=per-condition 値形(§8.5 masterkey/scbdec クラスタ)。OT柱項目(OTint/OTpred/OTmulti)は §8.7 補題（Trans が標準形を保つこと）配下で追跡〕[r0]
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔残=R2 dstep+R3 newOT/gbt+帰納組立〕
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨🤖 otIII/otIV脚+{PredNp,Lpv,L1v}(§8.2終切片base3点)〔otx3_OTint_slot は III/IV を r35 形で、nadm 脚は {PredNp,Lpv,L1v} を carry。III/IV=cpx/cnv mnform 値形からの同型攻め(condV の otx3 パターン踏襲)。PredNp/Lpv/L1v は §8.2 終切片 base 3点=既存 bpx/hqx/kyx 機構から discharge できる見込み〕[r0]
      - 🚨 OTpred(with-parent Pred corner OT-step)〔r53 で大幅reduce〕
        - ✅ keystone 4-clause分析(fresh閉+multi/corner閉、opx_)〔r53: fresh clauses(1)/(2)=**真に閉**(`opx_OT_removal`=最終 deposit principal 除去は OT_B 保存、rgx_gbt_prefix_restrict+descP_append1、無残差)。multi Pred=mono 最終成分へ還元(junction=m_7_3_Pred_Trans_descend で無料)。(0,0)/Lng=2 corners 閉。r52 draft 修理: 1/Suc-0 正規化・butlast 連言 simp 非合流(metis pin)・BP型 replicate snoc・butlast_append 結合〕[r2][0.262][0.30h]
        - 🚨🤖 DEEPOT(deepened clauses)+NOBR(branch-free mono)〔deepened keystone clauses(3)/(4)の OT 保存=DEEPOT、branch-free mono=NOBR。ddx/hgx/tsx の deepened 分析が素材〕[r0]
      - 🚨 OTmulti(multi P成分lift OT-step)〔r53 で大幅reduce〕
        - ✅ operB局所化+replicate regime閉(opx_OTmulti)〔r53: r52 prompt の m_6_8 隣接head比較は不要だった——`opx_OT_append_rep`+operB 局所化(`opx_operB_snoc_local`)で junction-free(脚I/II/VI)、replicate regime=m_8_1_Trans_replicate_pred_condI。非循環監査済(apx_AP_grounded/orx_ 不使用)〕[r2][0.262][0.30h]
        - 🚨🤖 ordIntC(内部exchange脚のleBT弱形)〔III/IV/V 内部脚にのみ要る新 sharp 残差=descP junction 用の leBT 弱形。exchange 定理群(閉済)の leBT 版読み替えで discharge できる見込み〕[r0]
