        ...
        - 🚨 unbounded-depth chain排除→level jump 1本化〔r51 で構造確定〕
          ...
          - 🚨 wfs_level_jump→collapse_core再尖鋭化〔r52 で構造刷新〕
            ...
            - 🚨 pbody-acc本体(distinguished sets route)
              ...
              - 🚨 collapse補題(D3)+ladder接続(残=単一 wcl_upper、外部級)〔r63 front B: wds_collapse は wds_collapse_iff_wf で wf RPrel(=wfc_pbody_acc=[Buc1]2.2、ψ_0(ε_{Ω_ω+1}))と厳密同値=論理強度削減不可・一撃不可。11緑wcl_で攻め面鋭利化: wcl_min_bad_secured/wcl_core_iff_no_bad_secured/wcl_min_bad_body_acc/wcl_lower_head_bad_exists/wcl_accfrag_distinguished_of_core+downclosed/relacc/prog。**r64 front B クローズ前進**: r63 の minimality tension(size-min→body-acc だが head-<v premise 無/head-min→premise 有だが body-acc 無)を **辞書式 (wfj_hd,wfs_szP) minimal-bad witness で解消**(wcl_min_bad_lex、head 優先)。minimal-bad body の head-≤n segment を **無条件 discharge**(head<n=clause i/head=n=proper subterm は size小=clause ii)→[Buc1]2.2 を **単一残差 wcl_upper**(head>n の ψ_n-shielded collapsed 係数が RPrel-acc)に固定。closing thm wcl_wf_of_upper + wcl_upper_of_wf/wcl_upper_iff_wf(**wf RPrel と厳密同値=faithful reduction、overshoot 無**)+corollary wcl_collapse_of_upper/wcl_buc1_2_2_of_upper。残=wcl_upper 本体=Buchholz-Schütte Hauptlemma(head>n 係数は subterm/size 降下でも head-minimality でも到達不可、distinguished-set tower 帰納 on n=[Buc1]§5 fund.seq. 要、wcl_accfrag_distinguished_of_core は現状 collapse core 仮定=循環で未接続)。推定 2-4 round(tower が level 毎 feed 可なら)or 外部開放〕[r2][0.360][0.52h]
  - 🚨 §8.1 条件 (I) の下での展開規則
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨 otIII/otIV脚(一ブロック塔4事実へ還元)〔r55 front A(Fable、緑到達後 spend limit で死亡→salvage、heap-verified 緑 draft を Opus が回収統合、c6a41b0→9b97c72)。otIII/otIV=host-level deep-insertion を **一ブロック塔の isOT_BP/setle 計算 4事実** {OTA1(condIII/IV共通 d4vx_ins 塔の isOT_BP)、SETLE1(同塔の GBT setle)、IVADMEQ(condIV adm=transJm1 隅)、IVNP(condIV no-parent 隅)} へ還元。machinery=oi5_OTint_condIII/condIV(oi4_OTint_slot_IIIIV に注入)。cpx_condIII_mnform/cnv_c2_shape_condIV/oi5_regime/oi5_d4vx_* 経由。census 系(oi5_termination_census)は {4事実,DEEPOT,NOBR,FINRC} modulo〕
          ...
          - 🚨 {OTA1,SETLE1}(condIII/IV共通塔isOT_BP/setle計算)〔d4vx_ins s0 (e-1) b0 (bpHeadT(Trans(Pred(s84x_N P)))) の isOT_BP と GBT setle。両者 STEP-0 で真(899/899)。**setle_body は偽(u=0)→naive otx3_core route 死、otx3_core_tri+tri0 を使う**〕
            ...
            - 🚨 SETLE1_ltJ(r64: 右spine L1/L2反証、残=左端 bpHeadV(body)≥v1)〔r62 sub-brick(i)ox7_align3_track+(iii)ox7_scbext_leBT_hole。head-bound(ii)「A0 head≤ub」反証。r63: ox7_RightNodes_body_ge_v1 緑(右spine 下界)。**r64 front A=r63 の L1/L2 route を反証(negative)**: STEP-0(python census filter 130 host)で ox6_setle_scbext_restr の restricted spineH(leBT t′ X1、t′=A1 の右spine 祖先)は **45/130 で偽・逆向き lessBT X1 t′ 成立**。構造的事実2つ: (1)**L1/L2 の向きが逆**——通過 host では first lex diff は position 0(**第1 principal=左端**)、t′ 外head=ub=v1-1 < X1 外head=v1 で t′ が小=t′<X1。task の主張(t′ head≥v1>ub=X1 head)は無用な逆 leBT X1 t′ を導く。(2)**pass ⟺ bpHeadV(body)≥v1**——比較は body の **第1 principal(左端)head** が支配、右spine ではない。scb_kind1_def(pss_paper 898-903)は RightNodes 節のみ→r63 の RightNodes≥v1 は bpHeadV(body) を **証明不可能**=右spine machinery は本義務に本質的に不十分。b1x_setle 直検も 43/130 で偽(=それら M は real ST_PS∩PT_PS host でない、filter が census より緩い)+2 host は goal-holds-but-spineH-fails(lbA は X1 でなく **より深い GBT u X1 元 D_v1 0** に支配)。**真の残差=左端 head 下界 bpHeadV(body)≥v1**(r63 右spine 下界の mirror、自然な source は Trans M∈OT_B の G-condition、scb_kind1 ではない、証明可能性 open) OR ox6_setle_scbext_restr の spineH 結論を「∃y∈insert X1 (GBT u X1). leBT t′ y」に **再スレッド**(X1 でなく deeper GBT 元で支配)。⚠️2 曖昧 host(M=[(0,0),(1,0),(1,1),(2,1),(1,1),(2,1)]+その(2,2)variant)が real census host なら spineH は real host 上でも偽→engine 再スレッド必須、non-real なら bpHeadV(body)≥v1 で閉じる。既存 GBT/RightNodes→leBT bridge 無〕[r6][1.131][2.56h]
