...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（基本列の降下性）〔`fseq_descend`〕
      ...
      - 🚨 残=census modulo{OTA1,SETLE1,IVADMEQ,FINRC}(oi7_termination_census、両柱)〔r55: oi6_termination_census が両停止性の柱(∀M∈ST_PS. Trans M∈OT_B / 基本列 lessBT 降下)を **{OTA1,SETLE1,IVADMEQ,IVNP,FINRC} のみ modulo** に成立。DEEPOT/NOBR は od4(front B)で discharge 済、otIII/otIV は 4事実へ還元済。残る5残差=一ブロック塔 isOT_BP/setle 計算4本(OTA1/SETLE1/IVADMEQ/IVNP)+FINRC(nth-artifact、§8.2 fin と同格)。🚩**循環警告**(過去): orx_ 経由 multiD 接地は循環不可、独立 closure は per-condition 値形(§8.5 masterkey/scbdec)。OT柱は §8.7 補題（Trans が標準形を保つこと）配下で追跡〕[r0]
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨 otIII/otIV脚(一ブロック塔4事実へ還元)〔r55 front A(Fable、緑到達後 spend limit で死亡→salvage、heap-verified 緑 draft を Opus が回収統合、c6a41b0→9b97c72)。otIII/otIV=host-level deep-insertion を **一ブロック塔の isOT_BP/setle 計算 4事実** {OTA1(condIII/IV共通 d4vx_ins 塔の isOT_BP)、SETLE1(同塔の GBT setle)、IVADMEQ(condIV adm=transJm1 隅)、IVNP(condIV no-parent 隅)} へ還元。machinery=oi5_OTint_condIII/condIV(oi4_OTint_slot_IIIIV に注入)。cpx_condIII_mnform/cnv_c2_shape_condIV/oi5_regime/oi5_d4vx_* 経由。census 系(oi5_termination_census)は {4事実,DEEPOT,NOBR,FINRC} modulo〕
          ...
          - 🚨 {OTA1,SETLE1}(condIII/IV共通塔isOT_BP/setle計算)〔d4vx_ins s0 (e-1) b0 (bpHeadT(Trans(Pred(s84x_N P)))) の isOT_BP と GBT setle。両者 STEP-0 で真(899/899)。**setle_body は偽(u=0)→naive otx3_core route 死、otx3_core_tri+tri0 を使う**〕
            ...
            - 🚨🤖 ltJ再スレッド+OTA1_ltJ/SETLE1_ltJ組立〔**構造発見(r57)**: census OTA1/SETLE1 は forall-P だが oi5_OTint_IIIIV_hp は ltJ host でのみ適用(condIII=常時ltJ/condIV-admeq→IVADMEQ/condIV-not-admeq→cnv_condIV_ltJ/no-parent→IVNP)。forall-P が唯一の障害(admeq で ltJ 偽=A0-tower mnform 不在)。**FIX: ltJ を再スレッドして census を {OTA1_ltJ,SETLE1_ltJ,IVADMEQ,FINRC} に**(oi8_ chain=oi5_OTint_IIIIV_hp/condIII/condIV/census を ltJ付き OTA1/SETLE1 に書換、application sites に ltJ を渡すだけ)。OTA1_ltJ 組立=ot1_OTA1_from_bricks[OF fX1 fA1 fX2 b0RP X1OT X2OT loPe hiPe nub o1 o2 tri0](tri0=済/o1,o2=base0,base1/donor=fseqX+e4x/nub=newOT_body)。newOT_body=Trans(s84x_N P)∈OT_B slice輸送(s84x_N=§8.4 終端mono slice、∈ST_PS)→od4_OTpred_mono→bpHeadT OT principal→b1x_GBT_antitone。SETLE1_ltJ=top-anchored GBT-descent(X1固定、base1+body driver BE、oix_condV_adm_setle 移植)〕[r1][0.257][0.65h]
          ...
          - 🚨🤖 IVADMEQ(condIV adm-eq隅=標準condIVの主OT内容、残差{d1,d2,d3,HB,NEWOT,SETLE})〔Adm P (s84x_jm2 P)=transJm1 P。**標準condIVの47/48がこれ**。r57 front B: ltJ-free admeq mnform c4cx2_condIV_mnform_of_slice(clean hole transT2 P=Trans(Pred P)の部分項)で ot2_IVADMEQ_of_pkg(OT step→{NEWOT,SETLE}+mnform pkg、oix_transportD[OF otx3_transport])+ot2_IVADMEQ_mod(mnform pkg を host-only 化→残差{d1,d2,d3,HB,NEWOT,SETLE})+ot2_dins_addBT_of_shape(surgery→nested+B rewrite=engine 基盤)。残=condV流 newOT/setle engine(oix_condV_adm_newOT/setle wip)を transT2 塔へ移植+d1/d2/d3 slice regime(m_8_4_slice_scb_part1/w84x_d2/c4wx_d3)+HB〕[r2][0.297][0.65h]
