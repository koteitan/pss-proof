        ...
        - 🚨 unbounded-depth chain排除→level jump 1本化〔r51 で構造確定〕
          ...
          - 🚨 wfs_level_jump→collapse_core再尖鋭化〔r52 で構造刷新〕
            ...
            - 🚨 pbody-acc本体(distinguished sets route)
              ...
              - 🚨 collapse補題(D3)+ladder接続(wcl_ 攻め面済、残=wds_collapse 本体=外部級)〔r63 front B: wds_collapse は wds_collapse_iff_wf で wf RPrel(=wfc_pbody_acc=[Buc1]2.2、ψ_0(ε_{Ω_ω+1}))と厳密同値=論理強度削減不可・一撃不可。11緑wcl_で攻め面鋭利化: wcl_min_bad_secured(szP-minimal 非acc principal は G_v-trace 全secured=標準entry)/wcl_core_iff_no_bad_secured(残差=secured principal 全acc)/wcl_min_bad_body_acc(witness の body も RTrel-acc)/wcl_lower_head_bad_exists(head<v CEX 上に)/wcl_accfrag_distinguished_of_core+downclosed/relacc/prog(acc fragment=具体 maximal v-distinguished)。残=head-<v lower segment collapse〕[r1][0.200][0.30h]
  - 🚨 §8.1 条件 (I) の下での展開規則
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨 otIII/otIV脚(一ブロック塔4事実へ還元)〔r55 front A(Fable、緑到達後 spend limit で死亡→salvage、heap-verified 緑 draft を Opus が回収統合、c6a41b0→9b97c72)。otIII/otIV=host-level deep-insertion を **一ブロック塔の isOT_BP/setle 計算 4事実** {OTA1(condIII/IV共通 d4vx_ins 塔の isOT_BP)、SETLE1(同塔の GBT setle)、IVADMEQ(condIV adm=transJm1 隅)、IVNP(condIV no-parent 隅)} へ還元。machinery=oi5_OTint_condIII/condIV(oi4_OTint_slot_IIIIV に注入)。cpx_condIII_mnform/cnv_c2_shape_condIV/oi5_regime/oi5_d4vx_* 経由。census 系(oi5_termination_census)は {4事実,DEEPOT,NOBR,FINRC} modulo〕
          ...
          - 🚨 {OTA1,SETLE1}(condIII/IV共通塔isOT_BP/setle計算)〔d4vx_ins s0 (e-1) b0 (bpHeadT(Trans(Pred(s84x_N P)))) の isOT_BP と GBT setle。両者 STEP-0 で真(899/899)。**setle_body は偽(u=0)→naive otx3_core route 死、otx3_core_tri+tri0 を使う**〕
            ...
            - 🚨 SETLE1_ltJ(§7.4 RightNodes(body)≥v1済、残=spineH cross-hole lex)〔r62 sub-brick(i)ox7_align3_track+(iii)ox7_scbext_leBT_hole。head-bound(ii)「A0 head≤ub」反証→真=RightNodes(body)≥v1。r63: ox7_RightNodes_body_ge_v1 緑(想定より易、oi5_IIIIV_pkg の k1 scb_kind1+rnsub_RightNodes_t0_lastv、新empirics不要)。残=spineH(ox6_SETLE1_reduce_restr 唯一残差=cross-hole lex leBT t′ X1、hole経由不可(leBT A0 X0 偽)→wrapper first-diff)。要新2補題 L1(tx′∈GBT u X1+hole-flat→right-spine subterm+head bound、hole D_ub 一意で sc を s0 suffix pin)+L2(lex first-diff: 外wrapper 一致で片方 head≥v1>他方ub なら leBT)。既存 GBT/RightNodes→leBT bridge 無(b1x_GBT_size/trans/antitone のみ)。condIV leftDj0枝 t′/X1 left-prefix 非共有=re-parsed-flat 要〕[r5][0.971][2.34h]
