  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔残=R2 dstep+R3 newOT/gbt+帰納組立〕
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨🤖 otIII/otIV脚(condVのotx3パターン踏襲)〔cpx_condIII_mnform/cnv_c2_shape_condIV の値形で塔を組み otx3_transport(定理)に differing donors を渡す——condV と同型の作業〕[r0]
      - 🚨 OTpred(with-parent Pred corner OT-step)〔r53 で大幅reduce〕
        ...
        - 🚨🤖 DEEPOT(deepened clauses)+NOBR(branch-free mono)〔deepened keystone clauses(3)/(4)の OT 保存=DEEPOT、branch-free mono=NOBR。ddx/hgx/tsx の deepened 分析が素材。r54 front は月次limitで死亡、**壊れた draft(4エラー)が wt-s4b に残置**(/tmp/pss-r54-salvage-wt-s4b.log)——次回修理〕[r1][0.233][0.21h]
      - 🚨 OTmulti(⛔ OTint、oc4_で{OTint,TVall}のみ化)[r3][0.495][0.51h]〔r53 opx_OTmulti(operB局所化、junction-free 脚I/II/VI、replicate=m_8_1)[r2][0.262][0.30h]+r54 oc4_ordIntC **クローズ**(strict lessBT を全6枝で閉じた exchange 定理群から: III=cpx(2)/III-noParent=npx+3.2a/IV=cnv(2)/IV-noParent=dpx+le_less_trans/V=m_8_5 adm+atx nadm)+`oc4_OTmulti`=slot modulo {OTint,TVall} のみ。TVall 済→実質 ⛔OTint。oc4_ には **L1v クローズ**(nf2x_L1v∘atx_c2L1、未合成だった組合せ)と master census v2(7残差)も〕
