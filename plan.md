...
### 残タスク一覧（2026-07-13 時点、優先順）
1. **`Red(Red M) ∈ RT_PS`**(🤖稼働中、wt-y3)〔§7 の残差 4命題+1系がすべてこれ1本に依存。`y3s_RedStab_of_Red2` 経由。既知: 非複項なら `idem_nonmulti` で即座、**難所は「Red M が複項かつ非簡約」=660/7380** で稀ではない。route案=`m_6_6_reduced_iff_cond`(RedCondA/B 特徴づけ、無条件)で係数条件を直接検証(Red の再帰に関する帰納は閉じない=A15)。28000例+乱択1000で反例0〕
2. **§8.4/§8.5 の未転記 scb 分解クラスタ 6+2 補題**(🤖稼働中、wt-s4c)〔**原文カバレッジ最大の穴**。deferred の理由=言明が `Trans` 再帰の内部記号(c1/c2/t2/t3/t4/v/s1/b1/s0/b0/j0/jm1)に言及するが、それらは `let` 束縛で関数として露出していない。**まず内部記号を top-level 関数として露出**(`y3i_*`、既存 `transJ0`/`transC1`/`transC2`/`transT2`/`s84x_*`/`d4vx_*` を再利用)→忠実性補題→転記→証明。内容の多くは layerB に別名で証明済みの可能性(`cpx_`/`cnv_`/`c4dx_`/`c4cx_`/`crx_`/`d13x_`/`w84x_`/`e5x_`)〕
3. **§8.4 補題（基本列の基本性質）(2)**〔原文は真(130/130 非空虚)。障害は構造的: part(1) の右辺が「`M[n]` に追加ブロックの1エントリだけ足した中間列」で `M[m]` の形をしていないため、既存の `Trans` 閉形式(全て基本列で添字づけ)が届かない。次手=1エントリ拡張の readback 補題(`fOn` が index n-1 で名指す 0_B-seeded 塔レベルに着地することを示す)→`m_7_flatBT_inj`〕
4. **§8.3 命題の結論 (1)(2)(3)**〔未転記。(4)=降下 のみ `y5_8_3_TransCondII_oper_descend` で証明済。2 と同じく内部記号の露出が要る〕
5. **§6 命題（単項性と Red の関係）の M_{1,0}=0**〔原文は真(訂正 A42 を取り下げ済)。Isabelle の nat 減算で literal 形が degenerate になるため `m_6_5_monoT_Red_m10pos` が `0 < entry M 1 0` を仮定している。Z 読みを別定義で入れるか、m10=0 を別途処理〕
6. **§7 系（Mark と NextAdm の関係）の T_PS 版**〔結論は移送できるが仮定(nextAdm/leR/Marked)が Red 不変でない(A4)ため移送不可。`y3s_7_4_Mark_nextAdm_TPS_reduct`(仮定を reduct で読む形)は証明済。原文の形が真かどうかは未判定——**偽の可能性あり**(A45/A46 と同じ機構)。まず経験検証すべき〕
7. **pss_paper の偽命題 sorry を訂正形へ書換**〔取り下げ15件を除いた真の偽命題のみ。偽命題の sorry は False を導ける地雷(現在引用ゼロ)〕
8. **stale 引用1件**〔`layerB/pss_wip.thy:27070` の `m_8_7_toplevel_OT_tail_annihilate` が sorry 版 `buc1_2_2_OT_B_wf` を使用。同命題は `y4_buc1_2_2_OT_B_wf` として証明済だが layerC にあるため参照不可。b1x_ ブロック同様の前方移動か layerC 再証明〕
