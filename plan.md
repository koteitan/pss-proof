...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔残=R2 dstep+R3 newOT/gbt+帰納組立〕
      ...
      - 🚨🤖 deep-insertion OT所属(keystone{resid,multiD}; pcompPrefix反証→multiD route dead; resid=Fable攻撃中)〔r35: 4脚→単一 keystone {resid,multiD}(`orx_OTint`)。r39: multiD を pcompPrefix に還元したが 🚩**r40(部分)で pcompPrefix を ST_PS 上で反証**(r39「771/771」も浅corpus偽陽性、`otkx_multiD_of_pcomp_prefix` は dead 条件補題)→**multiD は別 route 要**。**resid**(R2 descP head-step+R3 [Buc1]OT2、13+反証、最難)を **r40b で Fable-only 一点投入中**(3連言 C1 isOT_BP/C2 head-step leBT/C3 GBT[Buc1]OT2、deep検証先行)。反証registry: operB(term z) DEAD、最右spine scb-substitution 偽、pcompPrefix 偽〕[r5]
