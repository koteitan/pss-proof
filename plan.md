...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
  ...
  - 🚨 §8.2 強単項性
    ...
    - 🚨 命題（条件 (II) か (IV) の下での終切片と $\textrm{Trans}$ の関係）〔`p_8_2_condIIIV_terminal_slice_Trans`(pss_paper 1624)=condVのVE'のcondII/IV版〕
      ...
      - 🚨 VE3/VE4
        ...
        - 🚨🤖 bridgesU 終切片readback(残=keystone RightNodes値 cpU/chainOK+scb readoff)〔bridgesU=`Trans(seg N 0(FN!LastStep-1))=D_{e1 0}t1`∧∃t2.`Trans(seg N(Joints!last)(Lng-1))=D_{e1 j0'}(t1+_B t2)`(t2≠0)。=原文§8.2 終切片 term-readback(condV a0x_base_VE 類似)。128/128 deep(29 BASE+99 STEP、初 STEP 網羅)。🚩r39発見: **keystone `m_8_2_subexpr_component_Pred` 自体が非無条件**=`RightNodes(Trans(Pred M'))!1` 値(cpU/branchParAll/chainOK)残差を持つ。bridgesU の壁は終切片幾何でなくこの §7.4 RightNodes 値+scb-context read-off〕[r1]
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔残=R2 dstep+R3 newOT/gbt+帰納組立〕
      ...
      - 🚨🤖 deep-insertion OT所属(keystone{resid,multiD}; multiD→pcompPrefix還元済、resid最難)〔r35: 4脚→単一 keystone {resid,multiD}(`orx_OTint`)。r39: **multiD を surgery-free 還元**(`otkx_multiD_of_pcomp_prefix`+`otkx_leBT_Trans_of_prefix`)→単一 §6.2 事実 **pcompPrefix**=`drop(Pcut N) N`(最終P成分)が `P N!(Lng(P N)-2)`(2番目)の接頭辞。771/771 deep(**要 ST_PS**、非標準 multiT で偽 CEX (2,2)(1,1))。multiD が近い方。**resid**(R2 descP head-step+R3 [Buc1]OT2、13+反証)が最難で遠い。反証: operB(term z) DEAD、最右spine scb-substitution 偽〕[r4]
