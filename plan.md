...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（基本列の降下性）〔`fseq_descend`〕
      ...
      - 🚨 残=census modulo{A0OT,SETLE1_ltJ,IVADMEQ{d1,d2,d3,HB},FINRC}(oi8_census_via_A0OT、両柱)〔r55: oi6_termination_census が両停止性の柱(∀M∈ST_PS. Trans M∈OT_B / 基本列 lessBT 降下)を **{OTA1,SETLE1,IVADMEQ,IVNP,FINRC} のみ modulo** に成立。DEEPOT/NOBR は od4(front B)で discharge 済、otIII/otIV は 4事実へ還元済。残る5残差=一ブロック塔 isOT_BP/setle 計算4本(OTA1/SETLE1/IVADMEQ/IVNP)+FINRC(nth-artifact、§8.2 fin と同格)。🚩**循環警告**(過去): orx_ 経由 multiD 接地は循環不可、独立 closure は per-condition 値形(§8.5 masterkey/scbdec)。OT柱は §8.7 補題（Trans が標準形を保つこと）配下で追跡〕[r0]
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨 otIII/otIV脚(一ブロック塔4事実へ還元)〔r55 front A(Fable、緑到達後 spend limit で死亡→salvage、heap-verified 緑 draft を Opus が回収統合、c6a41b0→9b97c72)。otIII/otIV=host-level deep-insertion を **一ブロック塔の isOT_BP/setle 計算 4事実** {OTA1(condIII/IV共通 d4vx_ins 塔の isOT_BP)、SETLE1(同塔の GBT setle)、IVADMEQ(condIV adm=transJm1 隅)、IVNP(condIV no-parent 隅)} へ還元。machinery=oi5_OTint_condIII/condIV(oi4_OTint_slot_IIIIV に注入)。cpx_condIII_mnform/cnv_c2_shape_condIV/oi5_regime/oi5_d4vx_* 経由。census 系(oi5_termination_census)は {4事実,DEEPOT,NOBR,FINRC} modulo〕
          ...
          - 🚨 {OTA1,SETLE1}(condIII/IV共通塔isOT_BP/setle計算)〔d4vx_ins s0 (e-1) b0 (bpHeadT(Trans(Pred(s84x_N P)))) の isOT_BP と GBT setle。両者 STEP-0 で真(899/899)。**setle_body は偽(u=0)→naive otx3_core route 死、otx3_core_tri+tri0 を使う**〕
            ...
            - 🚨🤖 A0OT(=isOT_BT(bpHeadT(Trans(Pred(s84x_N N)))))〔**crack(r59策定)**: od4_OTpred_mono は ST_PS を 809-811行(MR:RT_PS/MT:T_PS/MP:PT_PS 導出)にしか使わない。本体は MR/MP/mono/hostOT のみ。**slice は RT_PS∩PT_PS∩monoT で既知**(monoT wip:78640)なので od4_OTpred_mono_RT(RT_PS+PT_PS版)にすれば ST_PS ブロッカー回避。slice RT_PS=§8.4 slice構成/1<Lng=wip:58724 LN+jm3lt/Trans(slice)∈OT_B=princN+k1+m_8_7_OT_scb_recursive on ihOT→Trans(Pred slice)∈OT_B→bpHeadT OT principal。r59 front A@wt-s4c〕[r0]
            - 🚨🤖 SETLE1_ltJ(top-anchored GBT descent)〔dedicated GBT descent(tri0 brick に還元不可=triG≠setle)。GBT u A1⊆{y.leBT y X1}∪GBT u X1、TOP anchor X1固定(local-tLo は深levelで失敗)。base=base1(A0<X1)+body driver BE(∀y∈GBT u A0.leBT y X1、r58真)。descent=IH+b1x_GBT_trans、spine-body domination 補題。oix_condV_adm_setle(wip:110623)移植。r59 front B@wt-y3〕[r0]
          ...
          - 🚨 IVADMEQ(condIV adm-eq隅=標準condIVの主OT内容、残差{d1,d2,d3,HB})〔Adm P (s84x_jm2 P)=transJm1 P。**標準condIVの47/48がこれ**。r57 ltJ-free admeq mnform c4cx2_condIV_mnform_of_slice(clean hole transT2 P)+ot2_IVADMEQ_of_pkg/_mod/ot2_dins_addBT_of_shape で残差{d1,d2,d3,HB,NEWOT,SETLE}に。**r58 front B: NEWOT engine完成(ot2_tower_newOT/ot2_NEWOT_of_pkg/ot2_tower_inv、TRI route=otx3_core_tri+otx3_pOT、SETLE不要と判明)→残差{d1,d2,d3,HB}**(condIV cfbx_reg regime facts regS/regSP、admeq で jm3=transJm1 forced の corner注意)。次round攻略〕[r3][0.550][1.30h]
