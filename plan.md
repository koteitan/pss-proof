...
- 🚨 §7 Buchholzの表記系への翻訳
  ...
  - 🚨 系（$\textrm{Mark}$ と NextAdm の関係）の $T_{PS}$ 版〔**原文カバレッジ最後の1件**。詳細は上の残タスク一覧 1 番〕
    - ✅ (F) $\le_M$ の Red 保存 + engine 内側列の無仮定緩和[r3][0.291][3.60h]〔`y3w_Red_le0`: `le0 M a b ⟹ le0 (Red M) a b`、全 T_PS 無仮定。engine の内側列 j は**何の仮定も要らない**(1271/0, 2845/0)——外側列 j0 の `adm` だけが load-bearing で、それが (C4) の死因〕
    - ✅ 反射半（$j = j_0$）[r1]〔`y3y_Mark_princ`: `N ∈ RT_PS ⟹ dfree_BT (Mark N m) ∧ (Mark N m = 0_B ∨ isPTB_str (flatBT (Mark N m)))`。**許容性も Marked 性も祖先関係も不要**。Lng N 強帰納 × Mark 再帰3分岐(1列/単項/複項)。複項枝は最後の P 成分へ降下(`m_6_6_P_reduced` で簡約性保存、`Pcut_le` で真に短縮)。死んだ Red²-adm ルートが死んだケースをちょうど覆う〕
    - 🚨🤖 非反射半（$j < j_0$）＝ engine の差し替え[r0]〔既存 engine (`Mark_nest_common_marked`/`m_7_3_Trans_Mark_MarkedB`/`Mark_MarkedB_nest`) は reduct の Marked 性を要求する。**経験的に `(M,j0) ∈ Marked` だけで共通 scb 位置が一意に決まる(419/419)**——内側列 j には仮定不要。census は entry-bound を必ず広げてから(≤8)〕
    - 🚨🤖 $T_{PS}$ への移送と本体の組み立て[r0]〔`y3s_Mark_funpow_Red`(`Mark M i = Mark (Red² M) i`、常時成立) + `y3r_RED2`(`Red² M ∈ RT_PS`) で RT_PS 版を T_PS へ運ぶ。移送できないのは engine の**仮定**であって主題ではない。最後に `y3t_7_4_Mark_nextAdm_TPS` を ML 監査リストへ登録〕
