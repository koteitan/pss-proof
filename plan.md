...
- 🚨 §8 停止性 〔r18: capstone `m_8_termination_modulo_CF`=停止性は CF-META(原典Pred同時帰納)+`buc1_2_2`整礎 の2点に崩壊。CF が唯一の Fable target〕
        ...
        - 🚨 bridgesU 終切片readback(readoff⟺VE34判明→残=article {BASE,STEP} back-peel)〔bridgesU=原文§8.2 終切片 term-readback。128/128 deep。r39発見の keystone RightNodes値残差を r40 で DISSOLVED し dispatcher を「readoff」残差 `kyx_VE34_of_DT_modReadoff` に還元していた〕
          ...
          - 🚨 {BASE,STEP} back-peel(article Pred-induction on j1-TrMax)〔r30-r43抵抗の壁。r45でSTEP側が単一方程式に崩落〕
            ...
            - 🚨 BASEf(same-head branch-run帰納)〔r46 で run-peel 機構は完全機械化、残=非許容j0' Mark-readback 族3点〕
              ...
              - 🚨🤖 {TSPIN-base,PIN,SPLIT0}(非許容j0' Mark-readback族、tsx機構のbase適用)〔⚠️bfx capstone の TSPIN は r45 の step-guard(cfbx_j1p<Lng-1)を落とした base-host 版——tsx_TSPIN(STEP-context)はそのまま挿さらない。PIN=front-slice pin ∃a.Trans N=D(F+B D a)(run-step では r45 pin 機構で従う、run-base では clause(2) が PIN そのもの、clause(1) は guard+¬adm で排除→残=vg5x_base_transM_form の isleft selector が run 状態を読む同値)。SPLIT0=run-base 自己相似 split。3点とも tsx が STEP で建てた非許容 Mark-readback と同族→co-discharge 狙い。empirics: PIN/SPLIT0/TSPIN-base は r38 bridges corpus 141/141+8/8 で真〕[r0]
  ...
  - 🚨 §8.7 主結果
    ...
    - 🚨 補題（基本列の降下性）〔`fseq_descend`〕
      ...
      - 🚨 残={TOT,TVall}(⛔ OT所属、⛔ §8.2命題)〔TOT=「Trans N∈OT_B」(OT柱、otx dispatch 経由で {OTint,OTpred,OTmulti,deep-insertion slots} に還元済)/TVall=condII tailval=p_8_2_condIIIV=VE34={BASEf}。両方とも別ツリー項目で追跡中——census はこの2点に全てを絞った〕[r0]
      ...
      - 🚨 deep-insertion OT所属(keystone{resid,multiD}、resid真)〔r35: 4脚→単一 keystone {resid,multiD}(`orx_OTint`)。r40b(Fable): **resid を deep検証(真、1468/0、最大Lng43、pcompPrefix と違い偽陽性でない)+4slot分解**。keystone `m_8_7_Trans_OT_step_keystone` の resid slot が強IH+Trans(Pred M)∈OT_B を受領する形に再構築(`rgx_Trans_preserves_OT_of_slots`)。反証registry: operB(term z)/最右spine scb-substitution/pcompPrefix 偽〕[r5]
        ...
        - 🚨 slotTail(C2: leBT q qb、真)〔r44 偽チェック=TRUE(0-fail)。r45: 比較相手は M 内部の前枝 deposit qb であって Pred 残余 qp ではない(r44 が混同)〕
          ...
          - 🚨 devel残差〔r46 で devpair に尖鋭化〕
            ...
            - 🚨🤖 devpair(枝標準性+body readoff枝拡張+D_0 0_B corner)〔devpair=∃N N' n 短い標準列: Trans N=Dpt x q ∧ Trans N'=Dpt hdv qb ∧ (N=N' ∨ N=N'[n])。discharge に要る: (i)body-principal readoff の枝拡張(bpx/tsx 機構、raw from-joint は反証済) (ii)**枝標準性 last(Br M)∈ST_PS——§6.7 支持なし**(m_6_7_standard_prefix は prefix のみ、枝=post-trunk seg の P成分) (iii)D_0 0_B corner witness((0,0)(0,0)∈ST_PS の具体 oper 計算)〕[r0]
