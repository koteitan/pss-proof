    ...
    - 🚨 補題（$\textrm{Trans}$ が標準形を保つこと）= OT所属〔残=R2 dstep+R3 newOT/gbt+帰納組立〕
      ...
      - 🚨 OTint(値形OT-step)〔r51 で condV 完了、残=transport 核+III/IV〕
        ...
        - 🚨 otIII/otIV脚(一ブロック塔4事実へ還元)〔r55 front A(Fable、緑到達後 spend limit で死亡→salvage、heap-verified 緑 draft を Opus が回収統合、c6a41b0→9b97c72)。otIII/otIV=host-level deep-insertion を **一ブロック塔の isOT_BP/setle 計算 4事実** {OTA1(condIII/IV共通 d4vx_ins 塔の isOT_BP)、SETLE1(同塔の GBT setle)、IVADMEQ(condIV adm=transJm1 隅)、IVNP(condIV no-parent 隅)} へ還元。machinery=oi5_OTint_condIII/condIV(oi4_OTint_slot_IIIIV に注入)。cpx_condIII_mnform/cnv_c2_shape_condIV/oi5_regime/oi5_d4vx_* 経由。census 系(oi5_termination_census)は {4事実,DEEPOT,NOBR,FINRC} modulo〕
          ...
          - 🚨🤖 {OTA1,SETLE1}(condIII/IV共通塔isOT_BP/setle計算)〔d4vx_ins s0 (e-1) b0 (bpHeadT(Trans(Pred(s84x_N P)))) の isOT_BP と GBT setle。r56 front A(Opus): 両者 STEP-0 で真(899/899 condIII/IV host)。緑ブリック2本 commit(otx3_core_tri=setle-free transport で hole-level tri0 を直接取る/ot1_OTA1_from_bricks=OTA1 を {donor-OT,newOT_body,base0/1,tri0} へ還元)。**setle_body は偽(u=0,694/899)→naive otx3_core route は死、otx3_core_tri+tri0 を使う**。残 OTA1=tri0(新 scbext_triG fixed-z wrapper-lift+A0/X1 共有 wrapper 再導出、crux)+newOT_body(Trans(s84x_N P)∈OT_B slice 輸送+s84x_N∈ST_PS)。SETLE1=committed brick に還元不可、top-anchored GBT-descent 別途要(body driver BE: G u A0≤X1 は真)〕[r1][0.257][0.65h]
          ...
          - 🚨🤖 IVADMEQ(condIV adm-eq隅=標準condIVの主OT内容)〔Adm P (s84x_jm2 P)=transJm1 P。**標準condIVの47/48がこれ=condIV OT の主内容(稀な隅ではない)**。障害: 汎用 oi5_OTint_IIIIV_hp は ltJ(jm3<jm1)要だが admeq で偽(jm3=jm1)。だが **ltJ-free の admeq mnform が既存**: c4cx2_condIV_mnform_of_slice(wip:78214)が clean hole transT2 P(Trans(Pred P)の部分項)で flatBT(Trans(P[m])) を与える。既存 admeq 機械(c4dx/c4wx/c4rx)は全て OT-free(descent sandwich のみ)なので、condV 脚が strong-IH 無しで newOT/setle を出す方式(oix_condV_adm_newOT wip:110531/oix_condV_adm_setle 110623)を **d4vx_core/d4vx_ins 塔用に移植**して transT2 hole の一ブロック OT 2事実を作る。transT2∈OT_B は od4_OTpred_final から、host principal は Trans P の部分項から〕[r1]
