theory Support_6_038
  imports Frontier_6_056
begin

text \<open>a3 main: §6.5 命題（\<open>Red\<close>と\<open>Pred\<close>の可換性）\<open>Red (Pred M) = Pred (Red M)\<close> for
  \<open>M \<in> T\<^sub>PS\<close>.  Proved by @{thm [source] Red.pinduct}; per branch:
  zeroT trivial; multiT via @{thm [source] pred_P_decomp} + @{thm [source]
  a3_butlast_concat} + the IH on \<open>last (P M)\<close>; core-trunk via @{thm [source]
  Red_Pred_core_diagSeq}; core-nontrunk via @{thm [source] m_6_6_Br_Pred} + the
  branch IHs; shift (\<open>m\<^sub>1\<^sub>0=0\<close>) via @{thm [source] cdn_Red_shiftRow0_m10z}; \<open>m\<^sub>1\<^sub>0>0\<close>
  via @{thm [source] m_6_5_T4_coreArg_Pred} + the IH on the core-reduce argument.
  Empirically \<open>Red(Pred M) = Pred(Red M)\<close> holds 7380/7380.\<close>

text \<open>BANKED CONDITIONAL: \<open>Red (Pred M) = Pred (Red M)\<close> for \<open>M \<in> T\<^sub>PS\<close>, modulo the
  single core-nontrunk obligation \<open>Hbr\<close>.  All five other @{thm [source] Red.pinduct}
  branches (zeroT, multiT, core-trunk, shift, \<open>m\<^sub>1\<^sub>0>0\<close>) are discharged here; the
  remaining core-nontrunk branch (\<open>m\<^sub>0\<^sub>0=m\<^sub>1\<^sub>0=0\<close>, \<open>TrMax M \<noteq> Lng M - 1\<close>) is the
  open blocker (it needs Joints/FirstNodes/NJ-under-\<open>Pred\<close> transfer plus the
  branch-wise IH; see @{thm [source] m_6_6_Br_Pred}).  Banking this makes the
  whole reduction reusable: discharging \<open>Hbr\<close> finishes \<open>p_6_5_Red_Pred\<close>.\<close>

lemma a3_Red_Pred_cond:
  assumes MT: "M \<in> T_PS"
    and Hbr: "\<And>K. K \<in> T_PS \<Longrightarrow> monoT K \<Longrightarrow> entry K 0 0 = 0 \<Longrightarrow> entry K 1 0 = 0
              \<Longrightarrow> TrMax K \<noteq> Lng K - 1 \<Longrightarrow> Red (Pred K) = Pred (Red K)"
      ...
      next
        ...
        next
          ...
          proof (cases "?m00 = 0 \<and> ?m10 = 0")
            ...
            next
              case tne: False
              \<comment> \<open>Branch 3b: core non-trunk — the banked open obligation \<open>Hbr\<close>.\<close>
              show ?thesis by (rule Hbr[OF MT' mono c0 c1 tne])
            qed
          next
            case nc: False
            show ?thesis
