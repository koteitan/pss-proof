...
- `content.md` line 1026（命題本文）、line 1032（証明）
...
## A6. §6.7 命題（標準形の単項成分が標準形であること）: 証明 1392 行の誤り（未定義変数 $k_0$ を使い、$M'$ の成分が $S_k$ に入ることを帰納法の仮定から導いている）

**位置**
- §6.7 / 命題（標準形の単項成分が標準形であること）
- `content.md` line 1364（命題本文）、line 1390–1392（証明の末尾 case `Lng(P(M')_{J_0}) > 1`）
...
この文の $S_{k_0-1}$ は **$k_0$ が証明中で一度も定義されていない**（設定は $k$ と $k-1$ のみ。1378 行の帰納法の仮定も
「$P(M') \in S_{k-1}T_{\textrm{PS}}^{<\omega}$」）。どちらの読みでも文が破綻する:

- **読み① $k_0 = k$**（添字そのまま）：結論「$P(M) \in S_{k-1}T_{\textrm{PS}}^{<\omega}$」が**偽**。
  $M \in S_kT_{\textrm{PS}}$ の成分は $S_{k-1}$ に入らない（例: $M=(0,0)(1,1)(2,0)\in S_3$ の $P(M)_0=M\notin S_2$)。
  また「帰納法の仮定から $P(P(M')_{J_0}[n]) \in S_{k-1}$」も**偽**：$P(M')_{J_0}\in S_{k-1}$ なので
  $P(M')_{J_0}[n]\in S_k$（基本列で 1 ランク上がる）であり、$S_{k-1}$ の元についての帰納法の仮定は
  これに適用できない。
- **読み② $k_0 = k+1$**（$S_{k_0-1}=S_k$ ＝目標ランクに合わせる）：先頭部分の主張
  「$(P(M')_J)_{J=0}^{J_0-1} \in S_k$」「$P(M')_{J_0} \in S_k$」が、まさに **「$M'$ の成分（先頭部分）が $S_k$
  に入る」** という主張になる。だがこれを「**帰納法の仮定から**」導いているのが誤り — 帰納法の仮定は
  $P(M') \in S_{k-1}$ までしか与えず、$S_{k-1} \subseteq S_k$ は**偽**（下記）。特に $M'$ の**末尾**成分
  「$P(M')_{J_0} \in S_k$」は偽になりうる（末尾成分は $S_{k-1}$ には入るが $S_k$ に持ち上がらない例がある）。

**根因：$S_{k-1} \subset S_k$（単調性）は成り立たない**

忠実モデルで反例多数。$S_kT_{\textrm{PS}}$ の定義の $n \in \mathbb{N}$ を $n=0$ 込みと読んでも $M[0] \ne M$
（otherwise 分岐は $\textrm{take}\,j_0\,M$、退化分岐は $\textrm{Pred}(M)$）なので $S_{k-1} \subseteq S_k$ は出ない。
従って「$P(M')$ の成分（$\in S_{k-1}$）＝先頭部分が $S_k$ に入る」は帰納法の仮定からは出ない。

**正しい証明の骨子（本形式化で採用予定）**

経験的監査（`python/sk_67_audit.py`）で次が判明し、これが原文に欠けている:
1. **(R) $P(X)$ の非末尾成分は行 1 が全て 0**（$X \in S_k$ で違反 0）。
2. **(U) 行 1 が全 0 かつ $c \in S_k$ ならば $c \in S_{k+1}$**（違反 0）。

これらにより、先頭部分（$=P(M')$ の非末尾成分）は (R) で行 1 全 0、帰納法の仮定で $\in S_{k-1}$、(U) で
$\in S_k$ へ**正当に**持ち上がる。末尾は $P(M')_{J_0}\in S_{k-1}\Rightarrow P(M')_{J_0}[n]\in S_k$ の上で構造帰納。

**訂正案**

1392 行を、$M'$ の成分が $S_k$ に入ることを (R)(U)（または同等の「先頭部分の行 1 が 0 であることと、行 1 が 0 の
標準形のランク上方安定性」）を経由して導く形に差し替える。命題の主張は原文どおり（同ランク $S_k$）で**証明のみ補強**。

**形式化での扱い**

`pss_paper.thy` の `p_6_7_standard_P_components`（同ランク $S_k$）を、補助補題（非末尾成分の行 1 全 0 性・
行 1 全 0 のランク上方安定性）経由で証明する。
