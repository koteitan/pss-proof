...
- **各訂正の特定方法**（リポジトリ読者向け）: 記事の**節 (§) と命題名**（「命題（…）」）
  および**訂正対象の本文を逐語引用**することで、公開記事中で直接特定できるようにする。
  これらは記事そのものに存在するため、ローカル作業ファイルが無くても理解できる。
- **注意**: 本文中に現れる `original.html`（記事 HTML）・`content.md`（その整形抽出
  テキスト）への**バイト位置・行番号**は、著者が記事 HTML を編集する際の補助ポインタ
  であり、**ローカル作業コピー（`tmp/`, gitignore 済・本リポジトリには未収録）**を指す。
  記事は著者のものでありここでは再配布しないため、これらの番号は読者が手元に持つとは
  限らない — 特定は上記の節・命題名・引用で行うこと。
...
## A6. §6.7 命題（標準形の単項成分が標準形であること）: 証明が依拠する単調性補題 $S_{k-1}\subseteq S_k$ の省略（と $k_0$ 誤記）
...
1. **未定義変数 $k_0$**: 証明の設定は $k$ と $k-1$ のみ（1378 行の帰納法の仮定は「$P(M') \in S_{k-1}T_{\textrm{PS}}^{<\omega}$」）。
   $k_0$ は導入されておらず、**$k$ の誤記**と読むのが自然。すると各 $S_{k_0-1}$ は $S_{k-1}$ と読める。
2. **帰納法の仮定が与えるのは $S_{k-1}$ どまりで、命題の結論は $S_k$**。先頭部分 $(P(M')_J)_{J<J_0}$ は $P(M')$ の成分なので
   帰納法の仮定で $\in S_{k-1}$。これを目標の $S_k$ に上げるには **$S_{k-1} \subseteq S_k$（階層の単調性）が要る**が、
   原文はこの補題を**明示も証明もしていない**（暗黙に使っている）。

**単調性 $S_{k-1}\subseteq S_k$ は真（本形式化で `SkT_PS_mono` として証明済）**

> ⚠️ 当初の本項は「単調性は偽」と書いたが**それは誤り**だった（忠実モデルでの経験的検証を有界集合で行ったための
> truncation アーティファクト。境界の見かけの反例は、$S_{k-1}$ の生成元を 1 つ大きい対角列に拡張すれば消える）。
...
**結論：原文証明は本質的に正しく、瑕疵は「単調性補題の省略」＋「$k_0$ 誤記」のみ**

単調性さえ補えば、先頭部分（$\in S_{k-1}\subseteq S_k$）も末尾（$P(M')_{J_0}\in S_{k-1}\subseteq S_k \Rightarrow P(M')_{J_0}[n]\in S_{k+1}$…
実際は $S_k$ 上の構造帰納）も $S_k$ に収まり、結論 $P(M)\in S_k$ が出る。**当初想定した Row1Zero 経由の補強 (R)/(U) は不要**。

**訂正案**

(a) $k_0 \to k$ の誤記修正、(b) 「補題（標準形の階層の単調性）：$S_{k-1}T_{\textrm{PS}} \subseteq S_kT_{\textrm{PS}}$」を
§6.7 に追加（証明は上記対角列拡張）し、1392 行で先頭部分を $S_{k-1}\subseteq S_k$ により $S_k$ とする一文を補う。
...
（単項性を示した直後、$k_0$ 帰納を宣言する一文。補助: `content.md` line 1434）。
