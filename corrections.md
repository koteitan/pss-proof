...
>
> A1〜A44 を `tmp/original.html`（真の原文）と突き合わせて全数検証中。既に判明したこと：
>
> - **A14 / A36 / A42 / A43 は我々の誤りだった**（原文は正しい）。取り下げ済み。
...
>   well-definedness を脚注で自ら論じており、**我々が「穴」と呼んだものの一部は著者が既に塞いでいた**。
>
> 監査完了までは、各項目の判定（VALID / 要書き直し / 取り下げ）を信用すること。
...
### 位置
§5.3 基本列（写像 \([\ ]\)）の定義（content.md 328）。

### 原文
\(G := (M_j)_{j=0}^{j_0-1} \in T_{PS}\)

### 訂正案
\(G := (M_j)_{j=0}^{j_0-1} \in T_{PS} \cup \{()\}\)

### 原文の問題点
\(j_0 = 0\) は実際に起こり（例 \(M = ((0,0),(1,1))\)：\(j_1 = 1\) の親として \(j_0 = 0\) が一意に存在する）、そのとき \(G = ()\) は空配列なので \(T_{PS} = \{M \in \Sigma^{<\omega} \mid M \neq ()\}\) に属さない。なお記法 \((a_i)_{i=i_0}^{i_1}\) は \(i_0 > i_1\) のとき \(()\) と定めているが、\(j_0 - 1\) は \(\mathbb{N}\) 上の減算として読むと \(j_0 = 0\) で \(0\) になり長さ 1 の配列になってしまうため、この添字も \(\mathbb{Z}\) 的に読む必要がある。形式化は `take j0 M` としており正しい（\(M[n] \in T_{PS}\) 自体は `oper_T_PS` で証明済み）。
