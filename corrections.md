...
### 訂正案
part (5): \(\textrm{Trans}(M[n]) = s_1 D_{M_{1,j_{-1}}} (s'_1 D_{M_{1,j_0}})^{n-1} t_2 (b'_1)^{n-1} b_1\)（part (4) の帰納見出しの指数は \(n+1\)）。
...
### 訂正案
part (3): \((s, D_{M_{1,j_{-2}}} 0, b)\) が \(\textrm{Trans}(L')\) の scb 分解（part (2) と同一、無条件）。
...
### 訂正案
結論 (1) の \([0]\)-反復回数の誤り（A37 と同一の問題。訂正は A37 にまとめる）。
...
### 訂正案
条件(VI) の交換則 (1)(3) は全ての許容的 \(j_0\)（\(M_{1,j_0}=0\) を含む）で成立する。ただし結論 (1) に印字された \([0]\)-反復回数は誤りで、正しい回数は基本列展開が定める値である（単純な閉形式では書けない）。

### 原文の問題点
r25-CONDVI13 が真正 condVI mono host（深 Lng≤11）で印字形 (1) を 518/796(adm) で反証。A34（(1) n=1&adm 脚が偽）の精密化に相当。
