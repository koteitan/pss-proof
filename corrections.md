...
## A6. §6.7 命題（標準形の単項成分が標準形であること）: 原文の証明が誤り（同ランク $S_k$ を主張するが証明は $S_{k-1}$ 止まり）

**位置**
- §6.7 / 命題（標準形の単項成分が標準形であること）
- `content.md` line 1364（命題本文）、line 1366–1392（証明）

**原文（命題）**

任意の $k \in \mathbb{N}$ と $M \in S_kT_{\textrm{PS}}$ に対し、$P(M) \in S_kT_{\textrm{PS}}^{<\omega}$ である。
（＝ $M \in S_kT_{\textrm{PS}}$ ならば $P(M)$ の各成分が**同じランク** $S_kT_{\textrm{PS}}$ に属する。）

**問題点**

命題は経験的に**真**（忠実モデルで $k \le 5$・計 3413 要素まで違反 0）。しかし**原文の証明が結論を導けていない**:

- $k>0$ で $M=M'[n]$（$M' \in S_{k-1}$）と分解し、帰納法の仮定（$P(M') \in S_{k-1}^{<\omega}$）から
  「$P$ と基本列の関係」(1)/(2) で $P(M)$ を組む。
- 先頭成分 $(P(M')_J)_{J=0}^{J_0-1}$ は $P(M')$ の成分なので帰納法の仮定で $S_{k-1}$ にしか入らない。
  原文はこれを「$\in S_{k_0-1}T_{\textrm{PS}}^{<\omega}$」と結論しており（添字 $k_0$ は $k$ の誤記と思われる）、
  **$S_{k-1}$ どまりで目標の $S_k$ に達していない**。
- $S_{k-1} \subset S_k$（単調性）は**成り立たない**（忠実モデルで反例多数。$S_kT_{\textrm{PS}}$ の定義の
  $n \in \mathbb{N}$ を $n=0$ 込みと読んでも、$M[0] \ne M$（otherwise 分岐は $\textrm{take}\,j_0\,M$、退化分岐は
  $\textrm{Pred}(M)$）なので $S_{k-1} \subseteq S_k$ は出ない）。

**正しい証明の骨子（本形式化で採用予定）**

経験的監査（`python/sk_a7.py`, `sk_a8.py`）で次が判明:
1. **(R) $P(X)$ の非末尾成分は行 1 が全て 0**（$X \in S_k$ で違反 0）。
2. **(U) 行 1 が全 0 かつ $c \in S_k$ ならば $c \in S_{k+1}$**（違反 0）。

これらを使うと、$T(k):\forall X\!\in\!S_k.\ P(X)_J\!\in\!S_k$ の帰納ステップで、先頭成分
（$=P(M')$ の非末尾成分）は (R) で行 1 全 0、帰納法の仮定で $\in S_{k-1}$、(U) で $\in S_k$ に持ち上がる。
末尾は $P(M')_{J_0}\!\in\!S_{k-1}\Rightarrow P(M')_{J_0}[n]\!\in\!S_k$ の上で構造帰納。
（原文の証明には (R)(U) に相当する補題がなく、ギャップが埋まっていない。）

**形式化での扱い**

`pss_paper.thy` の `p_6_7_standard_P_components`（同ランク $S_k$ のまま）を、補助補題
（非末尾成分の行 1 全 0 性・行 1 全 0 の rank 上方安定性）経由で証明する。命題の主張自体は
原文どおり（訂正不要）で、**証明のみ原文より補強**する。
