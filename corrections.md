...
- 全数走査（\(M \in RT_{\textrm{PS}}\)、成分 \(< 4\)、\(\textrm{Lng} \leq 4\)）：**1,786 例 / 反例 0**
- 標準形プール（\(\textrm{diagSeq}\) の基本列閉包、109 列、成分の最大値 **9**、最大長 9）：**1,505 例 / 反例 0**

いずれも非空虚（\(j'_0 < j'_1 \leq j_1\) かつ \((0,j'_0) \leq_M (0,j'_1)\) を実際に行使）。
...
\(N\) の右端 \(2\) の \(<_N^{\textrm{Next}}\) 親は \(1\) であって \(j'_0 = 0\) ではない。すなわち \(j_0^N = 1 \neq 0 = j'_0\)。
...
\((M,1) \in T_{\textrm{PS}}^{\textrm{Marked}}\) であり、\(1 < \textrm{Lng}(M)-1 = 3\)（＝仮定はすべて成立）。
