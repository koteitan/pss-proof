...
- 数式は GitHub が読める形で書く。インラインはバッククォートを内側に足した形、
  別行立ては math フェンス。素のドル記号と二重ドル記号は使わない。
...
また$`\textrm{Lng}(M) \gt  1`$ならば以下は同値である：
...
- 補足: $`\textrm{Lng}(M)=1`$ のときは $`M[n]=M`$, $`F_M(n)=f(n)=F_{M[n]}(n)`$ なので原文の
  $`n`$ が正しい。$`\textrm{Lng}(M)\gt 1`$ のときに $`f(n)`$ でなければならない。**単一の固定
  引数では両ケースを両立できない。**
...
### 今回の独立再検証
**判定**：系の束縛変数は $`M, j'_0, j'_1, j_1, N`$ のみで、$`m`$ はどこにも導入されていない（原典 content.md 1302 で確認）。
なお $`m`$ は §6.6 の別の箇所（content.md 1199）では束縛されており、そこからの転記と見られる。

**訂正案（指数を $`M_{0,j'_0} - M_{1,j'_0}`$ とする形）の検証**：

- 全数走査（$`M \in RT_{\textrm{PS}}`$、成分 $`\lt  4`$、$`\textrm{Lng} \leq 4`$）：**1,786 例 / 反例 0**
- 標準形プール（$`\textrm{diagSeq}`$ の基本列閉包、109 列、成分の最大値 **9**、最大長 9）：**1,505 例 / 反例 0**

いずれも非空虚（$`j'_0 \lt  j'_1 \leq j_1`$ かつ $`(0,j'_0) \leq_M (0,j'_1)`$ を実際に行使）。

## A3. §6.4 系（$`\textrm{FirstNodes}`$ と $`\textrm{Joints}`$ の単調性）(4): 不等号は $`\gt`$ ではなく $`\geq`$（反例あり）

### 位置
§6.4 幹と枝 / 系（$`\textrm{FirstNodes}`$ と $`\textrm{Joints}`$ の単調性）の主張 (4)

### 原文
任意の$`M \in PT_{\textrm{PS}}`$と$`J'_0,J'_1 \in \mathbb{N}`$に対し、$`J_1 := \textrm{Lng}(\textrm{Br}(M))-1`$と置くと、$`J'_0 \lt  J'_1 \leq J_1`$ならば以下が成り立つ：

(4) 任意の$`i \in \{0,1\}`$に対し$`M_{i,\textrm{Joints}(M)_{J'_0}} \gt  M_{i,\textrm{Joints}(M)_{J'_1}}`$である。

### 訂正案
(4) 任意の$`i \in \{0,1\}`$に対し$`M_{i,\textrm{Joints}(M)_{J'_0}} \geq M_{i,\textrm{Joints}(M)_{J'_1}}`$である。更に$`\textrm{Joints}(M)_{J'_0} \gt  \textrm{Joints}(M)_{J'_1}`$ならば$`M_{i,\textrm{Joints}(M)_{J'_0}} \gt  M_{i,\textrm{Joints}(M)_{J'_1}}`$である。

### 原文の問題点
(2) が $`\geq`$（複数の枝が同一の joint に接続し得る）なので、$`J'_0 \lt  J'_1`$ でも
...
- $`J'_0=0 \lt  J'_1=1`$ だが $`\textrm{Joints}(M)_{J'_0} = \textrm{Joints}(M)_{J'_1} = 1`$ なので
  $`M_{i,1} \gt  M_{i,1}`$ は偽。この $`M`$ は標準形・単項・簡約・$`\textrm{Br}`$ 降順（機械的に確認）。

### 経験的確認（今回、独立に再検証）
$`PT_{\textrm{PS}}`$（単項）の $`M`$ を全数走査（成分 $`\lt  4`$、$`\textrm{Lng} \leq 5`$）し、
$`J'_0 \lt  J'_1 \leq J_1`$ を満たす枝の組 **240,004 例**（すべて非空虚）で検査した：
...
| $`\textrm{Joints}(M)_{J'_0} \gt  \textrm{Joints}(M)_{J'_1}`$（joint が真に減少） | 11,464 | **11,464** |
...
我々が追えた範囲では、下流は (2) の $`\geq`$、$`\textrm{Joints}_J \leq \textrm{TrMax} \lt  \textrm{FirstNodes}_J`$、
...
- $`M`$ は複項。$`P(M) = [\,(0,0),\ (0,1)\,]`$。
- $`\textrm{Red}((0,0)) = (0,0)`$、$`\textrm{Red}((0,1)) = (1,1)`$（単項 $`m_{1,0}=1\gt 0`$ 枝）。
  よって $`\textrm{Red}(M) = (0,0)(1,1)`$。
- $`(0,0) \leq_M (0,1)`$ は偽（$`M_{0,0} \lt  M_{0,1}`$ すなわち $`0\lt 0`$ が不成立）だが
  $`(0,0) \leq_{\textrm{Red}(M)} (0,1)`$ は真（$`0\lt 1`$）。よって不変性は偽。
...
シフト分岐 $`(1,0) \lt_M^{\textrm{Next}} (1,j_1)`$ を「$`\textrm{Red}`$ と $`\textrm{Pred}`$ の可換性と $`B`$ の
...
### 経験的確認（今回、独立に再検証）
$`RT_{\textrm{PS}}`$ を全数走査（成分 $`\lt  4`$、$`\textrm{Lng} \leq 4`$）し、原文の前提
...
（case $`\textrm{Lng}(P(M')_{J_0}) \gt  1`$ の最終文）。命題の主張そのものは**真**であり、訂正は不要。

### 原文
$`(P(M')_J)_{J=0}^{J_0-1} \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$であり、$`P(M')_{J_0} \in S_{k_0-1}T_{\textrm{PS}}`$より帰納法の仮定から$`P(P(M')_{J_0}[n]) \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$であるので、$`P(M) \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$である。□

### 訂正案
$`(P(M')_J)_{J=0}^{J_0-1} \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$であり、$`P(M')_{J_0} \in S_{k_0-1}T_{\textrm{PS}}`$より帰納法の仮定から$`P(P(M')_{J_0}[n]) \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$であるので、$`P(M) \in S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega}`$である。ここで任意の$`k \in \mathbb{N}`$に対し$`S_kT_{\textrm{PS}} \subseteq S_{k+1}T_{\textrm{PS}}`$である。実際、任意の$`u \leq v`$に対し$`\textrm{diagSeq}(u,v) = \textrm{Pred}(\textrm{diagSeq}(u,v+1)) = \textrm{diagSeq}(u,v+1)[1]`$であるから$`S_0T_{\textrm{PS}} \subseteq S_1T_{\textrm{PS}}`$であり、一般の$`k`$については$`k`$に関する数学的帰納法から従う。従って$`S_{k_0-1}T_{\textrm{PS}}^{\lt  \omega} \subseteq S_{k_0}T_{\textrm{PS}}^{\lt  \omega}`$であり、$`P(M) \in S_{k_0}T_{\textrm{PS}}^{\lt  \omega}`$である。□

### 原文の問題点
命題の主張は「任意の$`k \in \mathbb{N}`$と$`M \in S_kT_{\textrm{PS}}`$に対し、$`P(M) \in S_kT_{\textrm{PS}}^{\lt  \omega}`$である」であるから、
示すべきは $`P(M) \in S_{k_0}T_{\textrm{PS}}^{\lt\omega}`$ である。しかし証明の結論は $`S_{k_0-1}T_{\textrm{PS}}^{\lt\omega}`$ で
...
### 経験的確認（今回、独立に再検証）
標準形かつ単項の $`M`$ を全数走査（成分 $`\lt  4`$、$`\textrm{Lng} \leq 4`$）し、
...
$`(\textrm{Br}(M)_{J_1})_{0,0} \gt  (\textrm{Br}(M)_{J_1})_{1,0}`$ならば$`\textrm{LastStep}(M) := \min \{J \in \mathbb{N} \mid (\textrm{Br}(M)_{J_1})_{0,0} = (\textrm{Br}(M)_J)_{0,0} \gt  (\textrm{Br}(M)_J)_{1,0}\}`$である[61]。

（脚注[61]: 「$`J = J_1`$が条件を満たすため$`\min`$が存在する。」）

### 訂正案
$`\textrm{Lng}(\textrm{Br}(M)) = 0`$ならば$`\textrm{LastStep}(M) = 0`$である[58]。
$`\textrm{Lng}(\textrm{Br}(M)) \gt  0`$とする[59]。
$`J_1 := \textrm{Lng}(\textrm{Br}(M))-1`$と置く。
$`(\textrm{Br}(M)_{J_1})_{0,0} = (\textrm{Br}(M)_{J_1})_{1,0}`$ならば$`\textrm{LastStep}(M) = J_1`$である[60]。
$`(\textrm{Br}(M)_{J_1})_{0,0} \gt  (\textrm{Br}(M)_{J_1})_{1,0}`$ならば$`\textrm{LastStep}(M) := \min \{J \in \mathbb{N} \mid J \leq J_1 \wedge (\textrm{Br}(M)_{J_1})_{0,0} = (\textrm{Br}(M)_J)_{0,0} \gt  (\textrm{Br}(M)_J)_{1,0}\}`$である[61]。
...
問題ではない**。問題は集合の **well-formedness** で、$`\textrm{Br}(M)_J`$ は $`J \lt  \textrm{Lng}(\textrm{Br}(M))`$ の
...
- **忠実性（機械検証）**: `ot9_LastStep_A9_faithful` — **旧定義の集合が有限だったとき（＝旧 `min` が well-defined だったとき、すなわち旧 `fin` 仮定が成り立っていた場合すべて）、新旧の $`\textrm{LastStep}`$ の値は一致する**。`if` 分岐と $`\textrm{Br}(M)=()`$ 分岐は不変。else 分岐では簡約形の host 上で $`J_1`$ 自身が有界集合に属し、旧集合が余分に持つ元は全て範囲外（$`\ge \textrm{Lng}(\textrm{Br}(M)) \gt  J_1 \ge \min`$）なので $`\min`$ は変わらない。すなわち**値を変えない曖昧性解消**であり、既存の証明済事実は一つも無効化されない。
...
$`\textrm{Red}`$ の再帰的定義（$`M`$ が単項・$`M_0 \neq (0,0)`$・$`M_{1,0} \gt  0`$ の場合）は次の通りである：
...
$`M_{1,0} \gt  j_1`$ならば$`\textrm{Red}(M) := M`$である[20]。
...
- 系（直系先祖の $`\textrm{Red}`$ 不変性）の証明は「$`\textrm{Lng}(M)`$ に関する数学帰納法」とされるが、
  ...
  $`M_{1,0} + \textrm{Lng}(M) \gt  \textrm{Lng}(M)`$ と**増える**ため、$`\textrm{Lng}`$ に関する帰納法の仮定が
...
### 原文
(2) 任意の$`v \in \mathbb{N}`$と$`s,c,b \in \Sigma^{\lt  \omega}`$に対し、$`(s,c,b)`$が$`t`$のscb分解であるならば$`(D_v s,c,b)`$は$`D_v t`$のscb分解である。

### 訂正案
(2) 任意の$`v \in \mathbb{N}`$と$`s,c,b \in \Sigma^{\lt  \omega}`$に対し、$`c \in PT_{\textrm{B}}`$かつ$`(s,c,b)`$が$`t`$のscb分解であるならば$`(D_v s,c,b)`$は$`D_v t`$のscb分解である。
...
### 今回の独立再検証（原典の定義だけから従う）
scb分解の定義「$`t = scb`$。$`t \neq 0`$ならば$`c \in PT_{\textrm{B}}`$。$`b`$は$`\underline{)}`$のみからなる」に対し
$`t = 0`$、$`s = b =`$ 空列、$`c = 0`$ と取ると、$`(s,c,b)`$ は $`0`$ の scb分解である
（$`t = 0`$ なので単項性は課されない）。しかし (2) の結論が主張する $`(D_v s, c, b)`$ は
$`D_v t = D_v 0 \neq 0`$ の scb分解であることを要求するので $`c = 0 \in PT_{\textrm{B}}`$ が必要になるが、
$`0`$ は単項ではない。よって (2) は偽。モデルを使わず原典の定義だけから従う。
...
### 原文
任意の$`s,b \in \Sigma^{\lt  \omega}`$と$`c_0, c_1 \in T_{\textrm{B}}`$に対し、「$`c_0`$が単項でないまたは$`c_1`$が単項である」かつ$`s c_0 b \in T_{\textrm{B}}`$かつ$`(s,c_0,b)`$が$`s c_0 b`$のscb分解であるならば、$`s c_1 b \in T_{\textrm{B}}`$かつ$`(s,c_1,b)`$は$`s c_1 b`$のscb分解である。

### 訂正案
任意の$`s,b \in \Sigma^{\lt  \omega}`$と$`c_0, c_1 \in T_{\textrm{B}}`$に対し、「$`c_1`$が単項であるまたは$`s c_1 b = 0`$」かつ$`s c_0 b \in T_{\textrm{B}}`$かつ$`(s,c_0,b)`$が$`s c_0 b`$のscb分解であるならば、$`s c_1 b \in T_{\textrm{B}}`$かつ$`(s,c_1,b)`$は$`s c_1 b`$のscb分解である。
...
### 原文
(3) 任意の$`v \in \mathbb{N}`$と$`s_0,s_1,b_0,b_1 \in \Sigma^{\lt  \omega}`$と$`c' \in PT_{\textrm{B}}`$に対し、$`s_1 D_v(t+c) b_1 \in T_{\textrm{B}}`$かつ$`(s_0,c,b_0)`$が$`s_1 D_v(t+c) b_1`$のscb分解であるならば、$`s_1 D_v(t+c') b_1 \in T_{\textrm{B}}`$かつ$`(s_0,c',b_0)`$は$`s_1 D_v(t+c') b_1`$のscb分解である。

### 訂正案
(3) 任意の$`v \in \mathbb{N}`$と$`s_0,s_1,b_0,b_1,\textrm{pre},\textrm{post} \in \Sigma^{\lt  \omega}`$と$`c' \in PT_{\textrm{B}}`$に対し、$`t+c = \textrm{pre}\,c\,\textrm{post}`$かつ$`t+c' = \textrm{pre}\,c'\,\textrm{post}`$かつ$`\textrm{post}`$が$`\underline{)}`$のみからなる文字列であり、$`s_0 = s_1 D_v \textrm{pre}`$かつ$`b_0 = \textrm{post}\, b_1`$であり、更に$`s_1 D_v(t+c) b_1 \in T_{\textrm{B}}`$かつ$`(s_0,c,b_0)`$が$`s_1 D_v(t+c) b_1`$のscb分解であるならば、$`s_1 D_v(t+c') b_1 \in T_{\textrm{B}}`$かつ$`(s_0,c',b_0)`$は$`s_1 D_v(t+c') b_1`$のscb分解である。
...
### 今回の独立再検証（原典の定義だけから従う）
$`t = 0`$、$`c = D_0 0`$、$`c' = D_0(D_0 0)`$、$`v = 0`$、$`s_1 D_v(t+c) b_1 = D_0(D_0 0) + D_0 0`$（複項）と取る。
このとき $`D_v(t+c) = D_0(D_0 0)`$ は**第1**主要項であり、$`(s_0,c,b_0)`$ は**第2**主要項 $`D_0 0`$ を中心とする
scb分解に取れる（前提はすべて成立）。結論は文字列として $`s_0 c' b_0 = s_1 D_v(t+c') b_1`$ を要求するが、
左辺は第2主要項を $`c'`$ に置換した $`D_0(D_0 0) + D_0(D_0 0)`$、右辺は第1主要項を置換した
$`D_0(D_0(D_0 0)) + D_0 0`$ であり、相異なる。よって偽。
...
## A18. §7.4 系（Mark と $`\lt_M^{\textrm{NextAdm}}`$ の関係）: 仮定の祖先 $`j`$ に「$`M`$ 許容（$`(M,j)\in\textrm{Marked}`$）」を補う必要（補わないと偽）

### 位置
§7.4「系（$`\textrm{Mark}`$ と $`\lt_M^{\textrm{NextAdm}}`$ の関係）」（形式化では `p_7_4_Mark_nextAdm`）。

### 原文
$`M \in T_{\textrm{PS}}`$とし、$`j_1 := \textrm{Lng}(M) - 1`$と置く。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在するとする。任意の$`j \in \mathbb{N}`$に対し、$`(0,j) \leq_M (0,j_0)`$ならば、一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
### 訂正案
$`M \in T_{\textrm{PS}}`$とし、$`j_1 := \textrm{Lng}(M) - 1`$と置く。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在するとする。任意の$`j \in \mathbb{N}`$に対し、$`(0,j) \leq_M (0,j_0)`$かつ$`(M,j) \in T_{\textrm{PS}}^{\textrm{Marked}}`$ならば、一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
### 今回の独立再検証
$`M = ((0,0),(1,1),(2,2),(3,1))`$（簡約）：$`j_1 = 3`$、$`\lt_M^{\textrm{NextAdm}}`$ の親は $`j_0 = 2`$ ただひとつ。
...
### 原文の問題点
$`T_{\textrm{B}}^{\textrm{Marked}}=\{(t,c)\mid (s,c,b)\,\text{が}\,t\,\text{の scb 分解}\}`$(content.md 1834)は **(whole, block)** の規約($`c`$ は $`t`$ の部分=被覆される側、cf. 1936 $`(t+c,c)`$)。一方 $`\textrm{Mark}(M,m)`$ は $`m`$ が小さいほど大きい($`\textrm{Mark}(M,0)=\textrm{Trans}(M)`$ 最大、$`\textrm{Mark}(M,j_1)=D_{M_{1,j_1}}0`$ 最小)。よって $`m_0\lt m_1`$ では $`\textrm{Mark}(M,m_0)`$ が whole、$`\textrm{Mark}(M,m_1)`$ が block で、正しい対は $`(\textrm{Mark}(M,m_0),\textrm{Mark}(M,m_1))\in T_{\textrm{B}}^{\textrm{Marked}}`$。原文の $`(\textrm{Mark}(M,m_1),\textrm{Mark}(M,m_0))`$ は **whole と block が逆**。

### 経験的確認
簡約な $`M`$（成分 $`\leq 4`$、$`\textrm{Lng} \leq 4`$、乱択 400 列）から、$`(M,m_0),(M,m_1) \in T_{\textrm{PS}}^{\textrm{Marked}}`$
かつ $`m_0 \lt  m_1`$ かつ像が相異なる組を全て取り出した **249 例**（すべて非空虚に仮定を行使）で：
...
### 確認（機械証明による）
`Mark_MarkedB_nest`（緑）: $`(M,m),(M,m')\in\textrm{Marked}\wedge m\le m'\wedge M\in RT_{\textrm{PS}}\Rightarrow(\textrm{Mark}\,M\,m,\textrm{Mark}\,M\,m')\in\textrm{MarkedB}`$。すなわち小さい添字が whole 側。`MarkedB_antisym`（緑）より両向きの入れ子は項の一致を強制するので、$`m_0\lt m_1`$（狭義、像が相異なる）では原文の向きは偽。
...
任意の$`M \in RT_{\textrm{PS}} \cap PT_{\textrm{PS}}`$に対し、$`\textrm{Trans}`$の再帰的定義中に導入した記号を用いると、$`j_0`$が$`M`$許容かつ$`j_1 \gt  1`$かつ$`M_{1,j_0} \geq M_{1,j_1}`$ならば、以下が成り立つ：

(1) $`t_1 \neq 0`$であり$`M`$は条件(I)か(III)を満たし$`\textrm{Trans}((M_j)_{j=j_0}^{j_1-1}) = c_1 \in PT_{\textrm{B}}`$である。

### 訂正案
(1) $`t_1 \neq 0`$であり$`M`$は条件(I)か(III)を満たし$`c_1 \in PT_{\textrm{B}}`$である。更に$`j_0 \lt  j_1-1`$または$`M_{0,j_0} = M_{1,j_0}`$（すなわち切片$`(M_j)_{j=j_0}^{j_1-1}`$が簡約）ならば$`\textrm{Trans}((M_j)_{j=j_0}^{j_1-1}) = c_1`$である。
...
（$`j_0`$ が $`M`$ 許容、$`j_1 \gt  1`$、$`M_{1,j_0} \geq M_{1,j_1}`$）を満たす **1,010 例**で part(1) の等式を検査:

- 成立 **965 例** / 不成立 **45 例**
- 不成立 45 例は**すべて** $`j_0 = j_1-1`$ かつ $`M_{0,j_0} \neq M_{1,j_0}`$（非簡約な 1 列切片）
- 訂正案のガード（$`j_0 \lt  j_1-1`$ または $`M_{0,j_0} = M_{1,j_0}`$）が許す例では**違反 0 件**（健全）。
...
補題の仮定は「任意の$`M \in RT_{\textrm{PS}} \cap PT_{\textrm{PS}}`$に対し、$`\textrm{Trans}`$の再帰的定義中に導入した記号を用いると、$`j_0`$が$`M`$許容かつ$`j_1 \gt  1`$かつ$`M_{1,j_0} \geq M_{1,j_1}`$ならば」である。

### 原文
(5) $`(M[n],j_0+(n-1)(j_1-j_0)) \in T_{\textrm{B}}^{\textrm{Marked}}`$かつ$`(0,j'_0) \lt_{M[n]}^{\textrm{Next}} (0,j_0+(n-1)(j_1-j_0))`$であり、$`j_1^N = j_0+(n-1)(j_1-j_0)`$かつ$`j_0^N = j'_0`$かつ$`j_{-1}^N = j'_{-1}`$かつ$`t_1^N \neq 0`$であり、$`N`$は条件(VI)を満たさない。

### 訂正案
(5) $`(M[n],j_0+(n-1)(j_1-j_0)) \in T_{\textrm{B}}^{\textrm{Marked}}`$かつ$`(0,j'_0) \lt_{M[n]}^{\textrm{Next}} (0,j_0+(n-1)(j_1-j_0))`$であり、$`j_1^N = j_0+(n-1)(j_1-j_0)`$かつ$`j_{-1}^N = j'_{-1}`$かつ$`t_1^N \neq 0`$であり、$`N`$は条件(VI)を満たさない。更に$`M`$が条件(I)を満たすならば$`j_0^N = j'_0`$である。
...
$`N = ((0,0),(1,1),(2,0))`$ であり、$`N`$ の右端 $`2`$ の $`\lt_N^{\textrm{Next}}`$ 親は $`1`$ であって
...
### 今回の独立再検証
$`M = ((0,0),(1,1),(2,1))`$（簡約かつ単項、$`M_{1,j_1} = 1 \neq 0`$ なので条件(I)ではなく条件(III)）：
$`j_1 = 2`$、$`j_0 = 1`$、$`j'_0 = 0`$。$`n = 2`$ とすると $`M[2] = ((0,0),(1,1),(2,0),(3,1))`$、
$`j_0+(n-1)(j_1-j_0) = 2`$、$`N = ((0,0),(1,1),(2,0))`$。
$`N`$ の右端 $`2`$ の $`\lt_N^{\textrm{Next}}`$ 親は $`1`$ であって $`j'_0 = 0`$ ではない。すなわち $`j_0^N = 1 \neq 0 = j'_0`$。
...
### 原文
任意の$`M in T_{\textrm{PS}}`$と$`n,r' \in \mathbb{N}_{+}`$と$`q,q' \in \mathbb{N}`$に対し、$`j_1 := \textrm{Lng}(M)-1`$と置くと、$`(0,j_0) \lt_M^{\textrm{Next}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在し$`M_{1,j_1} = 0`$かつ$`q,q' \leq n-1`$かつ$`r' \in j_1-j_0`$ならば、$`M[n]_{0,j_0+q(j_1-j_0)} \lt  M[n]_{0,q'(j_1-j_0)+r'}`$である。

### 訂正案
任意の$`M \in T_{\textrm{PS}}`$と$`n,r' \in \mathbb{N}_{+}`$と$`q,q' \in \mathbb{N}`$に対し、$`j_1 := \textrm{Lng}(M)-1`$と置くと、$`(0,j_0) \lt_M^{\textrm{Next}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在し$`M_{1,j_1} = 0`$かつ$`q,q' \leq n-1`$かつ$`r' \lt  j_1-j_0`$ならば、$`M[n]_{0,j_0+q(j_1-j_0)} \lt  M[n]_{0,j_0+q'(j_1-j_0)+r'}`$である。
...
2. **$`r' \in j_1-j_0`$** — $`r'`$ は自然数、$`j_1-j_0`$ も自然数であり、集合ではない。記事の他の箇所（例えば直後の補題の証明や §6.8 の証明）は同種の上界をすべて $`\lt`$ で書いており、$`\in`$ をこの意味で使っている箇所は記事全体でここ1箇所だけである。$`r' \lt  j_1-j_0`$ が意図されている。

3. **右辺の添字 $`q'(j_1-j_0)+r'`$** — $`j_0+`$ が脱落している。**原文の証明自身がそれを示している**：証明の最終行は
   「$`M[n]_{j_0+q(j_1-j_0)} = M_{j_0}`$かつ$`M[n]_{j_0+q'(j_1-j_0)+r'} = M_{j_0+r'}`$より$`M[n]_{0,j_0+q(j_1-j_0)} \lt  M[n]_{0,j_0+q'(j_1-j_0)+r'}`$となる。」
   と、$`j_0+`$ 付きの形を導いている。脱落した形は $`q' = 0`$ のとき添字が $`r' \lt  j_1-j_0`$ となり、基本列の反復ブロックではなく先頭の $`(M_j)_{j=0}^{j_0-1}`$ の領域を指してしまうため、主張として偽になりうる。
...
### 原文（脚注[30]、逐語）
すなわち[Buc1] $`([\ ].4)`$ (ii)の場合分けにおいて、各$`i \in \mathbb{N}`$に対し$`x_i`$を「$`i = 0`$ならば$`x_i = D_u 0`$、$`i \gt  0`$ならば$`x_i = b[D_u x_{i-1}]`$」と定め、$`a[n]`$の定義を$`D_v b[x_n]`$に変えるということである。

### 訂正案
すなわち[Buc1] $`([\ ].4)`$ (ii)の場合分けにおいて、各$`i \in \mathbb{N}`$に対し$`x_i`$を「$`i = 0`$ならば$`x_i = D_u 0`$、$`i \gt  0`$ならば$`x_i = D_u b[x_{i-1}]`$」と定め、$`a[n]`$の定義を$`D_v b[x_n]`$に変えるということである。

### 原文の問題点
$`x_i`$ の定義で $`D_u`$ と $`b[\cdot]`$ の適用順が入れ替わっている（$`b[D_u x_{i-1}]`$ ではなく $`D_u b[x_{i-1}]`$）。$`a[n] = D_v b[x_n]`$ の方は**印字どおりで正しい**。

印字された組み合わせ（$`x_i = b[D_u x_{i-1}]`$ かつ $`a[n] = D_v b[x_n]`$）だと $`b[\cdot]`$ が二重に適用され、$`x_n \notin \textrm{dom}(b) = T_u`$ となって項が増大し、[Buc1] Lemma 3.2(a)（$`a[z] \lt  a`$）が壊れる（主項 106 例中 3 例しか整合しない。反例 $`a = D_0(D_1 0 + D_1 0)`$）。
...
### 訂正案
(5) $`n = 1`$ならば$`\textrm{Trans}(M[n]) = s_1 D_{M_{1,j_{-1}}} t_2 b_1`$であり、$`n \gt  1`$ならば$`\textrm{Trans}(M[n]) = s_1 D_{M_{1,j_{-1}}} (s'_1 D_{M_{1,j_0}})^n t_2 (b'_1)^n b_1`$である。
...
主張と、その主張自身の証明の基底が食い違っている。$`n \gt  1`$ の場合の証明 (content.md 5337-5347) は
...
### 原文
(3) $`j_{-2} \lt  j_0`$かつ$`j_0`$が非$`M`$許容であるならば、$`(s,D_{M_{1,j_0}}(t_2 + D_{M_{1,j_0}} 0),b)`$は$`\textrm{Trans}(L')`$のscb分解である。

### 訂正案
(3) $`j_{-2} \lt  j_0`$かつ$`j_0`$が非$`M`$許容であるならば、$`(s,D_{M_{1,j_{-2}}} 0,b)`$は$`\textrm{Trans}(L')`$のscb分解である。
...
### 今回の独立再検証（長さ勘定で決定的）
$`M = (0,0)(1,1)(2,2)(3,1)(4,0)(5,1)(6,2)(6,1)`$（標準形かつ単項）：$`j_1 = 7`$、$`j_0 = 5`$、$`j_{-2} = 4`$、
$`j_0`$ は非 $`M`$ 許容（＝ part (3) の条件を満たす）。
$`N' = (4,0)(5,1)(6,2)(6,1)`$、$`L' = (4,0)(5,1)(6,2)(6,0)`$。

補題は **一意な $`(s,b)`$** が (1)(2)(3) すべてを満たすと主張しているので、$`(s,b)`$ は part (1)
（$`(s,D_{M_{1,j_1}} 0,b)`$ が $`\textrm{Trans}(N')`$ の scb分解）で一意に決まる。その $`(s,b)`$ に対し

- $`\textrm{Trans}(L')`$ の文字列長 $`= 14`$、$`s`$ と $`b`$ の長さの和 $`= 12`$ なので、**中心の長さは 2 に決まる**。
- 訂正案の中心 $`D_{M_{1,j_{-2}}} 0 = D_0 0`$ は長さ 2 であり、実際に $`(s, D_0 0, b)`$ は $`\textrm{Trans}(L')`$ の scb分解である。
- 原文の中心 $`D_{M_{1,j_0}}(t_2 + D_{M_{1,j_0}} 0) = D_1(t_2 + D_1 0)`$ は、$`t_2`$ が何であれ最短で長さ 6 以上であり、長さ 2 にはなりえない。

よって原文の part (3) は、part (1) と $`(s,b)`$ を共有できない。
...
### 今回の独立再検証
$`M = ((0,0),(1,1),(2,0),(3,1))`$（標準形かつ単項）：$`j_1 = 3`$、$`j_{-2} = 2`$、
$`N' = ((2,0),(3,1))`$、$`\textrm{Pred}(N') = ((2,0))`$ は**零項**。
よって $`\textrm{Trans}(\textrm{Pred}(N')) = 0`$。一方 $`n = 2`$ のとき
$`M[2] = ((0,0),(1,1),(2,0),(3,0))`$、$`\textrm{Trans}(M[2]) = D_0 D_1 D_0 D_0 0 \neq 0`$。
scb分解の定義により、対象が零項でない以上その中心は単項でなければならないが、$`0`$ は単項でない。
よって (5-3) の scb分解は存在しない。
...
一意に存在する）。そのとき記法の約束（$`i_0 \gt  i_1`$ のとき $`(a_i)_{i=i_0}^{i_1} := ()`$）により
$`G = ()`$ となるが、$`T_{\textrm{PS}}`$ は空列を含まない（$`T_{\textrm{PS}} = \{M \in \Sigma^{\lt\omega} \mid M \neq ()\}`$）ので、
...
$`j_1 = 1`$、$`M_{j_1} = (1,1) \neq (0,0)`$、$`i_1 = \max\{i \in \{0,1\} \mid M_{i,1} \gt  0\} = 1`$。
行 $`1`$ は $`M_{1,0} = 0 \lt  1 = M_{1,1}`$ なので $`(1,0) \lt_M^{\textrm{Next}} (1,1)`$ であり、
これを満たす $`j_0`$ は $`j_0 = 0`$ ただひとつ。よって記法の約束
（$`i_0 \gt  i_1`$ のとき $`(a_i)_{i=i_0}^{i_1} := ()`$）により $`G = (M_j)_{j=0}^{-1} = ()`$ である。
...
これは境界的な例外ではない：成分 $`\lt  4`$・$`\textrm{Lng} \leq 3`$ の $`T_{\textrm{PS}}`$ を全数走査した
...
$`\leq_M`$・$`\lt_M^{\textrm{NextAdm}}`$・$`M`$ 許容性は**元の列 $`M`$** から読まれる。そして
...
## A45. §7.4 命題（$`\textrm{Trans}`$ と $`\lt_M^{\textrm{NextAdm}}`$ の関係）: 定義域が $`T_{PS}`$ では偽（$`RT_{PS}`$ に制限が必要）

### 位置
§7.4 許容的親子関係、命題（$`\textrm{Trans}`$と$`\lt_M^{\textrm{NextAdm}}`$の関係）。形式化では `p_7_4_Trans_nextAdm`。

### 原文
命題（$`\textrm{Trans}`$と$`\lt_M^{\textrm{NextAdm}}`$の関係）

$`M \in T_{\textrm{PS}}`$とし、$`j_1 := \textrm{Lng}(M) - 1`$と置く。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在するならば、一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
$`M \in RT_{\textrm{PS}}`$とし、$`j_1 := \textrm{Lng}(M) - 1`$と置く。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在するならば、一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
（悉皆検査（成分 $`\lt  3`$、$`\textrm{Lng} \le 4`$）：反例 33 件、**すべて非簡約**。簡約な $`M`$ では 224/224 成立。）

### 今回の独立再検証
$`M = (0,0)(0,1)(1,2)(1,0)`$（非簡約）：$`j_1 = 3`$、$`\lt_M^{\textrm{NextAdm}}`$ の親は $`j_0 = 1`$ ただひとつ。
...
任意の$`(M,m) \in T_{\textrm{PS}}^{\textrm{Marked}}`$に対し、$`m \lt  \textrm{Lng}(M) - 1`$ならば一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
任意の$`(M,m) \in RT_{\textrm{PS}}^{\textrm{Marked}}`$に対し、$`m \lt  \textrm{Lng}(M) - 1`$ならば一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
### 今回の独立再検証
$`M = (0,0)(0,1)(1,2)(1,0)`$（非簡約）、$`m = 1`$：$`1`$ は $`M`$ 許容かつ $`(0,1) \leq_M (0,3)`$ なので
$`(M,1) \in T_{\textrm{PS}}^{\textrm{Marked}}`$ であり、$`1 \lt  \textrm{Lng}(M)-1 = 3`$（＝仮定はすべて成立）。
$`\textrm{Mark}(\textrm{Pred}(M),1) = D_2 0`$、$`\textrm{Mark}(M,1) = D_0 0`$。
$`\textrm{Trans}(\textrm{Pred}(M))`$ 側の scb分解は 1 個、$`\textrm{Trans}(M)`$ 側も 1 個あるが、
**共通のものは 0 個**である。すなわち結論の $`(s_0,b_0)`$ は存在しない。

## A47. §7.4 系（$`\textrm{Mark}`$ と $`\lt_M^{\textrm{NextAdm}}`$ の関係）: 定義域が $`T_{PS}`$ では偽（反例あり。$`RT_{PS}`$ に制限が必要）

### 位置
§7.4 許容的親子関係、系（$`\textrm{Mark}`$と$`\lt_M^{\textrm{NextAdm}}`$の関係）。形式化では `p_7_4_Mark_nextAdm`。
...
$`M \in RT_{\textrm{PS}}`$とし、$`j_1 := \textrm{Lng}(M) - 1`$と置く。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,j_1)`$を満たす一意な$`j_0 \in \mathbb{N}`$が存在するとする。任意の$`j \in \mathbb{N}`$に対し、$`(0,j) \leq_M (0,j_0)`$かつ$`(M,j) \in T_{\textrm{PS}}^{\textrm{Marked}}`$ならば、一意な$`(s_0,b_0) \in (\Sigma^{\lt  \omega})^2`$が存在し、以下を満たす：
...
**簡約後の列**で計算される一方、仮定の $`\leq_M`$・$`\lt_M^{\textrm{NextAdm}}`$・$`M`$ 許容性は
...
$`j_1 = 5`$。$`(0,j_0) \lt_M^{\textrm{NextAdm}} (0,5)`$ を満たす $`j_0`$ は $`j_0 = 3`$ ただひとつ。
