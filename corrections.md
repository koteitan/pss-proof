...
## A42. §6.5 命題（単項性と Red の関係）は \(M_{1,0} = 0\) のとき偽 [軽微]

### 位置
§6.5 命題（単項性と \(\textrm{Red}\) の関係）（content.md 946）。

### 訂正案
仮定に \(M_{1,0} > 0\) を追加する。

### 原文の問題点
\(M_{1,0} = 0\) のとき、原文が主張する先頭の対角列 \((\ (0,0),\dots,(M_{1,0}-1, M_{1,0}-1)\ )\) は \(\mathbb{N}\) 上の減算で \(0 - 1 = 0\) となり空にならず、主張が偽になる（A10 と同根の nat 切り捨て）。\(\textrm{Red}\) の再帰自身は \(M_{1,0} > 0\) の枝でのみこの形を使うので、仮定を補えば内容は失われない。形式化 `m_6_5_monoT_Red_m10pos` はこの仮定付きで証明している。

## A43. §6.6 補題（簡約性と左端の関係）の先頭対角にガードが必要 [軽微]

### 位置
§6.6 補題（簡約性と左端の関係）（content.md 1084）。

### 訂正案
先頭の対角列を \(\ u < M_{1,0}\) のときのみ \((\,(i,i)\,)_{i=u}^{M_{1,0}-1}\)、そうでなければ \(()\) と読む（A42 と同じく \(\mathbb{Z}\) 的な添字読み）。

### 原文の問題点
\(u = M_{1,0}\) かつ \(M_{1,0} = 0\) のとき、\(\mathbb{N}\) 上の減算では \((i,i)_{i=0}^{-1}\) が \(((0,0))\) と読めてしまい、存在しない先頭項を主張することになる。形式化 `m_6_6_reduced_leftend` は `if u < m10 then diagSeq u (m10-1) else []` とガードを入れている。A10/A42 と同根。
