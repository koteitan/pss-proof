...
**補足: 命題（$\textrm{Red}$ と基本列の可換性）の証明のシフト分岐にギャップ**

定義域を標準形（簡約）に制限すれば主張は真だが、原文の証明（$j_1$ に関する帰納法）の
シフト分岐 $(1,0) <_M^{\textrm{Next}} (1,j_1)$ を「$\textrm{Red}$ と $\textrm{Pred}$ の可換性と $B$ の
定義から $n$ に関する数学的帰納法により従う」とする箇所は、そのままでは閉じない:
$M[n{+}1]$ は $M[n]$ に1ブロック $B_n$（$w := j_1 - j_0$ 要素）を連結した形なので、
$\textrm{Red}$-$\textrm{Pred}$ 可換性（1要素ずつ）を $w$ 回適用しても
$\textrm{Pred}^w(\textrm{Red}(M[n{+}1])) = \textrm{Red}(M[n])$、すなわち $\textrm{Red}(M[n{+}1])$ の
**接頭辞（末尾 $w$ 要素を除く部分）が $\textrm{Red}(M[n])$ に一致する**ことしか determine しない。
末尾ブロック $B_n$ が簡約形であること（$=$ タイリング $M[n]$ への命題（簡約性と係数の関係）の
$\textrm{RedCondA}$/$\textrm{RedCondB}$ 成分の適用）は $n$ 帰納の外であり、独立の構造的議論を要する。
原文の「従う」はこの末尾ブロック簡約性を省略している。本形式化では標準形に制限した正しい主張を、
この末尾ブロック簡約性（row-1 親構造 $\Rightarrow$ row-0 が $+1$ ランプ）に還元したが、ここが現状の
未完の中核である。
