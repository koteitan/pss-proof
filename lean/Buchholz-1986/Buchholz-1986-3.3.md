...
辞書式順序 \(<\) は狭義線型順序である。以下、\(a\leq b\) は
\(a<b\) または \(a=b\) を表す。従って

\[
\begin{gathered}
a\leq a,\\
a\leq b\leq c\Longrightarrow a\leq c,\\
a\leq b<c\Longrightarrow a<c,\qquad
a<b\leq c\Longrightarrow a<c,\\
a\leq b\land b\leq a\Longrightarrow a=b,\\
...
\[
\begin{aligned}
M\subseteq N&\Longrightarrow M\preccurlyeq N,\\
M\preccurlyeq N\subseteq N'&\Longrightarrow M\preccurlyeq N',\\
...
\[
\begin{aligned}
x\in G_u(a)&\Longrightarrow |x|<|a|,\\
x\in G_u(a)&\Longrightarrow G_u(x)\subseteq G_u(a),\\
...
\[
\begin{gathered}
\operatorname{TriG}(z,b,a),\qquad b\leq a,\\
\forall x\in G_u(a)\ (x<a),\qquad
\forall x\in G_u(z)\ (x<b)
\end{gathered}
\]
...
- \(y\in G_u(a)\) なら \(y<a\) なので \(x\leq y<a\)、従って \(x<a\)。
- \(y\in G_u(z)\) なら \(y<b\) なので
  \(b\leq x\leq y<b\) となり、\(b<b\) に矛盾する。
- \(y=0\) なら \(x\leq0\leq x\) より \(x=0\) である。さらに
  \(b\leq0\leq b\) より \(b=0\) となるが、
  \(x\in G_u(b)=G_u(0)=\varnothing\) に矛盾する。

従って矛盾する二場合を除けば \(x<a\) であり、特に \(x\leq a\) である。
...
\(x<b\)、\(x=b\)、\(b<x\) のいずれかである。後二場合はいずれも
\(b\leq x\) を与えて (8) に反する。従って \(x<b\) であり、(7) が示された。
...
\(p_0<q\) と \(q<p_0\) は同時には起こらない。従って \(q=p_0\) であり、
両不等式は尾部の
...
\(u>v\) なら \(G_u(D_vb_0)=\varnothing\) なので自明である。
\(u\leq v\) なら
...
- \(b\neq0\)、\(\operatorname{dom}(b)=T_u\)、\(v>u\) の場合は
  \(w=u\) かつ \(a[z]=D_v(b[z])\) である。帰納法の仮定から
  \(b[z_1]<b[z_2]\) であり、同じ添字 \(v\) を付けて (17) を得る。
...
\(\operatorname{dom}(a)=T_w\) から \(w<h\) が従う。\(z\in T_w\) の全ての
主項添字は \(h\) より小さいので
...
\(\operatorname{dom}(b)=T_w\) かつ \(w<v\) である。このとき
\(a[z]=D_v(b[z])\) であり、添字境界から \(z<D_v(b[z])\) を得る。
従って全場合で (18) が成り立つ。
...
- \(\operatorname{dom}(b)=T_w\) かつ \(v>w\)：外側の定義域は
...
\[
\begin{gathered}
y\in OT_B,\qquad v<\omega,\qquad
\forall x\in G_v(y)\ (x<y)
\\
...
これは \(OT\) の主項生成条件そのものである。\(v<\omega\) と
...
\[
\operatorname{TriG}(X_i,Y_i,b),\qquad Y_i<b
\tag{33}
\]
...
すなわち \(X_0<X_1\) である。
帰納段階では \(X_i<X_{i+1}\) と (20) に
[BUC86-3-3-OPER-MONO](#buc86-3-3-oper-mono) を適用して
\(Y_i<Y_{i+1}\) を得る。同じ添字 \(D_w\) を付ければ
\(X_{i+1}<X_{i+2}\) である。従って (36) が全ての \(i\) で成立する。
...
\(G_v(X_0)=\{0\}\) であり、(35) から \(0<Y_0\)。従って (38) が成り立つ。
...
\(Y_i<Y_{i+1}\) は (36) から従う。\(x\in G_v(Y_i)\) なら
(39) より \(x<Y_i<Y_{i+1}\)。従って (38) が \(i+1\) でも成立する。
...
\[
\forall x\in G_v(Y_n)\ (x<Y_n)
\tag{41}
\]

となる。\(D_vb\) が \(D_\omega\)-自由なので \(v<\omega\) であり、(26) から
...
と書ける。\(u>v\) なら \(G_u(D_vY_n)=\varnothing\) なので自明である。
\(u\leq v\) なら
...
\(w<v\) なので [BUC86-3-3-G-ABOVE](#buc86-3-3-g-above) より
\(G_v(z)=\varnothing\)。
降下性から \(b[z]<b\) である。従って
...
また \(p\) は \(b\) の先頭主項以上である。\(\operatorname{dom}(a)=
\operatorname{dom}(b)\) なので、\(z\) は \(b\) に対する帰納法の条件も満たす。
帰納法の仮定から
