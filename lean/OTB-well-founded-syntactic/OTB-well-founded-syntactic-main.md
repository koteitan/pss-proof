...
R(a,b)
\quad\Longleftrightarrow\quad
a\in OT_B\ \land\ b\in OT_B\ \land\ a\lt b
\tag{1}
```
...
$`\operatorname{DF}(z)`$ より $`k_i\lt\omega`$ かつ
...
$`a,b\in OT_B`$ かつ $`b\lt a`$ とする。Bachmann 共終性定理
...
\begin{array}{l}
z\in W_m,\cr 
z\in\operatorname{dom}(a),\cr 
z\in OT,\cr 
\operatorname{DF}(z),\cr 
b\le a[z].
\end{array}\right.
\tag{12}
```
...
\begin{aligned}
&a\in OT_B, a\neq0
&&\Longrightarrow& a[\bar n]&\in OT_B,\cr 
...
$`R(y,c)`$ と仮定すると $`y\lt c=0`$ であるが、これは (4) に反する。
...
```math
b\in OT_B,\qquad c\in OT_B,\qquad b\lt c.
\tag{18}
```
