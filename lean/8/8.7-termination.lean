...
- 状態: 🤖 **GREEN-MODULO 27**（sorry 0、axioms = propext/Classical.choice/Quot.sound）。
  下の `TerminationResidual` の 27 フィールドが、Lean 版の停止性定理と
...
| `exchV` / `m_8_5_TransCondV_oper_descend_engine` | `..._of_ExchV` (props2) | `ExchV_*` 6 本 |
| `operI_j0zero_trans_mult` | **未配線** | 自身 |
| `f7x_Trans_append_Pblocks` | **未配線** | 自身 |
| `m_7_3_Trans_leftmost_2` | **未配線** | 自身 |
...
残差は 27 本**（12 + 1 + 1 + 2 + 6 + 2 + 3）であって 28 本ではない。
...
条件付き定理は仮定が充足不能なら無価値なので、27 本の出所を確認した。
**結論: `TerminationResidual` は空虚な仮定の束ではない。** ただし 2 点留保がある。

- 27 本は**すべて Isabelle 側で証明済み**（各フィールドの docstring に
...
- 数値監査も家ごとに存在する: `OTdisp_*` 12 本 →
  ...
  `Exch84_*` / `CondVI*` / 未配線 3 本には専用監査が**無い**（留保 1）。
...
（配線 13 本）、`6.8-d1pos-final`（`rankSuccD1posLeg_proved`: `RankSuccD1posLeg` を
...
/-- **§8.7 停止性定理の残差 27 本**。これが潰れた瞬間、停止性定理は無条件になる。

内訳: OT 柱 12（`OTdisp_*`）＋ 条件 (I) 1 ＋ 条件 (II) 1 ＋ 条件 (III)/(IV) 2
＋ 条件 (V) 6（`ExchV_*`）＋ 条件 (VI) 2 ＋ 未配線 `FseqDesc_*` 3。 -/
structure TerminationResidual : Prop where
  ...
  /-- 条件 (V) (1/6)。`8.5-Trans-fseq-condV`:106。 -/
  exchVadmForms : ExchV_scbdec_adm_forms
  /-- 条件 (V) (2/6)。`8.5-Trans-fseq-condV`:87。 -/
  exchVc1Shape : ExchV_scbdec_c1_shape
  /-- 条件 (V) (3/6)。`8.5-Trans-fseq-condV`:95。 -/
  exchVsetup : ExchV_condV_setup
  /-- 条件 (V) (4/6)。`8.5-Trans-fseq-condV`:100。 -/
  exchVt2ne : ExchV_t2_nonzero_condV
  /-- 条件 (V) (5/6)。`8.5-Trans-fseq-condV`:138。 -/
  exchVnf3x : ExchV_nf3x
  /-- 条件 (V) (6/6)。`8.5-Trans-fseq-condV`:129（訂正 A28 適用後の原典ルート）。 -/
  exchVfseq : ExchV_scbdec_fseq_condV
  ...
  /-- 未配線 (2/3)。Isabelle `f7x_Trans_append_Pblocks` (pss_wip.thy:51888)。 -/
  transAppendPblocks : FseqDesc_f7x_Trans_append_Pblocks
  /-- 未配線 (3/3)。Isabelle `m_7_3_Trans_leftmost` (pss_wip.thy:16569) の (2)
  （要 `_pc` 16067、~400 行）。 -/
  transLeftmost2 : FseqDesc_m_7_3_Trans_leftmost_2
...
theorem Trans_fseq_descend (H : TerminationResidual) (M : PS) (n : ℕ)
  ...
  p_8_7_fseq_descend
    ...
    (FseqDesc_exchV_of_ExchV H.exchVadmForms H.exchVc1Shape H.exchVsetup
      H.exchVt2ne H.exchVnf3x H.exchVfseq)
    ...
    (FseqDesc_m_8_5_TransCondV_oper_descend_engine_of_ExchV H.exchVadmForms
      H.exchVc1Shape H.exchVsetup H.exchVt2ne H.exchVnf3x H.exchVfseq)
