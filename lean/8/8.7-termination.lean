...
import «8».«8.7-otmulti-condII-close»
...
- 状態: 🎉 **無条件 GREEN（2026-07-21 達成）**（sorry 0、仮定 0、
  axioms = propext/Classical.choice/Quot.sound）。旧 `TerminationResidual`
  （wave L 22→10 → … → wave AV で `condIIIVts` → wave AW で `otMultiIntCond` 陥落）
  は**全滅し、構造ごと削除済み**。`p_8_7_termination` ほか本ファイルの全公開定理は
  仮定なしで成立する。（以下の詳細表・残差論・空虚性節は残差 22 時点の史料。）
...
/-! ## 2. 残差 — **ゼロ本（2026-07-21、無条件化達成）**

かつてここに `TerminationResidual`（最大時 22 フィールド）が立っていた。
wave L〜AW ですべて陥落し、**本ファイルの全定理は仮定なし**になった。
最後の 2 本の落ち方:
- `condIIIVts`（§8.2 condII/IV 終切片）: `condIIIVterminalSlice_holds`
  （`8.2-condIIIV-close`、VE34 back-peel キャンペーン Wave AH〜AV の合流）。
- `otMultiIntCond`（OT 柱 interior）: `8.7-otmulti-narrow` の condII 還元
  `otMultiIntCond_of_condII_on` ＋ `otMultiInteriorCondII_holds`
  （`8.7-otmulti-condII-close`。独立二重証明 = `8.7-otmulti-condII-blueprint`）。
  下の `otMultiIntCond_term` が組み立てる。 -/
...
private theorem exch84producer_term :
    Exch84_condIIIIV_producer :=
  Exch84_condIIIIV_producer_holds (Exch84_condIIIIV_pkg_holds (exch84slicepkg_of_cornerReadouts_nc2 (cornerCoreReadouts_of_residual cornerNpSliceValue_holds_cnv)))

/-- 条件 (V) 塔の供給（wave Q 統合）: 単一残差 `ExchVMTowerResidual` から
`ExchV_M_tower`（`8.5-exchV-M-tower`）を経て、adm 塔と `nf3x` の両方を
`8.5-exchV-props2` で導出。 -/
private theorem exchVMtower_term : ExchV_M_tower :=
...
private theorem exchVresAdm_term : ExchVres_adm_M_tower :=
  exchVres_adm_M_tower_of_M_tower exchVMtower_term

private theorem exchVnf3x_term : ExchV_nf3x :=
  nf3x_holds_xv2 exchVMtower_term
...
private theorem otIIIIVdata_term : OTintIIIIV_transportData :=
  otIIIIVdata_of_otSetle (otSetle_holds (otSetleCore_of_parts_cc2 a0otNub_holds setleCensusSpine_holds_a3c))

private theorem otInt_term : OTdisp_OTint :=
  OTdisp_OTint_of_legs OTdisp_OTpred_holds
    (OTint_hp_condIII_of_slicepkg (exch84slicepkg_of_cornerReadouts_nc2 (cornerCoreReadouts_of_residual cornerNpSliceValue_holds_cnv)) otIIIIVdata_term)
    (OTint_hp_condIV_of_slicepkg (exch84slicepkg_of_cornerReadouts_nc2 (cornerCoreReadouts_of_residual cornerNpSliceValue_holds_cnv)) otIIIIVdata_term)
    (OTint_hp_condV_adm_holds exchVresAdm_term)
    (OTint_hp_condV_nadm_holds exchVnf3x_term)

/-- **旧・最終残差フィールド `otMultiIntCond` の無条件供給**（wave AW）。
`8.7-otmulti-narrow` の condII 還元 `otMultiIntCond_of_condII_on` に、OT 柱
`otInt_term`・降下柱の exchIII/IV/V 脚・condII interior close
（`8.7-otmulti-condII-close` の `otMultiInteriorCondII_holds`）を渡す。 -/
private theorem otMultiIntCond_term : OTmulti_interior_intCond_nc1 :=
  otMultiIntCond_of_condII_on otInt_term
    (FseqDesc_exchIII_of_Exch84 exch84producer_term Exch84_condIIIIV_noParent_holds_enp)
    (FseqDesc_exchIV_of_Exch84 exch84producer_term Exch84_condIIIIV_noParent_holds_enp)
    (FseqDesc_exchV_of_ExchV (adm_forms_holds exchVresAdm_term) c1_shape_holds
      condV_setup_holds t2_nonzero_condV_holds exchVnf3x_term fseq_condV_holds)
    otMultiInteriorCondII_holds

private theorem otMulti_term : OTdisp_OTmulti :=
  OTdisp_OTmulti_of_interior_om2 (otMultiInterior_holds (otMultiNotCondI_nc1_holds otMultiIntCond_term))
...
theorem Trans_STPS_OT_B (M : PS) (hM : STPS M) :
    Trans M ∈ OT_B :=
  Trans_preserves_OT
    (OTdisp_exchI_of_CondI scx_condI_j0pos_masterCF) (OTdisp_exchII_of_CondII (condII_masterCF_of_condIIIV condIIIVterminalSlice_holds))
    otInt_term OTdisp_OTpred_holds otMulti_term OTdisp_zerocol_predval_holds
...
private theorem transPreservesOT_term : TransPreservesOT :=
...
`ExchV_nf3x` の 2 本だけ。 -/
private theorem exchVadmForms_term :
    ExchV_scbdec_adm_forms :=
  adm_forms_holds exchVresAdm_term

private theorem fseqDescTOT_term :
...
theorem Trans_fseq_descend (M : PS) (n : ℕ)
    (hM : STPS M) (hn : 1 ≤ n) (hL : 1 < Lng M) :
    lessBT (Trans (oper M n)) (Trans M) = true :=
  p_8_7_fseq_descend
    fseqDescTOT_term
    (FseqDesc_exchI_of_CondI scx_condI_j0pos_masterCF)
    (FseqDesc_exchII_of_CondII (condII_masterCF_of_condIIIV condIIIVterminalSlice_holds))
    (FseqDesc_exchIII_of_Exch84 exch84producer_term Exch84_condIIIIV_noParent_holds_enp)
    (FseqDesc_exchIV_of_Exch84 exch84producer_term Exch84_condIIIIV_noParent_holds_enp)
    (FseqDesc_exchV_of_ExchV exchVadmForms_term c1_shape_holds
      condV_setup_holds t2_nonzero_condV_holds exchVnf3x_term fseq_condV_holds)
    (FseqDesc_exchVI_of_CondVI condVIadmTower_term condVInadm_term
      transPreservesOT_term)
    ...
    (FseqDesc_m_8_5_TransCondV_oper_descend_engine_of_ExchV
      exchVadmForms_term c1_shape_holds condV_setup_holds
      t2_nonzero_condV_holds exchVnf3x_term fseq_condV_holds)
...
private theorem PSSstep_invImage_term {N M : PS}
    (h : PSSstep N M) :
  ...
  show Trans (oper M n) ∈ OT_B ∧ Trans M ∈ OT_B
    ∧ lessBT (Trans (oper M n)) (Trans M) = true
  exact ⟨Trans_STPS_OT_B _ (STPS.oper hST n hn), Trans_STPS_OT_B M hST,
    Trans_fseq_descend M n hST hn hL⟩
...
theorem PSS_acc (M : PS) (_hM : STPS M) :
    Acc PSSstep M :=
  PSS_wf.apply M
...
theorem p_8_7_termination (f : ℕ → ℕ) (M : PS) (n : ℕ)
