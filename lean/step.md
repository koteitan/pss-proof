...
★ ただし **`PSS/*.lean`（定義層）を編集したときだけは `lake build` → kimina 再起動が必要**。
REPL は header 単位で永久にキャッシュされるので、再起動しないと新しい定義が snippet から見えない
（[kimina.md](kimina.md) の「ハマりどころ」参照）。命題ファイルを触るだけなら再起動は不要。

### (e′) 定義層を触ったら忠実性を検証し直す

`PSS/` の定義を変えたら、**python の正本モデルとチェックサムを突き合わせる**。
Lean 側で全列挙してハッシュを畳み、python 側で同じ式を回して数値が一致することを見る
（`oper` は現在 成分 `< 4`・長さ `≤ 4` の全 69,904 列 × `n=0..3` で一致を確認済み）。
定義がずれたまま下流を証明すると、**正しく証明された別の命題**が積み上がる。
